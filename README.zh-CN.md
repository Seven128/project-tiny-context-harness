...
- `compile` 生成 Product/Control/Non-completing/Technical Claim，拒绝未覆盖 Claim，保留首次不可重置 baseline，按 Outcome 计算风险，并冻结 owner/binding、resolved runner 与 verification inputs；active binding 位于 Git common-dir，并由 Git config marker 双向校验。
...
0.6.0 退休 V1 Schema/runtime 与 repo-local Hook。Upgrade 会安全删除旧 Hook，把未完成 V1 active state 报告为 `manual_required`，且绝不会把 V1 progress/Receipt 导入 V2 权威。Delivery Set、`composite-campaign` 与 `composite-long-task` 命令都是不可执行 tombstone。
