...
async function rejects(id, transform, pattern) {
  const root = await mkdtemp(path.join(os.tmpdir(), `ltw-${id}-`)); const workdir = await writeHappyV3Contract(root); const file = path.join(workdir, "product-architecture-source.yaml"); const source = await readFile(file, "utf8"); await writeFile(file, transform(source)); await assert.rejects(() => parseLongTaskSources(workdir), pattern);
}

test("yaml_duplicate_key", () => rejects("dup", (s) => `${s}product_goal: duplicate\n`, /unique|duplicate/i));
test("yaml_anchor_alias", () => rejects("alias", (s) => s.replace('product_goal: Deliver the capability', 'product_goal: &goal Deliver the capability').replace('observable_outcome: Oracle observes the product value', 'observable_outcome: *goal'), /anchor|alias/i));
...
test("V2 source is rejected without compatibility parsing", () => rejects("legacy-v2", (s) => s.replace("product-source-v3","product-source-v2"), /source_schema_unsupported/));
