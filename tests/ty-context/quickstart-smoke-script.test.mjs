...
const outDir = await mkdtemp(path.join(tmpdir(), "ty-context-quickstart-smoke-test-"));
const sourcePreviewOutDir = await mkdtemp(path.join(tmpdir(), "ty-context-source-preview-pack-test-"));
...
try {
  ...
  assert.match(sourcePreviewResult.stdout, /npx --no-install ty-context init --adopt/);
