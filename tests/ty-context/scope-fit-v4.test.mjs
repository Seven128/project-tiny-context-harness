...
async function publish(root,campaignPath,scope,label){const scopeFile=`${label}-scope.json`;const coverageFile=`${label}-coverage.json`;const owners=new Map();for(const slice of scope.slices)for(const sourceRef of slice.source_refs)owners.set(sourceRef,[...(owners.get(sourceRef)??[]),slice.slice_id]);const coverage={schema_version:"composite-source-coverage-v2",source_plan_sha256:scope.request_sha256,items:[...owners].map(([source_item_id,slice_refs])=>({source_item_id,statement:"outcome",disposition:"slice",slice_refs,global_constraint_refs:[],rationale:"all capacity partitions preserve the source item",context_resolution:{status:"existing",context_refs:["project_context/context.toml"],task_local_reason:null}})),global_constraint_bindings:[]};await writeFile(path.join(root,scopeFile),JSON.stringify(scope));await writeFile(path.join(root,coverageFile),JSON.stringify(coverage));return applyCampaignScopeV5(root,campaignPath,scopeFile,coverageFile);}
async function writeContextFixture(root){const files={"project_context/context.toml":`[[areas]]\nid = "main"\nroot = "."\ncontext = "project_context/areas/main.md"\nkind = "app"\ndefault = true\n`,"project_context/global.md":"# Global Context\n\n- Freeze durable project facts.\n","project_context/architecture.md":"# Architecture Context\n\n- Keep Campaign boundaries deterministic.\n","project_context/areas/main.md":"# Main Area\n\n- Own the fixture outcome.\n"};for(const [relative,content] of Object.entries(files)){const target=path.join(root,...relative.split("/"));await mkdir(path.dirname(target),{recursive:true});await writeFile(target,content,"utf8");}}
