...
test("semantic references cover split decisions, repair stops, review, start, and continuation", async () => {
  ...
  assert.match(scope, /schema_version.*scope-fit-result-v3/is);
  assert.doesNotMatch(`${scope}\n${authoring}\n${lifecycle}`, /selected_slice_id|slice_id|--slice|ScopeFitResultV2/u);
  ...
  assert.match(authoring, /product-source-v3/);
  assert.match(authoring, /technical-plan-v3/);
  assert.match(authoring, /acceptance-checklist-v3/);
