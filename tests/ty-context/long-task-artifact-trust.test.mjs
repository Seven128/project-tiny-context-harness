...
test("recursive artifact glob accepts a deeply nested Playwright trace",async()=>{const r=await root();const trace=path.join(r,"playwright-output","CMD-PW","case-chromium","trace.zip");await mkdir(path.dirname(trace),{recursive:true});await writeFile(trace,"trace");const result=await collectFrozenArtifacts({id:"VS-PW",artifact_globs:["**/trace.zip"]},r,new Date(0).toISOString());assert.deepEqual(result.artifacts.map((item)=>item.path),["playwright-output/CMD-PW/case-chromium/trace.zip"]);});
