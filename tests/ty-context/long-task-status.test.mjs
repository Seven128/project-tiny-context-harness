...
test("blocked verification remains internal needs_work",async()=>{const dir=await mkdtemp(path.join(os.tmpdir(),"ltw-status-"));const run={contract_sha256:"a",run_id:"RUN-2",findings:[],spec_results:[{status:"blocked"}]};assert.equal((await writeLongTaskStatus(dir,run)).workflow_status,"needs_work");});
