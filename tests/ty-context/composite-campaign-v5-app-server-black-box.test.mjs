...
test("Campaign V5 Fake App Server authors on dedicated threads, routes Sol execution, runs two worktrees, and reaches the sole final gate",{timeout:120_000},async()=>{
  ...
  await writeFile(path.join(root,"src","value-b.txt"),"good\n");
  const oracle=await readFile(path.join(root,"tests","acceptance","oracle.mjs"),"utf8");await writeFile(path.join(root,"tests","acceptance","oracle-b.mjs"),oracle.replaceAll("value.txt","value-b.txt").replaceAll("value.read","value.read.b"));
  ...
  const packetA=await packet(task,"campaign-v5-e2e","SFC-001","SRCU-001",value=>value);packetA.authorities.product_architecture_source.requirements[0].context_refs=["project_context/a.md"];
  const packetB=await packet(task,"campaign-v5-e2e","SFC-002","SRCU-002",value=>replaceDeep(value,[[/src\/value\.txt/gu,"src/value-b.txt"],[/tests\/acceptance\/oracle\.mjs/gu,"tests/acceptance/oracle-b.mjs"],[/value\.read/gu,"value.read.b"]]));packetB.authorities.product_architecture_source.requirements[0].context_refs=["project_context/b.md"];
  ...
  assert.equal(result.status,"accepted");assert.equal((await readFile(path.join(root,"src","value.txt"),"utf8")).trim(),"good");assert.equal((await readFile(path.join(root,"src","value-b.txt"),"utf8")).trim(),"good");
  ...
  const author=starts.filter(row=>row.message.params.sandboxPolicy.type==="readOnly");const execution=starts.filter(row=>row.message.params.sandboxPolicy.type==="workspaceWrite");assert.equal(author.length,2);assert.equal(execution.length,3);assert.ok(author.every(row=>row.message.params.model==="gpt-5.6-sol"&&row.message.params.effort==="xhigh"));assert.ok(execution.every(row=>row.message.params.model==="gpt-5.6-sol"&&row.message.params.effort==="medium"&&path.isAbsolute(row.message.params.cwd)));assert.equal(new Set(starts.map(row=>row.message.params.threadId)).size,2);
  ...
  const indexedExecution=log.map((row,index)=>({row,index})).filter(({row})=>row.direction==="in"&&row.message.method==="turn/start"&&row.message.params.sandboxPolicy.type==="workspaceWrite");const initialExecution=indexedExecution.slice(0,2);const initialTurnIds=responseTurnIds(log,initialExecution.map(({row})=>row));const firstExecutionComplete=log.findIndex(row=>row.direction==="out"&&row.message.method==="turn/completed"&&initialTurnIds.has(row.message.params.turn.id));assert.ok(Math.max(...initialExecution.map(({index})=>index))<firstExecutionComplete);
});
