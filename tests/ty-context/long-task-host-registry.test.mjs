...
async function active(root) {
  const { getActiveLongTask } = await import("../../packages/ty-context/dist/lib/long-task-host-client.js");
  const value = await getActiveLongTask(root);
  assert.ok(value, "Host registry must retain the active authority");
  return value;
}
...
test("weaken_sources_then_recompile", async () => {
...
test("replace_oracle_then_recompile", async () => {
...
test("compile_second_workdir", async () => {
...
test("delete_repo_pointer_but_host_registry_exists", async () => {
...
test("rewrite_both_repo_pointers", async () => {
...
test("same_contract_idempotent_compile", async () => {
...
test("deleted or forged workdir contract mirror cannot replace the sealed Host contract", async () => {
  const { root, workdir, contract_sha256 } = await fixture("contract-mirror");
  const { readAuthoritativeLongTaskContract } = await import("../../packages/ty-context/dist/lib/long-task-host-client.js");
  ...
  assert.equal((await readAuthoritativeLongTaskContract(workdir)).contract.contract_sha256, contract_sha256);
  await rm(file, { force: true });
  assert.equal((await readAuthoritativeLongTaskContract(workdir)).contract.contract_sha256, contract_sha256);
  assert.equal((await active(root)).contract_sha256, contract_sha256);
});

test("failed compile does not strand a reservation", async () => {
  ...
  const { getActiveLongTask } = await import("../../packages/ty-context/dist/lib/long-task-host-client.js");
  assert.equal(await getActiveLongTask(root), null);
  await writeFile(file, original);
  const recovered = runCompositeCompile(root, workdir);
  assert.equal(recovered.status, 0, `${recovered.stdout}\n${recovered.stderr}`);
  assert.ok(await getActiveLongTask(root));
});
