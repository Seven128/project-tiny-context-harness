...
export async function writeHappyV3Contract(root, mutate = () => {}) {
  ...
  await Promise.all([
    ...
    writeFile(
      ...
      "# Main Area\n\n- Owns the fixture capability.\n",
      "utf8",
    ),
  ]);
  const task = await writeHappyV3Inputs(root, mutate);
  await installProjectCompletionGate(root);
  await exec("git", ["init"], { cwd: root });
  await exec(
    "git",
    ["config", "user.email", "long-task-v3-fixture@example.invalid"],
    { cwd: root },
  );
  await exec("git", ["config", "user.name", "Long Task V3 Fixture"], {
    cwd: root,
  });
...
export async function writeHappyV3Inputs(root, mutate = () => {}) {
  ...
  await Promise.all(
    [
      ...
      writeFile(path.join(task, file), YAML.stringify(value), "utf8"),
    ),
  );
  return task;
}
...
export function runCompositeCompile(root, task) {
  return spawnSync(
    process.execPath,
    [candidateCli, "composite-long-task", "compile", task],
    {
      cwd: root,
      encoding: "utf8",
...
export function observationV2Envelope(
  value = "good",
  bindingId = "IB-002",
  ...
  return {
    ...
    observations: {
      works: {
        ...
        actual: { binding_id: bindingId, capability, value },
        artifact_refs: [],
      },
...
export function observationV2OracleScript(
  value = "good",
  bindingId = "IB-002",
  capability = "value.read",
  delayMs = 0,
...
export function addSecondRequirementBranch(data) {
  data.product.owner_surfaces.push({
    id: "OS-SECONDARY",
    kind: "runtime",
    location: "secondary",
    primary_action: "run",
    expected_feedback: "secondary-visible",
  });
  data.product.requirements.push({
    id: "PR-002",
    statement: "Secondary capability works",
    observable_outcome: "Secondary result is visible",
    owner_boundary: "secondary",
    owner_surface_refs: ["OS-SECONDARY"],
    context_refs: ["project_context/context.toml"],
    population_policy: "not_applicable",
  });
  data.product.boundaries.push({
    id: "PB-002",
    rule: "Secondary boundary",
    requirement_refs: ["PR-002"],
  });
  data.product.non_completing_outcomes.push({
    id: "NCO-002",
    forbidden_outcome: "Secondary no-op",
    requirement_refs: ["PR-002"],
  });
    ...
    obligations: [
      {
        ...
        forbidden_shortcuts: [
          {
            id: "FS-002",
            statement: "No secondary no-op",
            source_boundary_ids: ["PB-002"],
            source_non_completing_ids: ["NCO-002"],
          },
        ],
  ...
  data.plan.counterfactual_controls.push({
    id: "CF-PI-002-OB-001",
    obligation_ids: ["PI-002-OB-001"],
    mutation: { type: "remove_binding_targets", binding_ids: ["IB-003"] },
    expected_failed_assertion_ids: ["PA-002"],
  });
  data.checklist.proof_requirements.push({
    id: "PRF-AC-002-RUNTIME",
    proof_surface: "runtime_behavior",
    obligation_refs: ["PI-002-OB-001"],
    owner_surface_refs: ["OS-SECONDARY"],
    verification_spec_ids: ["VS-AC-002"],
  });
  data.checklist.acceptance_criteria.push({
    id: "AC-002",
    title: "Secondary runtime works",
    obligation_refs: ["PI-002-OB-001"],
    validates: ["secondary runtime behavior"],
    does_not_validate: ["primary runtime behavior"],
    proof_requirement_refs: ["PRF-AC-002-RUNTIME"],
    verification_spec_ids: ["VS-AC-002"],
  });
  data.checklist.verification_specs.push(
    makeSpec("002", {
      requirement: "PR-002",
      planItem: "PI-002",
      obligation: "PI-002-OB-001",
      bindings: ["IB-003", "IB-004"],
      ac: "AC-002",
      proof: "PRF-AC-002-RUNTIME",
      boundary: "PB-002",
      outcome: "NCO-002",
      shortcut: "FS-002",
    }),
      ...
      owner_surfaces: [
        {
          id: "OS-RUNTIME",
          kind: "runtime",
          location: "value-reader",
          primary_action: "read-value",
          expected_feedback: "good-visible",
        },
      ],
      requirements: [
        {
          id: "PR-001",
          statement: "Capability works",
          observable_outcome: "Oracle observes the product value",
          owner_boundary: "runtime",
          owner_surface_refs: ["OS-RUNTIME"],
          context_refs: ["project_context/context.toml"],
          population_policy: "not_applicable",
        },
      ],
      boundaries: [
        { id: "PB-001", rule: "No shortcut", requirement_refs: ["PR-001"] },
      ],
      non_completing_outcomes: [
        {
          id: "NCO-001",
          forbidden_outcome: "No-op",
          requirement_refs: ["PR-001"],
        },
      ],
              ...
              forbidden_shortcuts: [
                {
                  ...
                  source_non_completing_ids: ["NCO-001"],
                },
              ],
      ...
      counterfactual_controls: [
        {
          ...
          mutation: { type: "remove_binding_targets", binding_ids: ["IB-001"] },
      ...
      acceptance_criteria: [
        {
          ...
          does_not_validate: ["unrelated behavior"],
          proof_requirement_refs: ["PRF-AC-001-RUNTIME"],
          verification_spec_ids: ["VS-AC-001"],
        },
      ],
      verification_specs: [
        makeSpec("001", {
          requirement: "PR-001",
          planItem: "PI-001",
          obligation: "PI-001-OB-001",
          bindings: ["IB-001", "IB-002"],
          ac: "AC-001",
          proof: "PRF-AC-001-RUNTIME",
          boundary: "PB-001",
          outcome: "NCO-001",
          shortcut: "FS-001",
        }),
...
function makeSpec(suffix, refs) {
  const runtimeExpected =
    suffix === "001"
      ? { binding_id: "IB-002", capability: "value.read", value: "good" }
      : { binding_id: "IB-004", capability: "value.secondary", value: "good" };
  return {
    id: `VS-AC-${suffix}`,
    runner_type: "node_oracle",
    proof_capabilities: ["runtime_behavior"],
    claims: {
      requirement_ids: [refs.requirement],
      plan_item_ids: [refs.planItem],
      obligation_ids: [refs.obligation],
      binding_ids: refs.bindings ?? [refs.binding],
      ac_ids: [refs.ac],
      proof_requirement_ids: [refs.proof],
    },
    ...
    command_steps: [
      {
        id: `CMD-${suffix}`,
        tool: "node_script",
        target: "tests/acceptance/oracle.mjs",
        argv: [],
        cwd: ".",
        timeout_ms: 10000,
        environment_refs: [],
    ...
    positive_assertions: [
      {
        id: `PA-${suffix}`,
        observation_id: "works",
        observation_kind: "runtime_behavior",
        operator: "equals",
    ...
    negative_assertions: [
      {
        ...
        operator: "not_equals",
        expected: "forbidden",
        source_boundary_ids: [refs.boundary],
        source_non_completing_ids: [refs.outcome],
        source_forbidden_shortcut_ids: [refs.shortcut],
      },
    ],
