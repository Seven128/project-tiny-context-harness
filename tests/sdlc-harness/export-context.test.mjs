...
test("export-context writes default single-file code implementation artifact", async () => {
  ...
  try {
    ...
    assert.equal(
      ...
      "tmp/sdlc/context-exports/code-level-implementation-20260607T080910Z/code-level-implementation.md"
    );
    assert.equal(report.sourceCodeCount, report.sourceFiles.length);
    await assert.rejects(
      stat(path.join(root, "tmp", "sdlc", "context-exports", "code-level-implementation-20260607T080910Z", "all.md"))
    );
...
test("export-context check reports sources without writing artifact", async () => {
  ...
  try {
    ...
    const codeReport = await runExportContext(root, {
      code: true,
      check: true,
      output: "tmp/sdlc/context-exports/code-check.md"
    });
    assert.equal(codeReport.wrote, false);
    assert.equal(codeReport.outputRelativePath, "tmp/sdlc/context-exports/code-check.md");
...
test("export-context --all check reports both default artifacts without writing", async () => {
  ...
  try {
    ...
    const fullOutput = /- full: (tmp\/sdlc\/context-exports\/full-project-context-\d{8}T\d{6}Z\.md)/.exec(
      result.stdout
    )?.[1];
    const codeOutput =
      /- code: (tmp\/sdlc\/context-exports\/code-level-implementation-\d{8}T\d{6}Z\/code-level-implementation\.md)/.exec(
        result.stdout
      )?.[1];
