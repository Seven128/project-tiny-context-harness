...
- Open Design remains the default generator. Explicit `design-system-authoring` adopts project Design Authority; `design-resource-authoring` commissions scoped resources and reconciles final selection once. Optional Figma-native inspection freezes exact version/node facts plus the editable-upstream route. It never edits a Source Plan, Context/`DESIGN.md`, code or Contract; resources remain ordinary Context-reachable Source.
- Selected implementation handoffs share one fail-closed residual `design-resource-handoff-v1`: native facts are referenced, while scope, conditions, gaps, product semantics, blockers and downstream bindings remain explicit. Exploration stays lightweight.
...
- External generation remains delegated and durable adoption routes to `context_uiux_design`. Optional Figma-native input adds no provider runtime or authority; the residual adapter validates semantic coverage/integrity, while consumers own conformance and acceptance.
...
- Design-resource input adapter: provider-neutral `design-resource-handoff-*`, `commands/design-resource.ts`, `long-task-design-resource-handoff.ts`, plus package-managed `design-resource-authoring/references/figma-native-handoff.md` for optional native acquisition and freeze rules.
