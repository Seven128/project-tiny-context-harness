...
- Provide explicitly invoked composite preparation and strict V2 execution capabilities that keep campaign authoring/provenance separate from temporary verifier runs and make accepted delivery depend on an immutable contract, active verifier, one final snapshot and Codex Stop enforcement rather than model prose.
...
- `validate-plan-contract` and ordinary `validate-plan-acceptance` remain separate temporary-artifact consistency checks and are not product proof. Composite V2 has no generic task-state validator: compile/verify/final-gate/stop-check own its contract, product observations and completion decision.
- `ty-context composite-long-task ...` is the public strict executor with only `init`, `compile`, `verify`, `status`, `final-gate`, `stop-check` and `render-goal`. It compiles the three YAML authorities and registered Context into `compiled-contract.json`, runs exact frozen verification specs in isolated snapshots, writes machine findings to `current-status.json`, reruns all specs in one final snapshot, writes `final-result.json` atomically and allows Codex completion only through the mandatory Stop Hook. The agent cannot submit a command, evidence artifact, assertion result, AC/PI state or closed proof layer.
- `/prepare-composite-long-task` is the explicit upstream path for a raw requirement that needs multi-SFC composite preparation. It records Scope Fit with stable non-renumbering SFC identifiers, selects only a unique dependency-ready candidate automatically, authors only the current SFC as `CompositeAuthoringPacketV2`, renders strict Product / Architecture Source, Technical Realization Plan and Acceptance Checklist YAML deterministically, and requires complete coverage plus oracle-ready preflight without creating runtime proof.
...
- Campaign request, coordination, audit and packet revisions are Git-trackable user-owned authoring/provenance. Compiled contracts, command runs, artifacts, status and final results remain under `tmp`; V2 has no aggregate campaign product-completion state. Unknown schema majors fail fast, and `init`, `sync` and `upgrade` never create, scan or migrate campaigns. Old V1/Markdown campaigns and workdirs are unsupported and receive no compatibility reader or migration.
...
- Context graph helpers live in `packages/ty-context/src/lib/context-manifest.ts`; validator dispatch lives in `packages/ty-context/src/lib/validators.ts`; plan artifact validator helpers live in `packages/ty-context/src/lib/plan-*-validator.ts`. Composite V2 is split across `long-task-contract-*`, `long-task-snapshot`, `long-task-verifier`, `long-task-command-runner`, `long-task-artifact-collector`, `long-task-assertion-evaluator`, `long-task-status`, `long-task-final-gate` and `long-task-stop-check` modules. The old state/evidence/slice/derived runtime has been removed.
...
- Composite preparation's durable data contract is request provenance -> campaign coordination plus append-only audit -> immutable `CompositeAuthoringPacketV2` revisions -> deterministic YAML projections -> frozen packet/projection/oracle handoff. After explicit start, `compiled-contract.json`, verifier-owned runs and the current full final gate retain execution and SFC completion authority.
