...
- Compile is static and fail-closed. Source/Context/Product/Global authority materials cannot be silently refrozen; reduced Check input coverage and weakened required outputs are Authority Revisions; one repository-pattern AST governs matching/subset/overlap; missing or incomparable Observations cannot prove Claims; every Global Claim and Outcome needs executable falsifiable proof; UI needs browser proof; requested risk cannot fall below the deterministic minimum; and strict risk adds compiler-enforced proof/recovery obligations.
...
- Status, progress, Receipts and the workdir compiled cache are audit/recovery projections only. The common-dir Active Authority V3 snapshot is the sole previous-authority source; its record and task/revision/identity Git-config marker must agree, and successful Stop/close atomically clears both.
