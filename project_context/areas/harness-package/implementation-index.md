...
- Mandatory Source marker parsing/inventory/text and disposition continuity: `long-task-source-item-parser.ts`, `long-task-source-inventory.ts`, `long-task-source-continuity.ts`, `long-task-source-shape.ts`, `long-task-source-claim-validation.ts`.
...
- Claim/all-of proof, adapter/Raw Execution Observation ownership, Counterfactual Claim-to-Binding policy, authority field policy, paths/risk and runner freeze: `long-task-claims.ts`, `long-task-claim-proof-policy.ts`, `long-task-evidence-adapter-policy.ts`, `long-task-observation-ownership.ts`, `long-task-counterfactual-claim-policy.ts`, `long-task-authority-policy.ts`, `long-task-check-execution-policy.ts`, `long-task-runner-freeze.ts`, `long-task-risk.ts`, `long-task-paths.ts`.
...
- AC-level multi-project Playwright decoding, detailed assertion evaluation, per-Check artifacts, passed-Check-only Claim/Population proof and Binding/value-sensitive Counterfactual evaluation: `long-task-playwright-evidence.ts`, `long-task-check-evidence-decoder.ts`, `long-task-assertions-v2.ts`, `long-task-artifacts.ts`, `long-task-evidence-v2.ts`.
...
- Delivery Contract Compact/parser/compiler/risk/preflight/Source-AC coverage: `tests/ty-context/long-task-delivery-*.test.mjs` plus `long-task-closure-invariants.test.mjs`, `long-task-source-authority-closure.test.mjs` and focused authoring/Requirement/Observation tests.
