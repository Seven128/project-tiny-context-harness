...
- Composite Campaign V6 Git/execution: `composite-campaign-worktree-expectation-v6.ts` derives the sole canonical expected set; `composite-campaign-worktree-budget.ts` separates pure inspection/budget assertion from explicit reconcile/cleanup and owns fixed paths plus the one/four/one/six limits. `composite-campaign-state-transition-v6.ts` centralizes monotonic Slice/Wave transitions. `process-identity.ts`, `composite-campaign-reconcile-v6.ts`, `composite-campaign-bootstrap-v6.ts`, `composite-campaign-runtime-helpers-v6.ts`, `composite-campaign-control-v6.ts` and `composite-campaign-runner-types-v6.ts` own cross-platform process identity, durable-stage/Integration-head reconciliation, dispatch guards, interrupts, abandon/cleanup and shared scheduler types. `composite-campaign-receipt.ts` owns Receipt V3 bindings alongside historical receipt readers.
...
- Campaign V6 infrastructure tests are `codex-exec-client-v1.smoke.test.mjs`, `composite-campaign-worktree-budget-v6.smoke.test.mjs`, `composite-campaign-worktree-reconcile-v6.smoke.test.mjs`, `composite-campaign-integration-head-v6.test.mjs`, `composite-campaign-interrupt-v6.test.mjs`, `composite-campaign-abandon-v6.smoke.test.mjs`, `composite-campaign-exec-routing-v6.test.mjs` and `composite-campaign-cli-v6-dry-run.test.mjs`. They use an ordinary fake child executable and temporary real Git repositories; they do not run a real Codex Campaign. Historical V5/AppServer runtime tests are legacy/audit-only and are excluded from active V6 runtime coverage.
