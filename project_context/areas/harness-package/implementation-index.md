...
- Composite Campaign V4 schema/store entry: `composite-campaign-schema-v4.ts` and `composite-campaign-v4.ts` own the no-compatibility V4 campaign state, immutable discussed-plan/packet/schedule identities, safe tracked persistence and derived status transitions. The former Scope Fit V2 / single-Slice `composite-campaign-v3.ts` API is not exported or retained; Contract V3 remains only as the per-Slice acceptance worker beneath V4.
- Composite Campaign V4 planning: `composite-campaign-graph.ts`, `composite-campaign-source-coverage.ts`, `composite-campaign-conflicts.ts` and `composite-campaign-scheduler.ts` own Scope Fit V3 DAG/global-constraint validation, complete source coverage, positive-evidence conflict reasons and deterministic ready-frontier waves.
- Composite Campaign V4 Git/execution: `git-worktree-paths.ts`, `composite-campaign-git-baseline.ts`, `composite-campaign-worktree.ts`, `composite-campaign-goal-manifest.ts` and `composite-campaign-receipt.ts` own Git-safe path resolution, checkpoint/base/integration topology, isolated Slice/repair worktrees, frozen Goal objectives and commit-before-final receipt validation.
- Composite Campaign V4 integration: `composite-campaign-integration.ts`, `composite-campaign-final-gate.ts` and `composite-campaign-orchestrator.ts` own deterministic merge/repair actions, impact-aware Wave Integration Gates, one-snapshot full Campaign recomputation, target resynchronization/finalization and the public `advance` action state machine.
...
- Composite Campaign V4 and preparation Skill behavior lives in `tests/ty-context/composite-campaign-*.test.mjs` and `tests/ty-context/prepare-composite-long-task-skill.test.mjs`: graph/source coverage, conservative scheduler, real Git worktrees/per-worktree bindings, CLI actions/recovery, immutable `CompositeAuthoringPacketV3` projection, Slice receipts, merge/repair, Integration/Campaign gates and target finalization. `tests/ty-context/composite-campaign-v4-black-box.test.mjs` owns the four real temporary-Git/Fake-Goal scenarios; real Codex Goal smoke is development-only.
