...
- Long-Task V2 commands: `packages/ty-context/src/commands/long-task.ts`, `long-task-revision.ts` and `long-task-command-args.ts`; the thin entry router delegates revision compile/diagnosis/approval, projects one hash-bound pending approval summary with exact semantic/Claim/proof/external-confirmation keys, classifies supporting-only Context revisions before derived Progress invalidation, emits `authority_revision_adopted` as a non-completion event that returns to rolling execution, and emits the additive one-time `execution_model_checkpoint` on first Compile.
...
- Strict YAML, Compact defaults and shape parsing: `strict-codec.ts`, `long-task-delivery-shape.ts`, `long-task-check-shape.ts`, `long-task-root-shape.ts`, `long-task-delivery-parser.ts`, `long-task-outcome-parser.ts`, `long-task-delivery-validation.ts`, `long-task-semantic-drift-migration.ts`. Target profile plus required target refs, runtime-family/root entrypoint, ordered stages, vertical success/degradation roles, Given/When scenarios, all-of evidence capabilities and typed external-impact declarations live in these canonical V2 surfaces. Old V2 contracts receive an actionable manual migration diagnostic rather than inferred proof semantics.
- Composition and protected authority: `long-task-boundary-check.ts`, `long-task-authority.ts`, `long-task-authority-revision*.ts`, `long-task-authority-revision-analysis.ts`, `long-task-protected-files.ts`, `long-task-progress.ts` and `long-task-risk-surfaces.ts`; canonical Source/Context/Product/Global revision materials, separated Check/Outcome revision analysis, three-way classification, exact external-confirmation change detection and concise material approval summary live in the authority modules.
...
- External design-resource consumption: managed default workflow, `context_uiux_design` and Long-Task authoring/evidence references classify candidate versus selected Source, preserve exact-target/constraint coverage and bind stable acceptance-affecting resources without a package-owned generation Skill or pack schema.
- Default Workflow and engineering Skills require manifest routing plus one bounded text search over `project_context/**` before `Context Delta`; material production UI additionally uses UI Authority Closure and the conditional Design Authority Check. Managed `product-surface-contract.md` and `screen-contract.md` provide cross-surface and optional per-screen/control structures without new roles or state.
- Full Long-Task Control projection: schema and `long-task-contract-types.ts`, `long-task-product-shape.ts`, `long-task-claim-definitions.ts`, `long-task-source-target-index.ts`, `long-task-authority-materials.ts`, `long-task-authority-material-diff.ts`, `long-task-authority-types.ts` and `long-task-authority-policy.ts`.
- Design Authority scaffold/advisory: `packages/ty-context/src/lib/design-md.ts`, `doctor.ts` and `tests/ty-context/sync-init-doctor.test.mjs`; managed workflow/Skill/template parity and Long-Task UI authority boundaries are covered by `tests/ty-context/visual-delivery-guidance.test.mjs`, `surface-contract-workflow.test.mjs` and `workflow-contract-routing.test.mjs`; `tools/affected_test_selection.mjs` maps runtime hotspots back to focused coverage.
- Active Long-Task Skill: `.codex/skills/long-task-workflow/SKILL.md` and managed/package copies; it owns the one-time post-Authority-Lock model choice, no automatic model switch/persistent model state, no proactive parallel-subagent scheduling, live `Context Delta`, blocker-to-protected-revision classification, post-adoption rolling return, risk-proportional causal evidence review, current-execution target-runtime proof, coalesced rolling runtime feedback, terminal Goal-to-Source veto and exact progress/final-status reporting. Its Contract-authoring, Evidence-design and Authority-lifecycle references own the detailed boundaries.
...
- External design-resource adoption and visual authority boundaries: `tests/ty-context/visual-delivery-guidance.test.mjs`, `workflow-contract-routing.test.mjs` and focused Long-Task design/claim coverage.
