...
- Composite Campaign V5 schema/store entry: `composite-campaign-schema-v5.ts`, `scope-fit-v4.ts` and `composite-campaign-v5.ts` own Campaign/Scope schema validation, immutable discussed-plan/packet/schedule identities, safe tracked persistence, host/thread/profile state and derived status transitions. `composite-campaign-v4.ts` remains an audit-only reader; V4 campaigns cannot enter automatic execution. Contract V3 remains the per-Slice acceptance worker beneath V5.
...
- Campaign thread execution: `composite-campaign-thread-state.ts`, `composite-campaign-thread-orchestrator.ts`, `composite-campaign-packet-author.ts`, `composite-campaign-goal-runner.ts` and `composite-campaign-host-recovery.ts` own the one-thread Authoring/Execution lifecycle, serialized packet application, Goal binding, concurrent wave turns, same-thread repair/resume, one reconnect and fail-closed launch reconciliation. The pure `advanceCampaignV4()` planner remains adapter-free and emits actions consumed by this V5 host layer.
