...
- Composite Campaign V5 Git/execution: `git-worktree-paths.ts`, `composite-campaign-git-baseline.ts`, `composite-campaign-worktree.ts`, `composite-campaign-change-envelope.ts`, `composite-campaign-transaction-store.ts`, `composite-campaign-transaction-artifacts.ts`, `composite-campaign-transaction-io.ts`, `composite-campaign-transaction-recovery.ts`, `composite-campaign-goal-manifest.ts` and `composite-campaign-receipt.ts` own Git-safe path resolution, nonintrusive hidden-ref checkpoints, isolated Slice/repair worktrees, closed Change Envelopes, split WAL/lease/artifact/recovery mechanics, Goal Manifest V2 objectives and commit-before-final receipt validation.
- Composite Campaign V5 integration: `composite-campaign-integration.ts`, `composite-campaign-wave-impact.ts`, `composite-campaign-final-gate.ts` and `composite-campaign-orchestrator.ts` own deterministic merge/repair actions, actual-diff impact-aware Wave Integration Gates, one-shared-snapshot full Campaign recomputation with exact verifier-result reuse, target resynchronization/finalization and the adapter-free public `advance` action state machine. The App Server runner is a separate orchestration adapter.
- Capability profile selection and explicit enablement: `packages/ty-context/src/lib/profiles.ts` and `packages/ty-context/src/commands/enable.ts`.
...
- Legacy plan-acceptance artifact validator behavior: `tests/ty-context/plan-validators.test.mjs`.
