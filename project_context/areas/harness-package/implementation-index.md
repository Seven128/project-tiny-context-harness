...
- Context evolution and Progress retention: `context-graph-snapshot.ts` classifies selected files, `long-task-context-authority.ts` normalizes legacy snapshots and compares Controlling Context, `long-task-authority-materials.ts`/`long-task-authority-material-diff.ts` distinguish supporting-only revisions, and `long-task-progress.ts` binds Progress to the controlling projection.
...
- Semantic authority, Context evolution, field completeness, conservative pattern containment/overlap and Global Claim coverage: `long-task-semantic-authority-revision.test.mjs`, `long-task-context-evolution.test.mjs`, `long-task-authority-field-completeness.test.mjs`, `long-task-pattern-containment.test.mjs`, `long-task-pattern-overlap.test.mjs`, `long-task-global-claim-coverage.test.mjs`.
...
- Efficiency boundaries and affected routing: `tests/ty-context/long-task-efficiency-design.test.mjs`, `tests/ty-context/affected-test-selection.test.mjs`, `tests/ty-context/affected-test-portable-command.test.mjs`, `tools/affected_test_selection.mjs`, `tools/npm_command_spec.mjs`, `tools/run_affected_tests.mjs`.
...
- Suite partitioning: `tests/ty-context/run-package-suite.mjs`; root scripts expose `test:affected`, `test:long-task:focused` and `test:delivery-contract:focused`, while package scripts retain complete `test:delivery-contract`, `test:long-task-workflow` and independent `test:long-task-performance` gates.
