...
- Default Workflow and engineering Skills require manifest routing plus one bounded text search over `project_context/**` before `Context Delta`; material production UI additionally uses the conditional Design Authority Check. This remains guidance, not a search service, required artifact tree or persisted state.
- Design Authority scaffold/advisory: `packages/ty-context/src/lib/design-md.ts`, `doctor.ts` and `tests/ty-context/sync-init-doctor.test.mjs`; managed workflow/Skill parity and Long-Task visual authoring boundaries are covered by `tests/ty-context/visual-delivery-guidance.test.mjs` and `workflow-contract-routing.test.mjs`; `tools/affected_test_selection.mjs` maps scaffold/advisory changes back to this focused coverage.
