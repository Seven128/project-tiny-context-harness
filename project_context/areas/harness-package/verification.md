...
- `node packages/ty-context/dist/cli.js composite-campaign app-server-check --json`
  - Manual transport/catalog preflight only. It initializes the installed real App Server and lists models; it does not prove SFC authoring, Goal execution or acceptance.
- `node packages/ty-context/dist/cli.js composite-campaign run --campaign <prepared-one-sfc-campaign> --controller-model <actual-model> --controller-effort <actual-effort> --json`, followed by `node packages/ty-context/dist/cli.js composite-campaign status --campaign <prepared-one-sfc-campaign> --json`
  - Manual real lifecycle smoke only, after `create` and `apply-scope` have produced a valid one-SFC Campaign V5 fixture. Expected signal: the real App Server performs Authoring Turn -> preflight -> Goal -> Execution Turn and status derives `accepted`; it is never a CI/publication gate and has no fake or manual-Goal fallback.
