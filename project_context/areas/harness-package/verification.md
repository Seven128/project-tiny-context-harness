...
- `node --test tests/ty-context/composite-campaign-v5-app-server-black-box.test.mjs`
  - This is the focused Campaign V5 orchestration gate using real temporary Git repositories and the local JSONL Fake App Server. It must cover `initialize`, `model/list`, thread start/resume/read, Goal set/get, turn start/completed notifications and interrupt without contacting real Codex services.
  - Required scenarios: Authoring Turn -> structured Packet -> serialized preflight -> Goal -> same-thread Execution Turn; Sol xhigh/max exact routing and lower/unrelated/unknown/unavailable passthrough; all independent wave turns start before waiting; `needs_work` repairs on the same thread; interruption resumes; one transport reconnect recovers known threads; an ambiguous launch fails closed without duplication; capacity evidence revises the graph only before Goal freeze; and repair uses a separate execution-profile thread without rewriting scope/packets.
  - Real-Git scenarios retain the prior independent-wave merge, overlapping-root serialization/new-base, real merge-conflict repair and post-merge regression cases. Every scenario exercises thread/turn/Goal identities plus worktree, branch, commit, Slice receipt, merge, integration and same-snapshot Campaign final result. The focused App Server E2E targets 2 minutes and the entire Composite profile targets 5 minutes.
