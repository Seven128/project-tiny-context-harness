...
- `tests/ty-context/long-task-design-context.test.mjs`: controlling objective, trusted-result boundary, Plan Item meaning, Contract Draft/Draft Outcome lifecycle, Outcome decomposition/non-scheduler semantics, integrated authoring rationale, registered Context and mechanism-to-risk consistency.
...
- Design consistency: ordinary Source and optional Source Plan remain valid; Contract Draft belongs to `long-task-workflow`; Draft Outcome is authoring-time only; Outcome decomposition covers rolling work, targeted verification, failure localization and recovery without Worker/scheduler or completion authority; only current-snapshot proof accepts; external pending stays qualified; no `draft_outcomes`, `plan_items`, Draft Receipt/State/Authority or standalone authoring Skill; the on-demand rationale is registered; managed/generated/package Skills remain byte-identical and platform-neutral.
