...
Pull-request Package CI builds once and runs the complete default suite plus the Trust Boundary Gate through package `test:trust:built`. `main`, publish and release retain `npm test`. Every package-suite runner emits ephemeral `test-suite-timing-v1` diagnostics; timing output is never Context, workflow state or acceptance authority.
...
The implementation, migration status, measurement rules and stable retrieval keys are indexed in `docs/test-suite-roi-redesign.md` under `TS-PURPOSE`, `TS-BASELINE`, `TS-TIER-DEV`, `TS-TIER-TRUST`, `TS-TIER-RELEASE`, `TS-ROUTING`, `TS-RERUN`, `TS-OPTIMIZE`, `TS-MIGRATION`, `TS-METRICS`, `TS-AC` and `TS-NONGOALS`.
