...
- Long-task tests explicitly cover Global and Outcome Claim Coverage, Source/Context/Product/Global semantic Authority Revision identity, compile-time field-policy completeness, `input_paths`/`expected_output_paths` reductions, shared-AST fail-closed pattern containment/overlap, Bundle normalization, immutable baseline, risk downgrade, explicit-Observation Assertion safety and Schema/Parser parity, protected symlink/hardlink inputs, scoped progress, forged/missing cache resistance, Active Authority V3 plus V2 migration/CAS, Counterfactual/Population/minimal-Environment V2, runner freeze, Live Final Gate and relocated-package/same-group user Hook preservation.
