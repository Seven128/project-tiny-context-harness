...
- `node --test --test-concurrency=1 tests/ty-context/sync-init-doctor.test.mjs tests/ty-context/visual-delivery-guidance.test.mjs tests/ty-context/affected-test-selection.test.mjs`: Design Authority scaffold/Doctor behavior, visual coverage/source/production-render guidance, Long-Task visual authoring/evidence boundaries, managed/generated/package parity and guidance-aware focused-test selection.
...
- Design Authority and visual delivery guidance: the default Workflow, managed AGENTS, `context_uiux_design` and `context_development_engineer` distinguish material production UI from local fixes/prototypes; treat an unconfigured starter, style-only rule or inspiration as insufficient production authority; classify exact targets/constraints/inspiration; preserve one authored token source/generation direction; bind production components/real routes; reject implementation screenshots promoted as targets; and report only checked combinations. Doctor reports the starter as advisory information. Long-Task authoring resolves missing authority before Compile and reuses existing Requirement/Control/Assertion, `ui_browser`, verification-input and external-confirmation mechanisms without adding Schema, lifecycle, risk or acceptance authority.
