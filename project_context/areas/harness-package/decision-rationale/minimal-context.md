...
- Use explicit long-task Skills because Context priority and drift checks are intentionally soft constraints. Short tasks should usually use the workflow contract plus Context directly. Ordinary long tasks can externalize one acceptance target through `/normal-long-task`. Composite work uses a stricter but lightweight mechanism because implementation drift grows with context length, handoffs, parallel branches and verification loops: `/prepare-composite-long-task` preserves one discussed plan as a complete stable Campaign V4 graph, centrally authors dependency-ready Contract V3 packets, conservatively schedules isolated worktree Goals, integrates and repairs them, and recomputes the whole campaign on one final snapshot. `/composite-long-task-workflow` remains the one-Slice worker that freezes one packet, registered Context, oracle/verifier identities and the full Requirement/PI/Obligation/Binding/AC/Proof/Spec/Counterfactual graph in ordinary per-worktree state. The workflow does not try to prevent the model from ever drifting and does not prescribe TDD or review mechanics; it allows implementation freedom while preventing known Slice, merge and final-delivery drift from passing. Process cost is justified only when a step closes a named false-completion path. Generic state reducers, evidence registration, slice/epoch cadence, progress ledgers and alternate execution-method adapters remain removed.
...
- Allow one narrow tracked exception for explicitly invoked Composite Campaign V4: sanitized discussed-plan provenance, complete source coverage, one stable graph, campaign coordination/audit, immutable structured-authoring and schedule revisions, Goal/branch/commit identities, receipt hashes, integration results and the current verifier-derived Campaign result are user-owned orchestration/provenance, not default lifecycle state. Mutable compiled contracts, Goal attempts, raw logs, Oracle payloads and active verifier workdirs remain under temporary execution storage. Campaign completion is derived from current Slice/Wave/final verification and target integration; it is never Goal-written state or a summary of historical Slice results.
