...
Live research showed that the thin package-managed `design-resource-authoring` commissioner closes a different, workflow-independent gap: it can interpret raw product/technical/design Source, enforce the user's explicit output or development-scope ceiling, subtract sufficient selected-source coverage from material in-scope UI/UX meaning through relevant controls, discover current Open Design capabilities, select only independently justified resources, preserve provenance and return an intent-sized handoff. It therefore improves upstream navigation without being defined by Long-Task, owning provider logic or adding a dependency. One artifact may cover many controls/conditions when inspectable; no prototype, Figma output, one-file-per-control rule or other artifact is universal. Raw drafts and optional Source Plans are independent inputs, and neither authoring Skill invokes the other.
