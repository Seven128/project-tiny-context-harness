...
For selected design resources, this includes one explicit design purpose: Agent implementation, acceptance and testing fully conform to every material UI/UX fact expressed within the resources' declared scope and conditions. Open Design remains the default generation engine. When native Figma inspection is operational and useful, exact versioned nodes carry high-density addressable layout/component/token/annotation/motion/asset facts into a repository-readable immutable capture. A provider-neutral residual handoff references those facts and supplies the scope, conditions, omissions, product semantics, blockers and downstream bindings that Figma does not reliably own. Long-Task preserves both native and residual meaning as Source/Control/Claim authority and independently falsifiable production Checks through Final Gate; a fact not specified or demonstrated is refined, decided or blocking, never invented.
...
The remaining independent gap is semantic loss between provider output and downstream implementation/proof. Direct visual inspection is not a reliable authoring or acceptance interface: an Agent may overlook or reinterpret layout, component anatomy, states, transitions, motion, breakpoints/input and accessibility, while free prose or a generic Contract condition may omit a category without detection. Resource integrity cannot reveal that loss. Conversely, manually retyping every numeric/style fact from a native design tool increases authoring cost and creates another drift surface.

The admitted mechanism is therefore a two-part adapter rather than “Figma replaces handoff”:

1. an optional Figma-native producer profile pins exact file version, node IDs and conditions; acquires bounded logical nodes through metadata, design context, variables/styles and screenshot plus applicable motion/assets/Code Connect; prefers Components/Variants, Variables, Auto Layout, semantic names and Annotations/dev resources; and freezes repository-readable immutable resources while keeping the editable upstream route;
2. one strict provider-neutral `design-resource-handoff-v1` residual index remains embedded in ordinary marked Markdown Source after final selection. It references instead of duplicating addressable native facts, expands grouped stable subjects across eight closed UI/UX dimensions, distinguishes missing from not-applicable, preserves non-native product/data/permission/recovery semantics, and binds declared targets/conditions/evidence to Source Items, verification methods and acceptance blockers.

This split is causal. Native Figma structure reduces lossy visual inference and transcription cost; small-node acquisition avoids incomplete large-frame context; immutable local capture removes live permission/network/rate-limit/mutable-latest state from recovery and Authority identity; residual coverage detects what Figma did not model; Context-reachable selection supplies durable meaning; Source Claims and applicable Controls/`surface_bindings` preserve that meaning into production owners; and only current target `design_conformance`, interaction and target-runtime evidence may satisfy Final Gate. Connector/extraction success, a screenshot, a resource digest or handoff preflight proves input facts only.

The hybrid path has materially positive ROI when Figma is already operational or its native inspectability/editability is valuable: it removes repeated manual transcription, improves precision for addressable facts and keeps the prior omission protection. It remains optional so other providers and exploration pay no Figma setup/runtime cost. Prompt-only wording, link-only handoff and flattened exports were rejected because they cannot discover omissions or provide immutable recovery; making text duplicate all native facts was rejected for drift cost; a Figma-specific Resource Pack/schema/registry or new UI Gate was rejected because it duplicates authority and lifecycle. External resources still enter as ordinary Source. Existing Source markers, Controls, Requirements, verification inputs, Bindings, Artifacts, Assertions, Checks, protected revision and Final Gate remain the only Long-Task authority/proof path. The profile adds no lifecycle state, provider registry, Design Authority or acceptance claim.
