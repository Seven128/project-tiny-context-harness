...
For selected design resources, this includes one explicit design purpose: Agent implementation, acceptance and testing fully conform to every material UI/UX fact expressed within the resources' declared scope and conditions. Open Design HTML, images and prototypes remain the visual Source, while `design-resource-authoring` adds a structured textual semantic handoff as part of the selected implementation resource set. Long-Task preserves that meaning as Source/Control/Claim authority and independently falsifiable project Checks through Final Gate; a fact not specified or demonstrated is refined, decided or blocking, never invented.
...
The remaining independent gap is semantic loss between provider output and those downstream mechanisms. Open Design HTML, images and prototypes are rich visual resources, but direct visual inspection is not a reliable authoring or acceptance interface: an Agent may overlook or reinterpret layout, component anatomy, states, transitions, motion, breakpoints/input and accessibility, while a prose handoff or generic Contract condition string may omit the category without detection. The default Workflow had the same omission path before its prompt-level UI Authority Closure. Resource integrity cannot reveal this loss.

The admitted mechanism is one strict `design-resource-handoff-v1` index embedded in ordinary marked Markdown Source only after final selection for implementation. This structured textual supplement is part of the selected implementation design-resource set rather than a summary outside it. Grouped stable subjects expand across eight closed UI/UX dimensions; covered rows bind declared target/conditions, immutable addressable evidence, Source Items and project verification methods; unresolved or unsupported rows fail shared preflight. Static-frame, hash and visual-render substitution are rejected by evidence compatibility. The default Workflow and Long-Task consume the same validated meaning, while only Long-Task adds freezing, claim/assertion/blocker binding and Final Gate lifecycle.

This has materially positive ROI: it closes a distinct omission path for every design-bearing delivery, costs one compact index per handoff rather than per-Control artifacts, and makes no exploration request pay. Prompt-only wording was rejected because it cannot discover omissions; a provider-specific Resource Pack/registry or new UI Gate was rejected because it duplicates authority and lifecycle. External resources still enter as ordinary Source. Existing Source markers, Controls, Requirements, verification inputs, Bindings, Artifacts, Assertions, Checks, protected revision and Final Gate remain the only Long-Task authority/proof path. The adapter adds no lifecycle state, provider registry, Design Authority or acceptance claim.
