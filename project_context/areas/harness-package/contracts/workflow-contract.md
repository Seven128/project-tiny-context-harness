...
- Goal and acceptance wording stays disambiguated: `audit_task_complete` means only the workflow or audit finished, `acceptance_target_status` is the final verdict status, and `product_goal_complete` is computed by the final gate only. Agents must not hand-set `product_goal_complete`. In Codex Goal mode, implementation / execution goals complete only when the computed state has `product_goal_complete=true`; a non-accepted `acceptance_target_status` continues work or reports blockers / missing evidence. A deliberately read-only audit / reporting goal may end at `audit_task_complete`, but non-accepted verdicts must say `Audit workflow completed; acceptance target not complete.` and must not use unqualified `Goal achieved` or `update_goal(status="complete")` as product acceptance.
