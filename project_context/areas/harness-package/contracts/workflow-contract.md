...
- V2 sources/Observation V1, V1/Markdown campaigns, Scope Fit V2 and slice-named campaign fields/options, partial bundles and old workdirs have no importer or compatibility path. Preparation V3 exposes only stable SFC terminology (`sfcs`, `sfc_id`, `selected_sfc_id`, `--sfc`). `init`, `sync` and `upgrade` install/refresh capability only and never create or scan campaigns.
...
- Strict completion requires system `requirements.toml` with managed hooks pinned on and `allow_managed_hooks_only = true`, an admin-installed managed Hook/helper, a fresh real Hook heartbeat and a valid Host registry. Repo/user/plugin Hook fallback is forbidden; ordinary no-active tasks are no-op. The privileged Host may return validated workdir outputs and the Host-created repo mirror to the authenticated peer identity, but Host Managed State remains privileged; Windows sandbox ACL grants are limited to exact declared trees, restore captured ACLs and do not modify volume/system ancestors.
...
- Harness self-development requires a separately versioned/integrity-pinned external audit runner over the exact candidate tarball. Security regression cases must construct real temporary Git repositories and execute the CLI/Managed Hook; source regex, case-name and file-existence checks cannot be false-accept proof. Candidate-controlled commands use a disposable non-administrator OS identity and least environment, cannot mutate the installed audit package, pre-signed Host release, signing keys or result directory, and must leave no surviving process for that identity between evidence phases.
...
- Workdir runtime remains limited to the three YAML sources, signed compiled mirror, thin goal pointer, user-readable Host-written current/final views and verifier-owned runs. Atomic workdir views are not authority: deletion, replacement or mutation is detected against the Host registry/commit and cannot authorize completion. Host registry/bundles/layers/keys never enter the repo or Context. Context Delta, source coverage, implementation binding and Conformance remain Tiny Context workflow behavior, not duplicate runtime gates.
