...
- If authority is incomplete and the explicit task is standalone design-resource generation, use `design-resource-authoring` to select and commission the smallest sufficient set from currently available Open Design capabilities, then use `context_uiux_design` only in a separately authorized downstream workflow to adopt or repair durable authority. Otherwise stop for the material preference/decision that would change the result. Do not promote the implementation's own generated screenshot into its target.
...
- Dedicated Open Design/Product Design, Figma, image-generation, prototype or human workflows remain the standalone generation engines. `design-resource-authoring` is a thin optional commissioner over live Open Design capabilities; it accepts raw drafts and optional Source Plans independently, copies no provider prompt/template, mandates no resource and mutates no proposal/Context/`DESIGN.md`/code/Contract. Its outputs enter this workflow as ordinary external Source in their project-native formats.
...
- Preserve stable surface/state/control/target keys and semantic Control/state meaning from the controlling brief or Source when available. Images or Figma frames alone do not silently supply missing product, interaction, accessibility or recovery semantics.
