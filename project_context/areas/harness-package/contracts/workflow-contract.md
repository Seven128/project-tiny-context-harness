...
- Final gate order is fixed: executor self-evidence, matrix update, verdict update, `validate-plan-acceptance`, read-only auditor when available, rerun `validate-plan-acceptance` if auditor-driven fixes change artifacts, then completion claim only if no blocking conflict remains.
