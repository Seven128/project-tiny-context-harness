...
- If authority is incomplete and the explicit task is standalone design-resource generation, use `design-resource-authoring` to keep the requested output/development content as the hard ceiling, subtract sufficient selected-source coverage and commission the smallest sufficient resources for the remaining material in-scope UI/UX meaning. For an implementation handoff this meaning reaches relevant controls and conditions without a one-artifact-per-control rule; use `context_uiux_design` only in a separately authorized downstream workflow to adopt or repair durable authority. Otherwise stop for the material preference/decision that would change the result. Do not promote the implementation's own generated screenshot into its target.
...
- Dedicated Open Design/Product Design, Figma, image-generation, prototype or human workflows remain the standalone generation engines. `design-resource-authoring` is a thin optional commissioner over live Open Design capabilities; it accepts raw drafts and optional Source Plans independently, fixes the explicit output/development scope, derives material UI/UX gaps from selected existing coverage and may combine page/flow targets, component-family specifications, dedicated complex-control studies, interaction/motion resources and necessary assets without mandating any type or one resource per control. It copies no provider prompt/template and mutates no proposal/Context/`DESIGN.md`/code/Contract. Its outputs enter this workflow as ordinary external Source in their project-native formats.
...
`request/source -> optional Source Plan and external design Source -> relevant Context -> one continuously revised Contract Draft -> target profile / execution targets / ordered stages -> vertical Draft Outcome decomposition -> Given/When Assertions with all-of evidence capabilities -> repository/Context binding -> Source/REQ/CTRL/OBL/NCOMP/AC coverage -> read-only Preflight repair loop with verifier-adequacy checks -> formal Compile / Authority Lock -> one-time execution-model choice -> stage-constrained native Goal Rolling Frontier -> targeted repair verification -> protected Authority Revision when needed -> adopted revision returns to Rolling Frontier -> clean candidate commit -> risk-proportional read-only conformance Check inside the source-recompiled one-snapshot Final Gate -> target-qualified machine result -> Stop/close qualification-preserving output -> native Goal conformance veto -> external Git/CI/deployment/human confirmation -> Context drift check`

The optional Source Plan and any external design resources remain ordinary Source, not a Contract Draft. Tiny Context does not prescribe an upstream design-generation pipeline. The Contract Draft remains the one Contract rather than an artifact chain, the execution-model choice is a single user pause rather than model routing, and the Rolling Frontier remains temporary Goal working state rather than a scheduler. The lifecycle still has exactly one Contract and one Final Gate.
...
- A research proposal, ordinary prose plan, optional Source Plan or external design resource remains ordinary Source input after marker-only enumeration and does not need to match a recommended authoring structure. These are Source, not a Contract Draft. Marker keys and `source_claim` keys are set-equal and globally unique; `statement` preserves marked text after limited whitespace normalization.
- For interactive work, carry every applicable Source Plan Control field through the existing Control projection rather than collapsing it into a broad Requirement or dropping it. Every non-empty field is an independently Source-backed Control Claim and protected product semantic; omitted fields create no claim. Stable control keys align Source, Screen Context, implementation bindings and Assertions.
...
- Missing recommended Source Plan headings or labels does not block authoring; missing mandatory Material Item markers does. A request to synthesize, refine, complete or use judgment delegates plan-level authoring, but it does not supply missing tradeoff priorities. Before comparative research or a material product, technical or provider selection, identify the decision criteria that could change the research scope, candidate set or recommendation. Infer them only from the user's words, supplied Source/Context or controlling constraints. If a material priority such as quality versus cost, delivery speed, reliability, privacy, lock-in or operational burden is unknown or ambiguous, stop before that research or selection and ask a concise targeted question. Do not impose a fixed questionnaire or interrupt minor, reversible choices whose recommendation would not change.
...
- Pending approval summaries enumerate changed semantic fields, Source/Product Claim reductions, proof reductions and external-confirmation keys so exact approval is informed without copying the whole Contract. No separate revision plan, review artifact or approval registry is created.
