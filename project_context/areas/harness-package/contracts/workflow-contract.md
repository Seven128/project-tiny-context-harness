...
- Final gate order is fixed for strict V2: load three inputs, recompute source hashes, load `task-state.json`, load current attempt, load command-run records, load registered EvidenceRecordV2 records, discard stale evidence, scan contradictions, recompute every AC, recompute every PI, recompute `acceptance_target_status`, recompute `product_goal_complete`, resolve `completion_output_status`, regenerate `derived/**`, then append the final-gate event. The Trusted Evidence Kernel owns proof recomputation; the completion-output resolver owns generated final-answer status. Validators and generated views may report kernel and resolver findings but cannot become proof or completion authority. Final gate recomputes from sources, current attempt, required command specs/runs, registered EvidenceRecordV2 records, under-specified AC checks, AC-010 bootstrap checks, Harness Drift Lock, protected baseline guard and current contradictions; it must ignore historical `events.ndjson` completion events and old `derived/**` completion text as proof. `product_goal_complete=true` may be computed only when required plan items, required ACs, required proof layers, machine assertions, command runs, negative scans, source drift, worktree drift, scope conflicts, stale evidence, under-specified checks, protected baseline checks, Harness Drift Lock, AC-010 bootstrap prevention, overclaim, sibling substitution, auditor blockers, derived-state consistency and newer same-AC/layer contradictions all pass. `completion_output_status=accept` additionally requires the current final-gate accepted result, successful required validators and no persistent generated-output mismatch; otherwise official generated output is `reject` or `blocked`. Any failure must report `partial`, `blocked`, `under_specified` or `invalidated` with concrete AC / PI / layer / evidence gaps.
