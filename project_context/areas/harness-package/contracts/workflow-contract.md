...
- Legacy Source Plan headings never block authoring; missing Material Item markers does. The Source-bound Draft loop preserves the existing preference/research/delegation gate and asks only when an unknown preference would change research or recommendation.
...
- Check `input_paths` and Binding carriers use the smallest sound causal envelope; broad repository/application/platform roots require a real any-matching-change invalidation reason. Counterfactual carriers must have a defensible route from the declared target root, while current-execution Counterfactual behavior—not a language-neutral path-name lint—proves reachability when repository evidence cannot. Project runners own runtime-specific readiness probes, build caching/progress and process cleanup.
