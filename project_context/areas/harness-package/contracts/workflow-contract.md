...
- Final gate order is fixed for strict V2: `derive`, `superpowers:verification-before-completion expected`, `validate_state`, `validate_derived`, `validate_plan_acceptance`, `auditor_blocker_scan`, `stale_overclaim_scope_scan`, `ac_evidence_assertion_gate`, `negative_evidence_scan_gate`, then `compute_completion`. `product_goal_complete=true` may be computed only when required plan items, required ACs, required proof layers, machine assertions, negative scans, source drift, scope conflicts, stale evidence, overclaim, sibling substitution, auditor blockers and derived-state consistency all pass. Any failure must report `partial`, `blocked` or `invalidated` with concrete AC / PI / layer / evidence gaps.
