...
- If authority is incomplete and the explicit task is to generate design resources, use an available dedicated Product Design capability outside Tiny Context to author candidate or selected upstream Source, then use `context_uiux_design` in this downstream workflow to adopt or repair durable authority. Otherwise stop for the material preference/decision that would change the result. Do not promote the implementation's own generated screenshot into its target.
...
- Dedicated Product Design, Figma, image-generation, prototype or human design workflows own standalone resource generation outside Tiny Context. Their outputs enter this workflow as ordinary external Source in their project-native formats.
