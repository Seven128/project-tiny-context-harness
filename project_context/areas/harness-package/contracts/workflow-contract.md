...
- Final gate order is fixed: derive all, run `superpowers:verification-before-completion`, validate `task-state.json`, validate derived artifacts, run read-only auditor when available, apply auditor fixes and rederive/revalidate when findings change state or evidence, run final stale/overclaim scan, run the AC Evidence Assertion Gate and Negative Evidence Scan Gate, then compute completion only if no blocking conflict remains.
