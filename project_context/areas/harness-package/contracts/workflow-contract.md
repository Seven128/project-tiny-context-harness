...
`request/source -> optional Source Plan -> relevant Context -> one continuously revised Contract Draft -> Draft Outcome decomposition -> repository/Context binding -> Source/REQ/CTRL/OBL/NCOMP/AC coverage -> read-only Preflight repair loop -> formal Compile / Authority Lock -> native Goal Rolling Frontier -> targeted repair verification -> clean candidate commit -> source-recompiled one-snapshot Final Gate -> qualified machine result -> Stop/close qualification-preserving output -> external Git/CI/deployment/human confirmation -> Context drift check`

The optional Source Plan remains ordinary Source, the Contract Draft remains the one Contract rather than an artifact chain, and the Rolling Frontier remains temporary Goal working state rather than a scheduler. The lifecycle still has exactly one Contract and one Final Gate.
...
- Compiler-generated Outcome/Check/Claim ids replace handwritten cross-entity references. Source AC references use stable `<outcome>.<check>.<assertion>` ids for Outcome Assertions and `GLOBAL.<check>.<assertion>` for Global Assertions. A Global Source AC is criterion-identical, proves only Global Claims and includes at least one Claim independently backed by another Source non-goal, technical-obligation or forbidden-shortcut item. Contract edits are normal Git history; there is no Packet chain or second plan.
...
- Global structured Claims use same-Global-Check Counterfactuals whose `binding_ref` resolves to an Outcome Binding; no Global Binding layer is introduced. `existing` mutation targets exist at Preflight/Compile, while `planned` targets may appear later but must exist at Final Gate and invalidate scoped Progress when changed.
- Standard frozen Playwright verifier content is trusted without mutation. A weak-observability Outcome requires same-Check sensitivity for every claim-bearing AC and related Claim. Its Counterfactual may accept Playwright exit one only when every unexpected Test Instance is uniquely the designated executed AC set and root errors, unbound/extra failures, missing/skipped/flaky/timed-out/interrupted cases and other Evidence failures are absent; only then do designated unexpected Findings normalize to exact mismatches. Structured Counterfactuals and ordinary Playwright Baselines still require exit zero.
...
- Targeted verify may run one Check, one Outcome or all requested repair checks. Counterfactual Findings are first projected into their owning Check Result, change a passed Main Check to `invalid_evidence`, clear Claim Proofs and then enter the existing Check Progress Record; status/resume therefore retain the failure, including for Global Checks without a new Global Outcome state. It rechecks active task/revision/compiled/worktree identity before writing derived current-snapshot status and is never accepted authority.
...
- `final-gate`, `status`, `resume`, `stop-check`, the Stop Hook and `close` preserve the same accepted workflow qualification. Status/resume expose `final_workflow_status` only for a valid fresh Final Receipt and expose Contract-declared confirmations independently of that historical proof. Stop/close may CAS-clear an accepted machine Authority; pending external delivery remains named in their output, `closed` means only machine Authority cleanup, and no confirmation tracker or Receipt is created.
