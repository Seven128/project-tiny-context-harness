...
- Open Design owns generation. `/design-system-authoring` is explicit-only, feature-detects MCP lifecycle support, uses the official same-daemon fallback when needed and adopts one selected system into canonical project authority without a provider registry. `/design-resource-authoring` gates only style-bearing resources, verifies `designSystemId`, commissions the smallest scoped set and reconciles accepted final decisions into the initial proposal once. For a selected implementation handoff it emits one strict marked `design-resource-handoff-v1` Source and runs the package-owned shared preflight; exploration stays schema-free. It never edits a Source Plan, Context/`DESIGN.md`, code or Contract and adds no provider dependency/authority.
