...
- Dedicated Open Design/Product Design, Figma, image-generation, prototype or human workflows own the actual generation engines. `/design-resource-authoring` is a business-agnostic base Skill that discovers current capabilities, chooses the smallest sufficient task-local resource set, commissions Open Design, iterates candidates and prepares optional handoff metadata without copying provider prompts/templates or adding a provider dependency. It requires no prototype/Figma/fixed sequence, invokes no Source Plan authoring, and never mutates the input proposal, Context, `DESIGN.md`, production code or Delivery Contract. `context_uiux_design` separately consumes selected Source during development to adopt or repair durable authority.
