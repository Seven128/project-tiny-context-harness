...
- `.codex/skills/**` exposes generated/default Skills in this source workspace. The base managed set includes `.codex/skills/design-resource-authoring/**`; the `long-task` profile additionally owns `.codex/skills/source-plan-authoring/**` and `.codex/skills/long-task-workflow/**`; `normal-long-task` is a retirement pointer. Package-managed copies are overwritten by sync.
...
- Dedicated Open Design/Product Design, Figma, image-generation, prototype or human workflows own the actual generation engines. `/design-resource-authoring` is a business-agnostic base Skill that makes the explicitly requested output or development content its hard ceiling; identifies material user-visible UI/UX needs from surface/flow structure through relevant control anatomy/variants, static/dynamic states, interaction/feedback/motion, responsive/platform/input behavior, accessibility and assets; subtracts sufficient selected-source coverage; then commissions the smallest remaining task-local resource set from current capabilities. It may map repeated controls to one selected component family or let one inspectable artifact cover several needs, but never infers unshown conditions or requires one artifact per control. It copies no provider prompts/templates, adds no provider dependency, invokes no Source Plan authoring, and never mutates the input proposal, Context, `DESIGN.md`, production code or Delivery Contract. User-visible interaction semantics may be represented in designs, while business/data/permission/algorithmic rules remain owned by product/technical Source. `context_uiux_design` separately consumes selected Source during development to adopt or repair durable authority.
