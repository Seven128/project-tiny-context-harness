...
- Package-managed visual-delivery guidance reuses the existing authority split and verification surfaces. `context_uiux_design` keeps durable surface/interaction facts in `project_context/**`, durable visual-system semantics and rationale in `DESIGN.md`, and a task-local risk-proportional Visual Coverage Set rather than a required matrix or artifact. `context_development_engineer` binds that intent to one declared authored token source/generation direction, production components/routes and project-owned rendered verification.
- Long-Task visual guidance is authoring and evidence specialization only: declared visual expectations use existing Requirement, Control, Assertion, `ui_browser`, verification-input and `external_confirmation` mechanisms. It adds no visual Schema, Claim kind, risk level, lifecycle state, baseline authority or Final Gate, and it cannot prove omitted coverage or subjective visual quality.
- `/source-plan-authoring` explicitly authors or audits one high-fidelity Markdown Source Plan from a near-complete draft or a sparse goal plus mixed attachments. It inventories every supplied artifact, preserves direct requirements, marks necessary derivations and explicitly delegated low-impact/reversible elaboration, expands interactive work through surface/region/control/state/feedback detail, retains independent CTRL fields, NCOMP meanings, one-scenario AC links and exact Risk Fact/Outcome/Basis/Consequence, and leaves unsupported high-impact semantics unresolved. It creates no Context update, Contract Draft/YAML, repository binding, workflow state, implementation or completion claim; its format remains optional Source guidance.
...
- CLI JSON, the package-owned Stop Hook, the managed Long-Task Skill and English/Chinese public docs preserve qualified completion consistently: pending external confirmation remains named through status/resume/stop-check/`systemMessage`/close, while `closed` denotes only machine Authority cleanup. Generated and packaged copies must not erase that distinction or add external-confirmation state.
