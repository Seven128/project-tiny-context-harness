...
- `/source-plan-authoring` is a retired compatibility pointer and creates no standalone plan. `/long-task-workflow` accepts revised proposals, selected resources, ordinary prose or legacy Source Plans; its `source-authoring.md` reference owns mixed-input inventory/synthesis/refinement, stable/control-level semantics and acceptance/risk completeness before the existing Contract Draft, marking, Preflight, Compile, execution and Final Gate.
