...
- Open Design owns default generation. `/design-system-authoring` is explicit-only, feature-detects MCP lifecycle support, uses the official same-daemon fallback when needed and adopts one selected system into canonical project authority without a provider registry. `/design-resource-authoring` gates only style-bearing resources, verifies `designSystemId`, commissions the smallest scoped set and reconciles accepted final decisions into the initial proposal once. Its optional `references/figma-native-handoff.md` profile feature-detects an operational Figma connector/auth/read-or-export path, pins exact versioned nodes, retrieves bounded native fact slices and freezes repository-readable immutable inputs plus an editable upstream route. For a selected implementation handoff it emits one strict marked provider-neutral residual `design-resource-handoff-v1` Source and runs the package-owned shared preflight; exploration stays schema-free. It never edits a Source Plan, Context/`DESIGN.md`, code or Contract and adds no live provider dependency/authority.
- `packages/ty-context/src/lib/design-resource-handoff-*.ts` plus `commands/design-resource.ts` own the provider-neutral embedded-block parser, semantic/resource validator and CLI. Long-Task activation calls the same validator and adds only existing Contract binding checks. These are package code, not managed generated assets, a provider runtime, registry, Gate or acceptance surface.
...
- External design briefs/indexes may enter `source_paths`; selected acceptance-affecting targets, token sources, exact-version/node Figma-native captures and fixed prototype fixtures enter the conformance Check's `verification_inputs`; production carriers use `input_paths`/Bindings; generated implementation renders/diffs/reports remain `artifact_globs`. The residual handoff references native facts while closing missing semantics and bindings. `design_resource_integrity` cannot satisfy `design_implementation_conformance`; `visual_render` cannot substitute for the selected-target comparison. Connector, extraction, export or authoring validation is not package, product or Long-Task acceptance.
