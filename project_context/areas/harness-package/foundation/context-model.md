...
- Source-quality authoring is a progressive reference inside `long-task-workflow`, not a separate Context/artifact lifecycle. It may refine one writable proposal or materialize one conversation-only Markdown Source, but creates no Source Plan schema, gate, state, Authority or proof. Legacy Source Plans remain ordinary Source.
- Explicit `design-system-authoring` may adopt a selected Open Design system into canonical `DESIGN.md`, one token source/direction and owning Context. Externally authored resource candidates remain ordinary Source. `design-resource-authoring` gates style-bearing work, binds provider identity, commissions/iterates task-locally and may reconcile accepted final decisions into the initial proposal once; it never mutates Source Plans, Context/`DESIGN.md`, production UI or Contract state. These flows add no Context role, pack, registry or acceptance authority.
