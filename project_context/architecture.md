...
- `ty-context composite-long-task ...` commands are explicit temporary-execution helpers for `/composite-long-task-workflow`: they initialize a workdir, compile the three input sources into `task-state.json`, start attempts, capture assertion command runs, register current-attempt EvidenceRecordV2 records, apply slice deltas, derive views, run slice/epoch/final gates, suggest next slices and render the thin Codex Goal objective plus frozen workflow protocol and execution binding. Final completion is centralized in the Trusted Evidence Kernel rather than in generated views, events or validators: final gate recomputes source hashes, current attempt, command specs/runs, registered evidence, contradictions, under-specified ACs, AC-010 bootstrap, Harness Drift Lock and protected baseline state before it can compute `product_goal_complete=true`. The old `ty-context superpowers ...` namespace may remain only as a hidden/internal compatibility alias. These commands do not run during `init` or `upgrade`, do not register Context graph nodes and do not make Harness a general workflow engine.
