...
- External design-resource boundary: dedicated Open Design/Product Design, Figma, image-generation, prototype or human workflows remain the generation engines. The base managed Skill `.codex/ty-context-managed/skills/design-resource-authoring/**` is only a thin capability-discovery, scope-bounded coverage, commission, iteration and handoff layer. Its `SKILL.md` owns the hard explicit development-scope ceiling and core workflow; `references/resource-selection.md` owns material UI/UX coverage through relevant controls, existing-source subtraction, conditional resource selection and stop rules; `references/downstream-handoff.md` owns scope/coverage/provenance transfer. It owns no provider prompt/template/runtime, business-rule authority, mandatory artifact, one-file-per-control rule, pack schema, validator, registry, fixed directory or plugin dependency. Tiny Context consumes selected external targets/constraints as ordinary Source through UI Authority Closure and existing default/Long-Task Source, Context, verification-input and evidence mechanisms.
