...
- Test feedback and runtime cost: `tools/affected_change_discovery.mjs`, `affected_test_selection.mjs` and `test_suite_policy.mjs` own fail-safe tier selection and explicit aggregate dominance; `tools/package_build_fingerprint.mjs` owns current-source `dist` freshness; `tests/ty-context/run-package-suite.mjs` plus its file reporter own exhaustive execution and ephemeral per-file timing; `long-task-delivery-fixtures.mjs` owns immutable suite-scoped seed preparation and per-fixture repository isolation. These surfaces never define Delivery acceptance, cache green results or remove complete release coverage.
...
- Test-cost optimization preserves one owner per concern: build freshness is derived from current build inputs, suite timing is disposable diagnostics, and fixture seeds are process-scoped infrastructure copied into independent repositories. None is workflow state, a scheduler, a result cache or reusable acceptance evidence.
...
- Test-routing tests must prove inferred local discovery ignores and reports only untracked `.work_products/**`, while tracked or explicit paths remain fail-safe inputs. One canonical plan selects only its highest aggregate, so complete Long-Task/release coverage supersedes a separate Trust invocation. Canonical Trust/focused/hotspot review budgets block silent structural growth, complete discovery remains exhaustive, and wide environment-specific ceilings run only in controlled Ubuntu CI as cost-regression guards rather than test-result authority.
- Test-runtime tests must prove stale `--no-build` fails before a suite starts, the fingerprint covers every declared build input, per-file diagnostics are exhaustive but non-authoritative, suite-seed copies have distinct Git common directories and no remotes, explicit isolation lanes are disjoint/exhaustive, and every unknown file remains serial.
