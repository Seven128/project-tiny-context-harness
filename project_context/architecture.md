...
- `ty-context composite-long-task ...` commands are explicit temporary-execution helpers for `/composite-long-task-workflow`: they initialize a workdir, compile the three input sources into `task-state.json`, apply slice deltas, derive views, run slice/epoch/final gates, suggest next slices and render the thin Codex Goal objective plus frozen workflow protocol and execution binding. The old `ty-context superpowers ...` namespace may remain only as a hidden/internal compatibility alias. These commands do not run during `init` or `upgrade`, do not register Context graph nodes and do not make Harness a general workflow engine.
