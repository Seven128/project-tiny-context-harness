...
- `validate-plan-contract` checks temporary `plan.md` workflow-contract surfaces for internal consistency, referenced path existence and declared Context-to-Implementation binding. `validate-plan-acceptance` checks long-task acceptance artifacts; when a Superpowers workdir contains `task-state.json`, it validates generated views against canonical state, and otherwise preserves legacy matrix/verdict validation. `validate-superpowers-state` validates the canonical Superpowers long-task state directly. These plan/state validators remain separate from `validate-harness` and do not prove product behavior.
- `ty-context superpowers ...` commands are explicit temporary-execution helpers for `/superpowers-long-task`: they initialize a workdir, compile the three input sources into `task-state.json`, apply slice deltas, derive views, run slice/epoch/final gates and suggest next slices. They do not run during `init` or `upgrade`, do not register Context graph nodes and do not make Harness a general workflow engine.
