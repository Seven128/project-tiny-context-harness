...
- `validate-code-modularity` enforces touched handwritten source line-count risk as a separate maintainability gate; `validate-harness` composes `validate-context` and `validate-code-modularity`. `validate-plan-contract` checks temporary workflow-contract plan surfaces for internal consistency, path-reference existence and declared Context-to-Implementation binding; `validate-plan-acceptance` checks long-task matrix/verdict artifacts for contradictory completion claims, dangling evidence references, weak-proof complete rows and declared surface/architecture binding gaps. Plan validators do not prove product quality and are not included in `validate-harness`.
