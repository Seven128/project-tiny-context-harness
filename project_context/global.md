...
- Complete selected-design conformance is a design-specific Long-Task objective: within declared scope and conditions, Agent implementation, acceptance and testing must carry every material UI/UX fact explicitly expressed by selected design resources through Contract authority and current-snapshot project checks. Open Design HTML/images/prototypes remain the visual resources; a structured textual handoff is part of the selected implementation resource set so semantic loss is detectable, while missing meaning is refined or blocks rather than being guessed.
...
- Explicit Open Design adapters separate `design-system-authoring` authority adoption from `design-resource-authoring` scoped commissioning. A selected implementation handoff emits one strict `design-resource-handoff-v1` Source for shared preflight; resources stay ordinary Source and development workflows own implementation/proof.
