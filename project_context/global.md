...
- Source-quality refinement/synthesis is integrated into `long-task-workflow`: it inventories raw/revised proposals, selected design resources and mixed attachments, preserves direct/derived/delegated/evidence-backed meaning, expands applicable controls/states and creates self-contained real Source before Contract mapping. `/source-plan-authoring` is only a compatibility pointer; legacy Source Plans remain valid ordinary Source. Unknown decision-changing preferences still cause one targeted clarification and high-risk actions remain external confirmations.
...
- `diagnose-revision` recompiles the same `delivery-contract.yaml` against the active Authority and may run only existing active Check identities with unchanged runner/verifier authority for a scope-only candidate. It writes no pending/approval state, Active Authority, cache, Progress or Receipt and never authorizes acceptance. Repeated edits accumulate only in the existing Contract Draft; ordinary `compile --revise` creates one hash-bound pending decision. Every pending projection derives a concise human decision brief from the canonical material summary, naming why approval is needed, material change categories, affected Outcomes and the no-completion/adoption effect before asking for the exact identity. The brief is not editable authority or state. Until approval and atomic adoption, the old Authority remains active; adoption invalidates derived evidence and the complete current-snapshot Final Gate remains mandatory.
