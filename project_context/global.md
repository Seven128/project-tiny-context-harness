...
- Every Long Task has real Source; each declared Source file contains a Material Item, while background-only references stay outside Source Authority. Items are wrapped in non-rendering, text-preserving markers. Marker keys and `source_claim` keys are set-equal; typed dispositions keep Requirement/Acceptance/Result/Risk/Non-goal/External/Decision meaning distinct, and `out_of_scope` is never resolved authority. Source Acceptance is criterion-identical to one named Assertion proving a non-Result Claim. The first successful formal Compile is the Authority Lock and freezes an immutable task base. Every later revision compares declared Source/Product/Technical/Acceptance/Risk/Context and verifier materials against active authority; deleting progress, Receipts or cache and restoring implementation code cannot reopen a pre-lock weakening window.
...
- One root V2 `delivery-contract.yaml` is authoring authority. Existing Outcome fragments are physical compatibility only. Compiled Source inventory, Coverage, findings and adapters are derived projections, never second editable authority or workflow state; there is no Packet revision chain, Source Unit inventory file, SFC graph, Wave, Delivery Set, top-level Contract split or second plan.
...
- Evidence adapters derive from runner kind: only Playwright may prove `ui_browser`; structured runners prove non-browser surfaces. Across all Checks sharing a Raw Execution, one Claim-bearing Observation belongs to one Assertion. Playwright Claim proof is only AC `passed equals true`; missing/skipped/flaky/unexpected and duplicate-per-project cases fail closed while distinct projects aggregate all-of. Counterfactual proof binds a declared implementation carrier and is valid only for designated value mismatches; weak evidence requires one bounded sensitivity control. Claim and Population proofs are emitted only for a fully passed Check.
...
- Compact and fully expanded V2 authoring normalize to the same Contract/authority/compiled identity. Machine-enumerated Source continuity, typed dispositions, atomic all-of Claims, runner-derived adapters, canonical multi-project Playwright AC proof, Binding-sensitive Counterfactuals and passed-Check-only Claim Proof are activation rules. Findings/explain project Source/Claim/AC/surface/Check/adapter/Observation/owner-path repair context.
