...
- Current best practice: short tasks use the workflow contract plus Context layer directly. Long-running ordinary acceptance preparation still uses `/normal-long-task` when useful. When multi-SFC composite work begins from a raw requirement, explicit `/prepare-composite-long-task` owns Scope Fit, stable SFC authoring and a narrow tracked authoring/provenance campaign; it authors only the dependency-ready SFC as `CompositeAuthoringPacketV2`, deterministically projects three strict YAML authorities and stops at `handoff_ready`. `/composite-long-task-workflow` is the strict downstream executor for an already-complete Product / Architecture Source, Technical Realization Plan and Acceptance Checklist. It compiles one immutable obligation/verification contract, allows model-chosen implementation methods, actively verifies exact frozen specs, reruns every spec on one final snapshot and uses a mandatory Codex Stop Hook to prevent unaccepted or stale completion replies.
...
- Tracked composite campaigns are an explicit source-authoring/provenance exception, never default lifecycle plan state. `init`, `sync` and `upgrade` do not create or scan campaigns; the V2 breaking workflow has no legacy importer, compatibility alias, dual runtime or aggregate campaign product-completion state.
...
- Composite preparation uses two deliberately separate planes: Git-trackable, user-owned campaign sources preserve request/authoring provenance, while compiled contracts, verifier runs and final results remain temporary execution data. This narrow source exception does not restore a lifecycle plan store.
- The three rendered YAML inputs remain separate authorities because product/architecture intent, atomic implementation obligations and acceptance/oracle semantics must not overwrite one another. The V2 structured authoring packet is their editable source before handoff; rendered inputs become frozen compiler inputs.
- No legacy campaign, Markdown input, partial-bundle, task-state or old-workdir importer is provided. Ambiguous historical authority and permanent compatibility paths weaken the no-false-completion boundary; users regenerate V2 inputs and verify a fresh snapshot.
...
- Long-running composite execution exists because Context and prompt discipline are soft constraints: they can guide implementation but cannot prevent a model from reporting completion after an unnoticed drift. The composite workflow therefore moves completion authority out of model prose. Its only default runtime costs are contract compilation, targeted implementation–verification loops, one final all-spec snapshot and Codex Stop enforcement; every step must close a named false-completion path or be removed.
- Composite long-task acceptance is verifier-owned. The compiler freezes requirement/obligation/AC/spec coverage, all registered Context, scope/population policy, exact commands, oracle identities and verifier identity. `verify` produces findings from actual command observations; the agent cannot choose commands or register evidence/status. `final-gate` reruns all specs on one isolated snapshot and is the only producer of `accepted`. Stop Hook validates the mirrored active binding, repeats full final verification, checks the real assistant message for external blockers and removes terminal binding state only after allowing exit.
...
- Composite input parsing is intentionally strict YAML rather than permissive Markdown. Stable IDs and exhaustive graph edges prevent references or prose from becoming accidental definitions. Duplicate/unknown fields, YAML aliases/merges/tags/multi-document input, manual-only or summary-only ACs, missing executable predicates and missing oracle provenance fail compilation.
- V2 replacement is deliberately non-transitional: no `ty-context superpowers`, hidden alias, old task-state reader, legacy Markdown compatibility, evidence importer, dual-write or old campaign/workdir migration. This keeps the normal path small and prevents compatibility code from weakening current acceptance semantics.
...
- `validate-code-modularity` enforces touched handwritten source line-count risk as a separate maintainability gate; `validate-harness` composes `validate-context` and `validate-code-modularity`. `validate-plan-contract` checks temporary workflow-contract plan surfaces. `validate-plan-acceptance` remains only for the ordinary long-task matrix/verdict surface. Composite V2 does not use a generic state validator; its compiler, verifier, final gate and Stop checker own contract integrity and acceptance.
- Composite V2 is the current source implementation: strict YAML authorities compile into an immutable graph, active verification runs frozen commands in isolated snapshots, final acceptance uses one all-spec snapshot, and Codex lifecycle Hooks enforce completion. The old Markdown/state/evidence/slice/derived runtime and aliases are absent.
