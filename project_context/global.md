...
- External Product Design, Open Design, Figma, image-generation, prototype or human design processes may supply authored design resources as ordinary Source. The base package includes an optional, explicitly triggered `design-resource-authoring` Skill that treats the requested output or development content as a hard scope ceiling, subtracts sufficient selected-source coverage from the material user-visible UI/UX decisions inside that scope, and commissions the smallest sufficient resource set from live Open Design without copying provider prompts/templates or owning its runtime. An implementation handoff reaches relevant surface/flow/region/component/control conditions without requiring one artifact per control or expanding into unrelated product areas; candidates authorize no fidelity, while the consuming default/Long-Task workflow owns UI Authority Closure, `Context Delta`, adoption, implementation and proof.
...
- Tiny Context has no required Open Design, Figma, Product Design, image-generation or prototype dependency and does not prescribe an artifact type, count, fixed sequence, one-file-per-control rule or design-resource directory. `design-resource-authoring` accepts raw drafts, product/technical/design Source or an optional Source Plan independently; neither it nor `source-plan-authoring` invokes the other. Page targets, component-family specifications, dedicated complex-control studies and interactive artifacts may each carry several declared conditions, but no static/default view silently covers unshown states, motion, responsiveness or accessibility. When external resources affect implementation or acceptance, the consuming workflow resolves selection, stable identity, declared condition coverage, Control meaning and verification binding from Source without inventing a second design lifecycle.
...
- An adopted Authority Revision is an authority-change event, never a delivery-completion event. When rolling implementation meets a blocker, implementation difficulty alone cannot reclassify machine-verifiable scope as external or remove it; any real scope/acceptance change is explicit marked Source plus exact revision approval. Adoption reports the material Source/Product/proof/external-confirmation changes, invalidates affected evidence and returns the current Goal to rolling implementation or repair under the new Authority.
