...
- External Product Design, Figma, image-generation, prototype or human design processes may supply authored design resources as ordinary Source. Tiny Context does not duplicate their generation workflows or require one package format. Candidates authorize no fidelity; selected exact targets and constraints control only declared conditions, while the consuming default/Long-Task workflow owns UI Authority Closure, `Context Delta`, adoption, implementation and proof.
...
- Tiny Context has no required Figma, Product Design, image-generation or prototype dependency and does not prescribe an artifact count or design-resource directory. When external resources affect implementation or acceptance, the consuming workflow resolves selection, stable identity, declared condition coverage, Control meaning and verification binding from Source without inventing a second design lifecycle.
