...
- Current product direction is Minimal Context Harness: preserve the smallest durable facts and priority workflow needed for fresh-agent recovery, iteration, debug and requirement changes.
- Design purpose: Minimal Context is a repo-native context contract, not only a memory folder. It combines compact durable facts with Context/code/evidence priority rules, read-order guidance, context-first classification and drift checks so agents keep project intent, implementation and verification evidence aligned.
