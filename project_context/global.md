...
- Original source requirements remain directly covered by Source Claims for strict and Bundle work. The first successful compile is the Authority Lock and freezes an immutable task base. Every later revision compares against active authority; deleting progress, Receipts or cache and restoring implementation code cannot reopen a pre-lock weakening window. Source hashes/file sets, selected Context topology/files/hashes, canonical Product/Global semantic projections and verifier content are revision materials.
...
- Compile is pure static preflight. It validates strict YAML/schema, Global and Outcome Claim Coverage, one canonical repository path/pattern grammar shared by parsing, hashing, matching, subset and overlap, Context/source/path/runner/proof identities, risk floor, UI proof, strict obligations and executable acceptance before product implementation begins. One leading `./` and Windows separators normalize to `/`; internal `.`/`..`, control characters, empty segments, absolute/drive/UNC paths and unsupported glob syntax are rejected.
