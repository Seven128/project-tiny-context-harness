...
- Current best practice: short tasks use the workflow contract plus Context layer directly. Long-running tasks should first externalize a plan through Web GPT or another external planning model when useful, then explicitly invoke `/normal-long-task` to produce the full checklist and optional generic target-mode prompt; when the user explicitly wants Superpowers execution, invoke `/superpowers-long-task` on the complete packet to produce the Superpowers-specific target prompt, then use the specific Superpowers plugin/workflow to derive concrete implementation slices and execute each slice through the workflow contract plus Context layer.
