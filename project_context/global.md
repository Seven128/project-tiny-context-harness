...
- External Product Design, Open Design, Figma, image-generation, prototype or human design processes may supply authored design resources as ordinary Source. The base package includes an optional, explicitly triggered `design-resource-authoring` Skill that discovers live Open Design capabilities, selects the smallest sufficient task-local resource set and commissions the external provider without copying its prompts/templates or owning its runtime. Candidates authorize no fidelity; selected exact targets and constraints control only declared conditions, while the consuming default/Long-Task workflow owns UI Authority Closure, `Context Delta`, adoption, implementation and proof.
...
- Tiny Context has no required Open Design, Figma, Product Design, image-generation or prototype dependency and does not prescribe an artifact type, count, fixed sequence or design-resource directory. `design-resource-authoring` accepts raw drafts, product/technical/design Source or an optional Source Plan independently; neither it nor `source-plan-authoring` invokes the other. When external resources affect implementation or acceptance, the consuming workflow resolves selection, stable identity, declared condition coverage, Control meaning and verification binding from Source without inventing a second design lifecycle.
