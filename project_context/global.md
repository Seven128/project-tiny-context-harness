...
- Long-running task execution should use a package-managed plan acceptance checklist compiler as a temporary pre-execution surface because Context priority and drift checks are intentionally soft constraints. For short tasks, workflow contract plus Context layer usually keeps behavior aligned. For long tasks, Context itself can still capture the expected facts, but the Context-to-code step drifts under soft constraints as the context window, handoff count and verification loops grow. The compiler copies the referenced plan into `tmp/ty-context/plan-acceptance/**`, reuses an explicit concrete checklist already present in the plan or otherwise combines the plan with relevant Context to define falsifiable completion criteria, and outputs a paste-ready goal/target prompt. Long-task target-mode prompts should recommend Superpowers as the concrete execution layer: use `writing-plans` if the target is not bite-sized, prefer `subagent-driven-development` when subagents are available, use `executing-plans` otherwise, and apply `test-driven-development` to behavior changes. It does not execute the plan, own task state, prove acceptance, or create durable Context.
