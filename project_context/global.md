...
- Complete selected-design conformance is a design-specific Long-Task objective: Agent implementation, acceptance and testing carry every in-scope expressed UI/UX fact through current-snapshot checks. Optional Figma-native exact-version/node capture improves precision; the provider-neutral structured handoff remains the residual semantic and coverage boundary for gaps and downstream bindings. Both are immutable Source: missing meaning blocks, and provider/preflight success never proves production conformance.
...
- Explicit Open Design adapters separate `design-system-authoring` adoption from `design-resource-authoring` commissioning. Optional Figma-native facts are frozen locally; selected implementation handoffs still emit one provider-neutral residual `design-resource-handoff-v1`. Development workflows own Context reachability, implementation and proof.
...
- Tiny Context has no required live design-provider dependency or fixed artifact sequence/count/directory. Figma-native input is optional and requires an operational path plus material benefit; mutable, flattened or metadata-only input is insufficient. Resources carry only declared conditions.
