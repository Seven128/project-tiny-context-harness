...
- Original source requirements and acceptance criteria remain directly covered by Source Claims mapped to first-class Requirement/control/obligation/global Claims, named Acceptance Assertions, external confirmations or explicit dispositions. Atomic Source items cannot collapse into the broad Outcome Result. The first successful formal Compile is the Authority Lock and freezes an immutable task base. Every later revision compares against active authority; deleting progress, Receipts or cache and restoring implementation code cannot reopen a pre-lock weakening window. Source hashes/file sets/mappings, selected Context topology/files/hashes, canonical Product/Acceptance/Global semantic projections and verifier content are revision materials.
...
- A recommended Source Plan structure is an optional authoring fast path, never a required input protocol or proof that every real requirement was declared.
...
- One root V2 `delivery-contract.yaml` is authoring authority. Existing Outcome fragments are physical compatibility only. Declared original sources remain provenance and Source Claims preserve direct coverage to Product/Technical Claims, named Assertions, Global Claims, external confirmations or explicit dispositions; there is no Packet revision chain, Source Unit inventory, SFC graph, Wave, Delivery Set, top-level Contract split or second plan.
- Ordinary prose and an optional Source Plan are equally valid Source inputs. When Source provides stable semantic keys and anchors, Contract authoring preserves them where practical. Structural decomposition and evidence-backed repository binding may continue without changing product meaning; new product semantics require an explicit decision instead of silent Contract expansion.
- `preflight` is a read-only Authoring check that applies Compact defaults and aggregates schema, Source, Context, Outcome, Claim/Assertion, risk, owner/path/binding, runner/input and proof diagnostics without creating Authority Lock, cache, progress, Receipt, pending revision or running project checks.
- Formal Compile is static and creates Authority Lock only after authoring is complete. It validates normalized schema, Source and Global/Outcome Result/Requirement/control-field/non-completing/obligation/shortcut Claim Coverage, named Acceptance mapping, one canonical repository path/pattern grammar shared by parsing, hashing, matching, subset and overlap, Context/source/path/runner/proof identities, risk floor, UI placement proof, strict obligations and executable acceptance. One leading `./` and Windows separators normalize to `/`; internal `.`/`..`, control characters, empty segments, absolute/drive/UNC paths and unsupported glob syntax are rejected.
...
- Assertions have readable criteria and fail closed when an Observation is missing or type-incomparable. Within a Check, one Observation path belongs to one Assertion. Playwright exposes stable AC-case observations and rejects missing, duplicate, skipped, flaky/unexpected or invalid cases. Negative proof requires an explicit Observation/value and implicit absence operators are unsupported. Counterfactual proof is valid only when the designated Assertions are the complete finding set.
...
- Active authority is a complete compiled snapshot in a Git common-dir V3 record paired with a worktree Git-config marker that binds task id, authority revision and compiled identity. Commit, legacy/verifier migration, CAS clear, valid abandon and corrupt-state cleanup share one diagnostic active-state lock. Final Gate and targeted verify recheck task/revision/compiled/worktree identity before acceptance or progress writes; Stop/close clear only the identity actually accepted. The workdir compiled file remains a rebuildable projection only.
...
- Compact and fully expanded V2 authoring normalize to the same Contract/authority/compiled identity. Product Requirements, control placement and Assertion criteria are first-class authority, Source AC may map directly to named Assertions, and findings project Source/Claim/AC/Observation/owner-path repair context.
