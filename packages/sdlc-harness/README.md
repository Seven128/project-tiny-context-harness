...
When REVIEWING, TESTING or RELEASING finds an implementation deviation while the existing PRD, UI/UX and technical plan are still correct, the manager uses `bugfix_implementation_gap` with `python3 tools/transition.py --to SPRINTING` to create a small bugfix task. When TESTING finds a bug, first record `Bugfix Route` in `.docs/07_test/TEST_REPORT.md`; requirement, acceptance, product-boundary, UX contract, screen contract, handoff matrix, `DESIGN.md`, technical-plan, interface-contract, task-breakdown, Development Self-Test Contract or Module Key Test Graph changes go through RFC instead of direct returns to design or development.
