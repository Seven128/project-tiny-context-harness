...
- 本 Prompt 不直接重切 PRD 或 tech plan；如果发现上游语义边界错误，进入 `BLOCKED`、创建 RFC，或请求回到 `ARCHITECTING`。
- gate 通过后调用 `pjsdlc_implementation_doc`，由该 Prompt 按真实实现更新或新增 `.docs/04_implementation/` 模块级 slice。
