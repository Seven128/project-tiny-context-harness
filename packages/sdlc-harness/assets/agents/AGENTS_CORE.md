...
- `REQUIREMENT_GATHERING`、`ARCHITECTING` 和 `SPRINTING` 都受 `plan.yaml` 管控；产品方案生成/切片使用 `PRD-*` task，架构和技术方案生成/切片使用 `DES-*` task，开发实现使用 `DEV-*` task。
- `next_task_sequence` 记录下一个可分配的 workflow task 序号，避免删除历史 task 后发生 id 冲突。
- 文档生产 task 使用 `result_docs` 指向本 task 产出的 PRD、architecture、tech plan、ADR 或 `plan.draft.yaml`；开发 task 使用 `implementation_doc` 指向模块级实现事实。
...
4. 在 `REQUIREMENT_GATHERING`、`ARCHITECTING` 和 `SPRINTING` 阶段，一次只执行一个任务。
5. 在 `REQUIREMENT_GATHERING`、`ARCHITECTING` 和 `SPRINTING` 阶段，只编辑当前 open task 的 `allowed_paths` 允许的文件。
...
13. open task 必须在 `plan.yaml` 中包含完整执行合同，再继续推进或交接；方案生成、现有文档切片和基于上一阶段事实源生成也必须先落到 `plan.yaml` task。
...
- “完善产品方案 / 写 PRD / 文档切片 / 我提供信息，你帮我完善产品方案” → 等价 `/prd`，一次只执行一个 `PRD-*` task。
- “设计技术方案 / 做架构方案 / 根据 PRD 做技术方案 / 切技术方案” → 等价 `/design`，一次只执行一个 `DES-*` task。
...
- `/prd`：在 `REQUIREMENT_GATHERING` 创建或选择一个最小 `PRD-*` task，澄清需求或切片文档，并只更新当前 task 对应的 PRD、验收标准、open questions、`.docs/INDEX.md` 和 overview。
- `/design`：在 `ARCHITECTING` 创建或选择一个最小 `DES-*` task，基于 PRD 生成或切分当前 task 对应的 architecture、tech plan 和 `plan.draft.yaml`。
