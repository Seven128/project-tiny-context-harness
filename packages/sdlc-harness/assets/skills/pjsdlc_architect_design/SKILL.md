...
如果是 `SPRINTING` 或之后通过 RFC 回到 `ARCHITECTING`，默认只修正 RFC impact analysis 和测试 / review 报告证明有问题的 tech plan、interface contract、task breakdown、Development Self-Test Contract 或 Module Key Test Graph，并引用 `.docs/rfc/RFC_*.md` 和相关失败 scenario。不要把这条路径扩成全量重设计；如果补方案时发现还需要修改 PRD、acceptance criteria、产品边界或 UI/UX 事实，回到 RFC 影响面并转向 `REQUIREMENT_GATHERING` 或 `UI_UX_DESIGNING`。
...
9. 从 RFC 回流的设计任务必须引用 `.docs/rfc/RFC_*.md` 和 review / test 失败 scenario，只补受影响的 tech plan / draft task，不修改产品或 UX 事实。若影响面属于 UX contract、screen states、handoff matrix 或 DESIGN.md，应返回 `UI_UX_DESIGNING`。
