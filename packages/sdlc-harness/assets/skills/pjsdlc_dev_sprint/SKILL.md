...
开发内部循环默认采用 `Standard Thin` gate 策略：实现、RFC follow-up 和 debug 修复过程中，优先运行当前变更面相关的 focused product/domain tests、smoke、self-test scenario 或 task `required_gates` 子集，并做必要的 dirty-file、allowed_paths、`plan.yaml` 和 implementation doc 轻量检查。完整 workflow gate 不在每次小改后重复运行；它集中在 task completion、pre-commit、task implementation commit 前、completion ledger 前、phase transition、release、package/source/managed asset 变更、public CLI / validator 变更和 high-risk provider/live 边界。Product quality gate 不打薄，打薄的是与当前变更无关或重复的 workflow-control gate。
...
- 当前任务关联的 PRD、`.work_products/02_experience/**`、`DESIGN.md` 和技术方案
- 当前源码和测试文件
...
- `.work_products/04_implementation/` 下相关模块、子系统或核心数据流的 implementation doc
- 当前 task `working_notes` 或 implementation doc `Verification` 中的 gate evidence
- high-risk runtime/live task 的 `plan.yaml#resume_capsule` 和 `.work_products/09_runbooks/**` runbook / evidence index / exploration appendix
...
- open task 在 `plan.yaml` 中直接保存 `phase: "SPRINTING"`、`work_products`、`allowed_paths`、`required_gates`、`acceptance_criteria`、`implementation_work_product` 和必要的 `working_notes`。
...
- gate 通过后调用 `pjsdlc_implementation_doc`，由该 Skill 按真实实现更新或新增 `.work_products/04_implementation/` 模块级 slice。
...
5. gate、implementation doc、`.work_products/INDEX.md` 和 `overview.md` 完成后，在当前 task 仍位于 `plan.yaml` 时创建 task implementation commit。
...
5. 开发中按 `Standard Thin` 优先运行 focused product/domain gate；`make validate-dev` 验证当前 open task，通常放在 task completion / pre-commit 边界，它通过不表示可以进入 `REVIEWING`。
...
18. worker 不更新主事实源；主 Agent 才能更新 `plan.yaml`、`.work_products/INDEX.md`、overview、implementation doc 和最终 gate 证据。
...
- [ ] UI/frontend/browser/page task 已实现 `work_products.uiux` / `work_products.design_system` 承诺的 screen contracts、interaction states、responsive、a11y/focus/keyboard/touch 和 DESIGN.md tokens，或已按 RFC/回流路径记录 blocker。
...
- [ ] 如果当前 task 是 high-risk runtime/live/remote-operator 工作，`resume_capsule` 已更新为 5-8 条恢复事实，`recovery_refs` 链接 implementation doc 和 `.work_products/09_runbooks/**` runbook/evidence，implementation doc 已填写短的 `Current Operator Path` 和分层 `Gate Breakdown`。
