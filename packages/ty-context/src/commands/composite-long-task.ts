...
export async function compositeLongTask(args: string[]): Promise<void> {
  ...
  if (subcommand === "compile") { const gate = await assertLongTaskCompletionGate(process.cwd()); const contract = await compileLongTaskContract(workdir, process.cwd(), { write: false }); await activateLongTask(contract, gate.bundle_sha256); await writeCompiledLongTaskContract(workdir, contract); console.log(`compiled contract=${contract.contract_sha256} requirements=${contract.requirements.length} plan_items=${contract.plan_items.length} obligations=${contract.obligations.length} bindings=${contract.bindings.length} acs=${contract.acceptance_criteria.length} proofs=${contract.proof_requirements.length} specs=${contract.verification_specs.length}`); return; }
