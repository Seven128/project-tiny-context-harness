...
import { compileLongTaskContract, writeCompiledLongTaskContract } from "../lib/long-task-contract-compiler.js";
...
export async function compositeLongTask(args: string[]): Promise<void> {
  ...
  if (subcommand === "init") { await assertLongTaskCompletionGate(process.cwd()); await initialize(workdir); console.log(`initialized composite long-task Contract V3 at ${workdirArg}`); return; }
  if (subcommand === "compile") { const campaignId=option(args,"--campaign-id"); const sliceId=option(args,"--slice-id"); if (!!campaignId !== !!sliceId) throw new Error("compile requires --campaign-id and --slice-id together"); rejectUnknown(args.slice(2), campaignId && sliceId ? ["--campaign-id",campaignId,"--slice-id",sliceId] : []); const gate = await assertLongTaskCompletionGate(process.cwd()); const contract = await compileLongTaskContract(workdir, process.cwd(), { write: false }); await activateLongTask(contract, gate.bundle_sha256, campaignId && sliceId ? { campaign_id:campaignId, slice_id:sliceId } : {}); await writeCompiledLongTaskContract(workdir, contract); console.log(`compiled contract=${contract.contract_sha256} requirements=${contract.requirements.length} plan_items=${contract.plan_items.length} obligations=${contract.obligations.length} bindings=${contract.bindings.length} acs=${contract.acceptance_criteria.length} proofs=${contract.proof_requirements.length} specs=${contract.verification_specs.length}`); return; }
