...
export async function compositeLongTask(args: string[]): Promise<void> {
  ...
  if (subcommand === "final-gate") { rejectUnknown(args.slice(2), []); const result = await finalGateLongTaskViaHost(workdir); console.log(JSON.stringify(result)); if (result.workflow_status === "externally_blocked") process.exitCode = 4; else if (result.workflow_status !== "accepted") process.exitCode = 1; return; }
