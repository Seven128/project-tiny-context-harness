...
export function parseArgs(args: string[]): ExportContextArgs {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    else if (arg === "--task-context") index = readValue(args, index, "export-context --task-context requires a name", (value) => {
    ...
    } else if (arg === "--output") index = readValue(args, index, "export-context --output requires a path", (value) => (parsed.output = value));
    else if (arg.startsWith("--output=")) parsed.output = inlineValue(arg, "--output", "export-context --output requires a path");
    else if (arg === "--profile") index = readValue(args, index, "export-context --profile requires an id", (value) => (parsed.profile = value));
    else if (arg.startsWith("--profile=")) parsed.profile = inlineValue(arg, "--profile", "export-context --profile requires an id");
    else if (arg === "--include-context") index = readValue(args, index, "export-context --include-context requires a repo-relative path or glob", (value) => parsed.includeContext.push(value));
    else if (arg.startsWith("--include-context=")) parsed.includeContext.push(inlineValue(arg, "--include-context", "export-context --include-context requires a repo-relative path or glob"));
    else if (arg === "--include-code") index = readValue(args, index, "export-context --include-code requires a repo-relative path or glob", (value) => parsed.includeCode.push(value));
    else if (arg.startsWith("--include-code=")) parsed.includeCode.push(inlineValue(arg, "--include-code", "export-context --include-code requires a repo-relative path or glob"));
    else if (arg === "--bundle-strategy") index = readValue(args, index, "export-context --bundle-strategy requires auto, area, topdir or config", (value) => (parsed.bundleStrategy = parseBundleStrategy(value)));
    else if (arg.startsWith("--bundle-strategy=")) parsed.bundleStrategy = parseBundleStrategy(inlineValue(arg, "--bundle-strategy", "export-context --bundle-strategy requires auto, area, topdir or config"));
    else if (arg === "--max-pack-files") index = readValue(args, index, "export-context --max-pack-files requires a positive integer", (value) => (parsed.maxPackFiles = parseInteger(value, "--max-pack-files", false)));
    else if (arg.startsWith("--max-pack-files=")) parsed.maxPackFiles = parseInteger(inlineValue(arg, "--max-pack-files", "export-context --max-pack-files requires a positive integer"), "--max-pack-files", false);
    else if (arg === "--max-bundle-characters") index = readValue(args, index, "export-context --max-bundle-characters requires a positive integer", (value) => (parsed.maxBundleCharacters = parseInteger(value, "--max-bundle-characters", false)));
    else if (arg.startsWith("--max-bundle-characters=")) parsed.maxBundleCharacters = parseInteger(inlineValue(arg, "--max-bundle-characters", "export-context --max-bundle-characters requires a positive integer"), "--max-bundle-characters", false);
    else if (arg === "--prune") index = readValue(args, index, "export-context --prune requires a non-negative integer", (value) => (parsed.prune = parseInteger(value, "--prune", true)));
    else if (arg.startsWith("--prune=")) parsed.prune = parseInteger(inlineValue(arg, "--prune", "export-context --prune requires a non-negative integer"), "--prune", true);
...
function readValue(args: string[], index: number, message: string, apply: (value: string) => void): number {
...
function parseBundleStrategy(value: string): "auto" | "area" | "topdir" | "config" {
  if (value === "auto" || value === "area" || value === "topdir" || value === "config") return value;
