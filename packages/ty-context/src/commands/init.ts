...
import {
  normalizeHarnessFolderName,
  readHarnessRootConfig,
} from "../lib/harness-root.js";
...
export async function init(args: string[]): Promise<void> {
  ...
  if (configuredRoot) {
    ...
    console.log(
      `configured package.json tyContext.harnessFolderName=${JSON.stringify(configuredRoot)}`,
    );
  }
...
export async function resolveInitHarnessRoot(
  projectRoot: string,
  args: string[],
): Promise<string | undefined> {
  const argRoot =
    valueForArg(args, "--harness-folder") ??
    valueForArg(args, "--harnessFolderName");
  if (argRoot) {
...
async function promptAgentHarnessRoot(): Promise<string> {
  ...
  try {
    const agent = await questionUntilValid(
      rl,
    ...
    const folder = await rl.question(
      `Harness folder name (default ${DEFAULT_HARNESS_ROOT}; press Enter to use default): `,
    );
...
async function questionUntilValid(
...
function formatAgentChoices(): string {
  ...
  AGENT_HARNESS_OPTIONS.forEach((option, index) => {
    const folder = option.harnessFolderName
      ? ` -> ${option.harnessFolderName}`
      : "";
...
export function resolveAgentHarnessFolderName(
  agentAnswer: string,
  customFolderAnswer = "",
): string {
  ...
  return normalizeHarnessFolderName(
    option.harnessFolderName ??
      (customFolderAnswer.trim() || DEFAULT_HARNESS_ROOT),
  );
}
