...
export async function exportContext(args: string[]): Promise<void> {
  ...
  if (
    parsed.help ||
    (!parsed.full && !parsed.code && !parsed.all && !parsed.sourceMode)
  ) {
  ...
  try {
    ...
    console.log(
      parsed.check
        ? "export-context check OK"
        : "export-context wrote artifact",
    );
    console.log(`mode: ${report.mode}`);
    console.log(`output: ${report.outputRelativePath}`);
    if (report.mode === "code") {
      console.log(
        `source code count: ${report.sourceCodeCount ?? report.sourceFiles.length}`,
      );
...
async function runLegacyAll(check: boolean): Promise<void> {
  ...
  console.log(
    check ? "export-context check OK" : "export-context wrote artifacts",
  );
  ...
  console.log(
    `source code count: ${codeReport.sourceCodeCount ?? codeReport.sourceFiles.length}`,
  );
...
function printSourcePackReport(report: SourcePackReport, check: boolean): void {
  ...
  for (const artifact of report.artifacts) {
    console.log(
      `- ${artifact.kind}: ${artifact.path} (${artifact.characters} chars, ${artifact.source_count} sources)`,
    );
  }
