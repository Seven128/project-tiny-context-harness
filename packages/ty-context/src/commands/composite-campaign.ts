...
import { compileAndSealLongTaskContractViaHost, finalGateLongTaskViaHost } from "../lib/long-task-host-client.js";
...
export async function compositeCampaign(args: string[]): Promise<void> { const command = args[0] ?? "help"; if (command === "help") { help(); return; } const options = parse(args.slice(1)); const root = process.cwd(); let result: unknown;
  ...
  else if (command === "apply-packet") result = await applyPacketV3(root, required(options, "--campaign"), required(options, "--slice"), required(options, "--input"));
  else if (command === "render") result = await renderCampaignV3(root, required(options, "--campaign"), required(options, "--slice"));
  ...
  else if (command === "start") { await assertLongTaskHostGate(root,{allow_existing_authority:true}); result = await bindCampaignGoalV3(root, required(options, "--campaign"), required(options, "--slice"), required(options, "--goal-id"),compileAndSealLongTaskContractViaHost); }
  else if (command === "record-result") result = await recordCampaignResultV3(root, required(options, "--campaign"), required(options, "--slice"), required(options, "--workdir"),finalGateLongTaskViaHost);
