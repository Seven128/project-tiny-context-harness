...
export async function readHarnessRootConfig(
  projectRoot: string,
): Promise<HarnessRootConfig> {
...
export async function readHarnessRootConfigCandidates(
  projectRoot: string,
): Promise<HarnessRootConfigCandidate[]> {
  const candidates: HarnessRootConfigCandidate[] = [];
  const packageJson = await readJsonConfig(
    path.join(projectRoot, "package.json"),
  );
  const packageConfig =
    packageJson && typeof packageJson === "object"
      ? (packageJson as Record<string, unknown>).tyContext
      : undefined;
  ...
  const explicitConfig = await readJsonConfig(
    path.join(projectRoot, HARNESS_JSON_CONFIG_PATH),
  );
  ...
  const legacyPackageConfig =
    packageJson && typeof packageJson === "object"
      ? (packageJson as Record<string, unknown>).sdlcHarness
      : undefined;
  ...
  const legacyExplicitConfig = await readJsonConfig(
    path.join(projectRoot, LEGACY_HARNESS_JSON_CONFIG_PATH),
  );
