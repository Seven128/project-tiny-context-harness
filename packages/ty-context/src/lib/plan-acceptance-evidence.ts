...
const BLOCKING_AUDITOR_STATUSES = new Set([
  "partial",
  "blocked",
  "invalidated",
]);
const SENSITIVE_EVIDENCE_PATTERNS = [
  {
    ...
    pattern:
      /\bauthorization\s*:\s*bearer\s+(?!<redacted>|redacted|\[redacted\])[A-Za-z0-9._~+/=-]{8,}/i,
  },
  {
    label: "cookie value",
    pattern:
      /\bcookie\s*[:=]\s*(?!<redacted>|redacted|\[redacted\])[^;\s]{8,}/i,
  },
  {
...
export function assertExternalReviewerFields(
  ...
  if (hasUnresolvedMissingRequiredLayers(row)) {
    errors.push(
      `${label} is complete but missing_required_layers is not empty`,
    );
  }

  const driftSeverity = String(row.drift_severity ?? row.driftSeverity ?? "")
    .trim()
    .toLowerCase();
  ...
  if (siblingSubstitutionUsed(row) && !hasSiblingSubstitutionApproval(row)) {
    errors.push(
      `${label} is complete but sibling_substitution_used without approval`,
    );
  }

  if (
    liveProofSubstitutionUsed(row) &&
    !hasLiveProofSubstitutionApproval(row)
  ) {
    errors.push(
      `${label} is complete but live_proof_substitution_used without approval`,
    );
  }

  const auditorStatus = String(row.auditor_status ?? row.auditorStatus ?? "")
    .trim()
    .toLowerCase();
  ...
  if (hasOnlySelfCertifyingEvidence(evidenceText)) {
    errors.push(
      `${label} is complete but fresh_evidence contains only summary or self-certifying evidence`,
    );
  }
  ...
  if (sensitiveEvidence) {
    errors.push(
      `${label} is complete but evidence contains raw secret/token/cookie material: ${sensitiveEvidence}`,
    );
  }
}

export function findSensitiveEvidence(text: string): string | undefined {
  return SENSITIVE_EVIDENCE_PATTERNS.find((item) => item.pattern.test(text))
    ?.label;
}

function hasUnresolvedMissingRequiredLayers(
  row: Record<string, unknown>,
): boolean {
  const value =
  ...
  if (Array.isArray(value)) {
    return value.some(
      (item) => !isBlankish(item) && !isStructuredLayerNa(item),
    );
  }
...
function isStructuredLayerNa(value: unknown): boolean {
  ...
  const hasNaStatus =
    isOutOfScope(object) || /\bout[_ -]?of[_ -]?scope[_ -]?NA\b/i.test(text);
  const hasSource =
...
function hasSiblingSubstitutionApproval(row: Record<string, unknown>): boolean {
  return (
    ...
    /\b(approved|explicitly allowed|out[_ -]?of[_ -]?scope[_ -]?NA)\b/i.test(
      primitiveText([
        row.sibling_substitution_approved,
        row.substitution_approval,
      ]),
    )
  );
}
...
function liveProofSubstitutionUsed(row: Record<string, unknown>): boolean {
  const value =
    row.live_proof_substitution_used ?? row.liveProofSubstitutionUsed;
  if (value === true) {
    return true;
  }
  const text = primitiveText(value);
...
function hasLiveProofSubstitutionApproval(
  row: Record<string, unknown>,
): boolean {
  return (
    ...
    /\b(approved|explicitly allowed|out[_ -]?of[_ -]?scope[_ -]?NA)\b/i.test(
      primitiveText([
        row.live_proof_substitution_approved,
        row.liveProofSubstitutionApproved,
      ]),
    )
  );
}
