...
export async function readSourcePackProfile(projectRoot: string, profileId?: string): Promise<SourcePackProfile> {
  if (!profileId) {
  ...
  const sourcePacks = parsed && typeof parsed === "object" ? (parsed as { source_packs?: unknown }).source_packs : undefined;
  const profileValue = sourcePacks && typeof sourcePacks === "object" ? (sourcePacks as Record<string, unknown>)[profileId] : undefined;
...
export async function parseContextAreas(projectRoot: string): Promise<ContextAreaMapping[]> {
...
export function mergePatterns(...groups: Array<string[] | undefined>): string[] {
...
function readPatternList(value: unknown, label: string, pathLabel: string): string[] {
