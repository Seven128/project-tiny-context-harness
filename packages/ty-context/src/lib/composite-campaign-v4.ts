...
import {
  ...
  type SourceUnitPacketBindingV4,
...
export async function createCampaignV4(
  ...
  try {
    ...
    await writeFile(
      ...
      `${buildInitialCampaignEventV1(event("campaign_created", campaign) as Record<string, unknown>)}\n`,
      { flag: "wx" },
    );
...
export async function applyCampaignScopeV4(
  projectRoot: string,
  campaignPath: string,
  scopeFile: string,
  coverageFile: string,
  ...
  const coverage = parseSourceCoverageV1(
  ...
  else validateSourceCoverageAgainstScopeV3(scope, coverage);
  const contextBaseline =
    scope.schema_version === "scope-fit-result-v4"
      ? await captureCampaignContextBaseline(
          projectRoot,
  ...
  return withCampaignMutation(
    ...
    async (root, campaign, transaction) => {
      ...
      if (
        currentSchema === CAMPAIGN_SCHEMA_V5 &&
        scope.schema_version !== "scope-fit-result-v4"
      )
        throw new Error("Campaign V5 requires scope-fit-result-v4");
      if (
        currentSchema === CAMPAIGN_SCHEMA_V4 &&
        scope.schema_version !== "scope-fit-result-v3"
      )
        throw new Error("Campaign V4 audit data requires scope-fit-result-v3");
      if (
      ...
      if (
        currentSchema === CAMPAIGN_SCHEMA_V5
          ? campaignHasGoalV5(assertCampaignV5(campaign))
          : Object.keys(campaign.waves).length > 0
      )
        throw new Error(
      ...
      if (prior) {
        if (scope.schema_version !== prior.schema_version)
          throw new Error("Scope Fit schema cannot change within a Campaign");
        if (scope.schema_version === "scope-fit-result-v4")
          validateScopeFitGraphV4(scope, prior as ScopeFitResultV4);
        else validateScopeFitGraphV3(scope, prior as ScopeFitResultV3);
      }
      const slices: Record<string, CampaignSliceV4> = {};
      for (const slice of scope.slices)
        slices[slice.slice_id] =
          campaign.slices[slice.slice_id] ??
          emptyCampaignSlice(currentSchema === CAMPAIGN_SCHEMA_V5);
      campaign.graph = {
      ...
      campaign.campaign_status =
        scope.decision === "blocked_for_decision"
          ? "decision_blocked"
          : "authoring";
      if (currentSchema === CAMPAIGN_SCHEMA_V5 && contextBaseline) {
        (campaign as unknown as CampaignV5).context_baseline = contextBaseline;
      }
...
export async function applyCampaignPacketV4(
  projectRoot: string,
  campaignPath: string,
  sliceId: string,
  packetFile: string,
  ...
  await withCampaignMutation(
    ...
    async (root, campaign, transaction) => {
      ...
      const revisionRelative = relativePath(root, revisionPath);
      await transaction.stageFile(
        `${revisionRelative}/authoring-packet.json`,
        canonicalJson(packet),
      );
      for (const [file, content] of packetAuthorityFiles(packet))
...
export async function renderCampaignPacketV4(
  projectRoot: string,
  campaignPath: string,
  sliceId: string,
...
export async function preflightCampaignPacketV4(
  projectRoot: string,
  campaignPath: string,
  sliceId: string,
): Promise<{
  ...
  const verified = await verifyCampaignPacketV4(
    projectRoot,
    campaignPath,
    sliceId,
  );
  await withCampaignMutation(
    ...
    async (_campaignRoot, campaign) => {
      ...
      if (schemaOf(campaign) === CAMPAIGN_SCHEMA_V5) {
        (campaign as unknown as CampaignV5).context_baseline =
          verified.context_baseline;
      }
      return campaign;
    },
  );
...
export async function verifyCampaignPacketV4(
  projectRoot: string,
  campaignPath: string,
  sliceId: string,
): Promise<{
  ...
  const rendered = await renderCampaignPacketV4(
    projectRoot,
    campaignPath,
    sliceId,
  );
  const previousBaseline = await optionalPacketContextBaseline(
  ...
  if (previousBaseline) {
    await assertCampaignContextBaselineFresh(
      projectRoot,
      previousBaseline,
      previousBaseline.context_files.map((file) => file.path),
    );
  }
  ...
  const sourceCoverage = parseSourceCoverageV1(
  ...
  if (scope.schema_version === "scope-fit-result-v4") {
    ...
    const scopeSliceV4 = scope.slices.find(
      (slice) => slice.slice_id === sliceId,
    )!;
    const selected = new Set(scopeSliceV4.source_unit_refs);
    sourceUnits = scope.source_units.filter((unit) =>
  ...
  await atomic(
    path.join(rendered.revision_path, "packet-index.json"),
    canonicalJson(index),
  );
  await atomic(
    path.join(rendered.revision_path, "conflict-profile.json"),
    canonicalJson(profile),
  );
  await atomic(
    path.join(rendered.revision_path, "context-baseline.json"),
    canonicalJson(contextBaseline),
  );
  await atomic(
    path.join(rendered.revision_path, "change-envelope.json"),
    canonicalJson(changeEnvelope),
  );
  return {
    coverage,
...
export async function loadCampaignV4(
  projectRoot: string,
  supplied: string,
  ...
  if (!inside(base, root) || root === base)
    throw new Error("Campaign path escapes the V4 campaign root");
  const [baseReal, rootReal] = await Promise.all([
    realpath(base),
    realpath(root),
  ]);
  if (!inside(baseReal, rootReal) || rootReal === baseReal)
    throw new Error("Campaign realpath escapes the V4 campaign root");
  await recoverCampaignStoreV1(rootReal);
  const text = await readBounded(
    path.join(rootReal, "campaign.yaml"),
    "campaign.yaml",
  );
  const parsed = parseStrictYaml(text);
  const campaign = (schemaOf(parsed) === CAMPAIGN_SCHEMA_V5
    ? assertCampaignV5(parsed)
    : assertCampaignV4(parsed)) as unknown as CampaignV4;
  if (
    sha256Hex(
      await readBounded(
...
export function campaignRootV4(projectRoot: string, id: string): string {
  return path.join(
...
export function currentPacketRevisionPathV4(
  campaignRoot: string,
  campaign: CampaignV4,
  sliceId: string,
): string {
  return currentRevisionRoot(path.resolve(campaignRoot), campaign, sliceId);
}
...
async function withCampaignMutation(
  ...
  try {
    ...
    const next = await mutate(loaded.root, current, transaction);
    ...
    await commitCampaignTransactionV1({
      ...
      event: event(eventType, next) as Record<string, unknown>,
      expectedGeneration,
...
function emptyCampaignSlice(v5 = false): CampaignSliceV4 {
  const slice: CampaignSliceV4 = {
    status: "planned",
  ...
  return v5
    ? ({ ...slice, thread: emptyThreadStateV5() } as unknown as CampaignSliceV4)
    : slice;
}
function revisionRoot(root: string, sliceId: string, revision: number): string {
  return path.join(
    root,
    "slices",
    sliceId,
...
function currentRevisionRoot(
  ...
  return revisionRoot(root, sliceId, revision);
}
async function optionalScope(
  root: string,
): Promise<ScopeFitResultV3 | ScopeFitResultV4 | null> {
  try {
    return parseCurrentScope(
      await readFile(path.join(root, "scope-fit.json"), "utf8"),
    );
  } catch (error) {
...
async function optionalPacketContextBaseline(
  root: string,
...
async function renderPacket(
  ...
  for (const [file, content] of packetAuthorityFiles(packet))
    await atomic(path.join(root, file), content);
}

function packetAuthorityFiles(
  packet: CompositeAuthoringPacketV3,
): Array<[string, string]> {
  const values = {
...
function parseCurrentScope(
  content: string,
): ScopeFitResultV3 | ScopeFitResultV4 {
...
function assertCurrentCampaign(value: unknown): CampaignV4 {
  return (schemaOf(value) === CAMPAIGN_SCHEMA_V5
    ? assertCampaignV5(value)
    : assertCampaignV4(value)) as unknown as CampaignV4;
}
...
async function readSafeFile(
  ...
  const lexical = resolveInside(project, supplied, label);
...
async function readBounded(file: string, label: string): Promise<string> {
  ...
  if (!info.isFile() || info.size > MAX_TRACKED_FILE_BYTES)
    throw new Error(
      `${label} must be a regular file no larger than ${MAX_TRACKED_FILE_BYTES} bytes`,
    );
...
function event(type: string, campaign: CampaignV4): unknown {
  return {
    type,
