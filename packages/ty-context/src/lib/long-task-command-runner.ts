...
export async function runFrozenCommand(spec: FrozenVerificationSpecV3, snapshotRoot: string, outputRoot: string, artifactRoot: string, environmentOverrides: Readonly<Record<string,string>> = {}): Promise<CommandRunV2> {
...
function allowedEnvironment(artifactRoot: string,overrides:Readonly<Record<string,string>>): NodeJS.ProcessEnv { const result: NodeJS.ProcessEnv = {}; for (const key of ["PATH", "Path", "PATHEXT", "SYSTEMROOT", "WINDIR", "HOME", "USERPROFILE", "TMP", "TEMP", "CI", "LANG", "LC_ALL"]) if (process.env[key] !== undefined) result[key] = process.env[key]; result.TY_CONTEXT_ORACLE_PROTOCOL = "ty-context-observation-v2"; result.TY_CONTEXT_ARTIFACT_DIR = artifactRoot;for(const [key,value] of Object.entries(overrides))result[key]=value; return result; }
