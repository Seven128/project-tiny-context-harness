...
export function decideLongTaskImpact(contract: CompiledContractV3, changedPaths: string[]): ImpactDecision {
  ...
  const selected = [...new Set([...matched, ...contract.verification_specs.filter((spec) => spec.global_invariant)].map((spec) => spec.id))].sort();
  return { mode: "affected", verification_spec_ids: selected, reason: "frozen input_paths matched current changes" };
}
function matches(value: string, glob: string): boolean { return new RegExp(`^${glob.replace(/[.+^${}()|[\]\\]/g, "\\$&").replace(/\*\*/g, ".*").replace(/\*/g, "[^/]*")}$`, "i").test(value); }
