...
export async function runLongTaskFinalGate(workdir: string): Promise<FinalResultV2> {
  ...
  const acceptanceResults: Record<string,"passed"|"failed"> = Object.fromEntries(contract.acceptance_criteria.map((criterion) => [criterion.id, criterion.verification_spec_ids.some((spec) => failedSpecs.has(spec)) ? "failed" as const : "passed" as const]));
  const obligationResults: Record<string,"passed"|"failed"> = Object.fromEntries(contract.obligations.map((obligation) => [obligation.id, obligation.related_ac_ids.some((ac) => acceptanceResults[ac] !== "passed") ? "failed" as const : "passed" as const]));
  const requirementResults: Record<string,"passed"|"failed"> = Object.fromEntries(contract.requirements.map((requirement) => [requirement.id, contract.graphs.requirements[requirement.id].obligation_ids.some((obligation) => obligationResults[obligation] !== "passed") ? "failed" as const : "passed" as const]));
