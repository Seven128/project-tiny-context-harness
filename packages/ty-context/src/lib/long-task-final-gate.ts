...
import type { FinalResultV3, LongTaskEntityResultV3, LongTaskFindingV2 } from "./long-task-run-result.js";
import { writeLongTaskFinalAuthority } from "./long-task-final-receipt.js";

export async function runLongTaskFinalGate(workdir:string):Promise<FinalResultV3>{
  ...
  try{
    ...
    const after=await hashLongTaskWorkspace(contract.repository_root,contract);const findings:LongTaskFindingV2[]=[...run.findings,...binding.findings,...counterfactual.findings];
    ...
    const result:FinalResultV3={
      schema_version:"long-task-final-result-v3",workflow_status:accepted?"accepted":"needs_work",contract_sha256:contract.contract_sha256,run_id:run.run_id,final_snapshot_sha256:run.snapshot.snapshot_sha256,
      ...
      started_at:run.started_at,completed_at:new Date().toISOString(),atomic_write_complete:true
    };
    await writeLongTaskFinalAuthority(contract,workdir,result);return result;
