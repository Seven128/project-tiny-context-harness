...
export const AC_STATUSES = new Set([
  "complete",
  "partial",
  "blocked",
  "not_run",
...
export const NON_COMPLETE_AC = new Set([
  "partial",
  "blocked",
  "not_run",
  "invalidated",
...
export function overallStatus(value: unknown): string {
  ...
  const object = value as Record<string, unknown>;
  return String(
    object.overall_status ?? object.overallStatus ?? object.status ?? "",
  ).trim();
}
...
export function isOutOfScope(row: Record<string, unknown>): boolean {
  return (
    statusOf(row) === "out_of_scope_NA" ||
    /out[_ -]?of[_ -]?scope|n\/a|not applicable/i.test(primitiveText(row))
  );
}
...
export function assertSurfaceConformance(
  ...
  if (isBlankish(row.owner_surface)) {
    errors.push(
      `${label} is surface/architecture conformance but owner_surface is empty`,
    );
  }
  if (isBlankish(row.required_user_paths)) {
    errors.push(
      `${label} is surface/architecture conformance but required_user_paths is empty`,
    );
  }
  if (isBlankish(row.real_page_evidence)) {
    errors.push(
      `${label} is surface/architecture conformance but real_page_evidence is empty`,
    );
  }
  if (isBlankish(row.context_fact_refs)) {
    errors.push(
      `${label} is surface/architecture conformance but context_fact_refs is empty`,
    );
  }
  if (
    !isBlankish(row.forbidden_primary_surfaces) &&
    isBlankish(row.negative_surface_checks)
  ) {
    errors.push(
      `${label} declares forbidden_primary_surfaces but negative_surface_checks is empty`,
    );
  }
  const userPathText = primitiveText([
    row.required_user_paths,
    row.primary_user_paths,
  ]);
  const evidenceText = primitiveText([
  ...
  for (const forbiddenSurface of valuesAsArray(
    row.forbidden_primary_surfaces,
  )) {
    if (userPathText.toLowerCase().includes(forbiddenSurface.toLowerCase())) {
      errors.push(
        `${label} routes a required/primary user path through forbidden surface: ${forbiddenSurface}`,
      );
    }
    if (evidenceText.toLowerCase().includes(forbiddenSurface.toLowerCase())) {
      errors.push(
        `${label} uses wrong owner surface evidence from forbidden surface: ${forbiddenSurface}`,
      );
    }
  }
  if (
    row.default_visibility_required === true &&
    !mentionsDefaultVisibility(primitiveText(row.real_page_evidence))
  ) {
    errors.push(
      `${label} requires default visibility but real_page_evidence does not record default-visible proof`,
    );
  }
}

export function assertStructuredNa(
  row: Record<string, unknown>,
  label: string,
  errors: string[],
): void {
  if (isBlankish(row.na_reason) && isBlankish(row.out_of_scope_reason)) {
    errors.push(
      `${label} is out_of_scope_NA but lacks na_reason or out_of_scope_reason`,
    );
  }
  if (
    isBlankish(row.scope_source) &&
    isBlankish(row.approval_source) &&
    isBlankish(row.source_reference)
  ) {
    errors.push(
      `${label} is out_of_scope_NA but lacks scope_source, approval_source or source_reference`,
    );
  }
}

function mentionsDefaultVisibility(value: string): boolean {
  return /\b(default[- ]?visible|visible by default|primary entry|first-level|top-level|main entry)\b/i.test(
    value,
  );
}
