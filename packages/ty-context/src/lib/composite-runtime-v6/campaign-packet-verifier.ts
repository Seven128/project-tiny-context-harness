...
import { loadCampaignStoreV6 } from "./campaign-store.js";
import {
  assertPacketProjectionsV6,
  currentPacketRevisionPathV6,
  optionalPacketContextBaselineV6,
  parseCurrentScopeV6,
  readPacketRevisionV6,
} from "./campaign-packet-io.js";

export async function renderCampaignPacketV6(
  projectRoot: string,
  campaignPath: string,
  sliceId: string,
  ...
  const revisionPath = currentPacketRevisionPathV6(root, campaign, sliceId);
  const packet = await readPacketRevisionV6(revisionPath, campaign, sliceId);
  await assertPacketProjectionsV6(revisionPath, packet);
  return {
...
export async function verifyCampaignPacketV6(
  projectRoot: string,
  campaignPath: string,
  sliceId: string,
): Promise<{
  ...
  const rendered = await renderCampaignPacketV6(
    projectRoot,
    campaignPath,
    sliceId,
  );
  const previousBaseline = await optionalPacketContextBaselineV6(
  ...
  if (!scopeSlice) throw new Error("packet_slice_absent_from_scope_fit");
  const packet = await readPacketRevisionV6(
    rendered.revision_path,
    campaign,
    sliceId,
  );
  ...
  for (const binding of coverage.global_constraint_bindings.filter(
    ...
    for (const id of binding.requirement_ids)
      if (!requirements.has(id))
        throw new Error(`global_binding_unknown_requirement:${id}`);
    for (const id of binding.acceptance_criterion_ids)
      if (!criteria.has(id))
        throw new Error(`global_binding_unknown_acceptance_criterion:${id}`);
    for (const id of binding.verification_spec_ids)
      if (!globals.has(id))
        throw new Error(`global_binding_requires_global_spec:${id}`);
  }
}
