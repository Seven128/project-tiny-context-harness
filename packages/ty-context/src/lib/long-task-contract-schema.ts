...
export type ObservationKind = "scalar" | "implementation_structure" | "browser_interaction" | "runtime_behavior" | "api_contract" | "data_state" | "security_boundary" | "population_coverage";
export type AssertionOperator = "equals" | "not_equals" | "contains" | "not_contains" | "matches" | "not_matches" | "greater_than" | "greater_or_equal" | "less_than" | "less_or_equal" | "truthy" | "falsy" | "exists" | "not_exists" | "set_equals" | "subset_of" | "superset_of";
export type OwnerSurfaceKind = "web" | "cli" | "api" | "runtime" | "data" | "security";
export type BindingKind = "path_glob" | "file" | "symbol" | "schema" | "route" | "runtime_capability";
...
export interface OwnerSurfaceV3 { id: string; kind: OwnerSurfaceKind; location: string; primary_action: string; expected_feedback: string }
export interface ProductRequirementV3 { id: string; statement: string; observable_outcome: string; owner_boundary: string; owner_surface_refs: string[]; context_refs?: string[]; task_local_reason?: string; population_policy: PopulationPolicy }
export interface ProductBoundaryV3 { id: string; rule: string; requirement_refs: string[] }
export interface NonCompletingOutcomeV3 { id: string; forbidden_outcome: string; requirement_refs: string[] }
export interface PopulationExclusionRuleV3 { id: string; rule: string; requirement_refs: string[] }
...
export type BindingVerificationV3 = { mode: "harness_static" } | { mode: "oracle_observation"; spec_id: string; observation_id: string };
export interface ImplementationBindingV3 { id: string; kind: BindingKind; target: string; verification: BindingVerificationV3 }
export interface ForbiddenShortcutV3 { id: string; statement: string; source_boundary_ids: string[]; source_non_completing_ids: string[] }
export interface LongTaskObligationV3 { id: string; statement: string; source_requirement_ids: string[]; implementation_bindings: ImplementationBindingV3[]; forbidden_shortcuts: ForbiddenShortcutV3[]; related_ac_ids: string[]; counterfactual_control_ids: string[] }
...
export interface TechnicalPlanV3 { schema_version: "technical-plan-v3"; plan_items: PlanItemV3[]; counterfactual_controls: CounterfactualControlV3[] }

export interface VerificationClaimsV3 { requirement_ids: string[]; plan_item_ids: string[]; obligation_ids: string[]; binding_ids: string[]; ac_ids: string[]; proof_requirement_ids: string[] }
export interface OracleDeclarationV3 { entrypoint: string }
export interface NetworkPolicyV3 { mode: "none" | "loopback" | "declared_hosts"; allowed_hosts: string[] }
export type CommandToolV3 = "package_script" | "project_binary" | "node_script" | "playwright_test";
export interface CommandStepV3 { id: string; tool: CommandToolV3; target: string; argv: string[]; cwd: string; timeout_ms: number; environment_refs: string[]; output_artifact_ids: string[] }
export interface EnvironmentRequirementV3 { id: string; reason_code: ExternalBlockerReasonCode; probe_spec_id: string; local_alternative_probe_ids: string[]; minimal_user_action: string }
export interface PositiveAssertionV3 { id: string; observation_id: string; observation_kind: ObservationKind; operator: AssertionOperator; expected?: unknown }
export interface NegativeAssertionV3 extends PositiveAssertionV3 { source_boundary_ids: string[]; source_non_completing_ids: string[]; source_forbidden_shortcut_ids: string[] }
export interface PopulationEnumeratorV3 { observation_id: string; exclusion_rule_ids: string[]; required_coverage_percent: 100 }
...
export interface ProofRequirementV3 { id: string; proof_surface: ProofSurface; obligation_refs: string[]; owner_surface_refs: string[]; verification_spec_ids: string[] }
...
export interface FrozenVerificationSpecV3 extends Omit<VerificationSpecV3, "positive_assertions" | "negative_assertions" | "environment_requirements"> {
...
export interface CompiledContractGraphsV3 {
  ...
  obligations: Record<string, { requirement_ids: string[]; binding_ids: string[]; ac_ids: string[]; proof_requirement_ids: string[]; counterfactual_control_ids: string[]; forbidden_shortcut_ids: string[] }>;
  acceptance_criteria: Record<string, { obligation_ids: string[]; proof_requirement_ids: string[]; verification_spec_ids: string[] }>;
  proof_requirements: Record<string, { obligation_ids: string[]; owner_surface_ids: string[]; verification_spec_ids: string[] }>;
...
export interface CompiledContractV3 {
  ...
  non_completing_outcomes: NonCompletingOutcomeV3[];
  population_exclusion_rules: PopulationExclusionRuleV3[];
  plan_items: Array<{ id: string; title: string; implementation_notes: string[]; obligation_ids: string[] }>;
