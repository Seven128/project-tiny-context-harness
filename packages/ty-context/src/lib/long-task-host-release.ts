...
export async function createUnsignedHostReleaseManifestV1(root: string, releaseVersion: string): Promise<HostReleaseManifestV1> {
  ...
  const required = ["requirements.toml", "long-task-hook.mjs", "ty-context-host-worker.mjs", process.platform === "win32" ? "ty-context-host-helper.exe" : "ty-context-host-helper", process.platform === "win32" ? "ty-context-host-admin.exe" : "ty-context-host-admin", process.platform === "win32" ? "ty-context-host-installer-ui.exe" : "ty-context-host-installer-ui"];
  ...
  const helperName = process.platform === "win32" ? "ty-context-host-helper.exe" : "ty-context-host-helper";
  const adminName = process.platform === "win32" ? "ty-context-host-admin.exe" : "ty-context-host-admin";
  const installerName = process.platform === "win32" ? "ty-context-host-installer-ui.exe" : "ty-context-host-installer-ui";
  const componentHashes = [helperName, adminName, installerName].map((name) => files.find((item) => item.path === name)?.sha256);
  if (new Set(componentHashes).size !== componentHashes.length) throw new Error("host_release_component_identity_conflict");
  return { schema_version: "ty-context-host-release-v1", release_version: releaseVersion, protocol: "ty-context-host-rpc-v1", files };
}

export function canonicalHostReleaseManifestV1(manifest: HostReleaseManifestV1): string { return canonicalJson(manifest); }

export async function verifyHostReleaseDirectoryV1(root: string, pinnedRootPublicKeyPem: string, options: { allow_runtime_files?: boolean } = {}): Promise<HostReleaseManifestV1> {
  ...
  if (manifest.schema_version !== "ty-context-host-release-v1" || manifest.protocol !== "ty-context-host-rpc-v1" || canonicalHostReleaseManifestV1(manifest) !== manifestText) throw new Error("host_release_manifest_invalid");
  ...
  const helper = process.platform === "win32" ? "ty-context-host-helper.exe" : "ty-context-host-helper";
  const admin = process.platform === "win32" ? "ty-context-host-admin.exe" : "ty-context-host-admin";
  const installer = process.platform === "win32" ? "ty-context-host-installer-ui.exe" : "ty-context-host-installer-ui";
  for (const required of ["requirements.toml", "long-task-hook.mjs", "ty-context-host-worker.mjs", helper, admin, installer]) if (!manifestPaths.includes(required)) throw new Error(`host_release_asset_missing:${required}`);
  const componentHashes = [helper, admin, installer].map((name) => manifest.files.find((item) => item.path === name)?.sha256);
  if (new Set(componentHashes).size !== componentHashes.length) throw new Error("host_release_component_identity_conflict");
