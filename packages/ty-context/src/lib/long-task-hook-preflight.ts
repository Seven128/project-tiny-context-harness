...
export async function checkLongTaskCompletionGate(
  ...
  try {
    ...
    const supported = fileURLToPath(
      new URL("../../assets/hooks/long-task-hook.mjs", import.meta.url),
    );
    const supportedBytes = await readFile(supported);
    if (
      sha256Hex(normalizeManagedText(scriptBytes)) !==
      sha256Hex(normalizeManagedText(supportedBytes))
    )
      findings.push("completion_hook_script_unmanaged");
  } catch {
    findings.push("completion_hook_script_missing");
  }
  try {
    ...
    if (containsContinueFalse(value.hooks?.Stop))
      findings.push("conflicting_stop_hook_continue_false");
  } catch {
...
export async function assertLongTaskCompletionGate(
  repositoryRoot: string,
): Promise<CompletionGateCheck> {
...
function containsContinueFalse(value: unknown): boolean {
  ...
  if (value && typeof value === "object") {
    return Object.entries(value as Record<string, unknown>).some(
      ([key, item]) =>
        (key === "continue" && item === false) || containsContinueFalse(item),
    );
  }
  return false;
}
