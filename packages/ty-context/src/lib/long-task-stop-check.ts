...
export async function stopCheckLongTask(workdir: string, lastAssistantMessage: string,options:LongTaskFinalGateOptionsV3={}): Promise<StopCheckResult> {
  const contract = (await readAuthoritativeLongTaskContract(workdir)).contract; let result: FinalResultV2 | undefined; try { result = JSON.parse(await readFile(path.join(workdir, "final-result.json"), "utf8")) as FinalResultV2; } catch { return block("Final gate has not produced a trusted result. Run final-gate and continue implementation."); }
