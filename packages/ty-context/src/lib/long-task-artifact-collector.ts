...
function matchesAny(value: string, globs: string[]): boolean { return globs.some((glob) => globExpression(glob).test(value)); }
function globExpression(glob:string):RegExp{let source="^";for(let index=0;index<glob.length;index+=1){const value=glob[index];if(value==="*"){if(glob[index+1]==="*"){index+=1;if(glob[index+1]==="/"){index+=1;source+="(?:.*/)?";}else source+=".*";}else source+="[^/]*";continue;}source+=/[.+^${}()|[\]\\]/u.test(value)?`\\${value}`:value;}return new RegExp(`${source}$`,"i");}
