...
export async function detectLegacyManagedDirectory(
  ...
  for (const file of files) {
    ...
    const classification =
      await classifyLegacyManagedFile(relativeToLegacyRoot);
    ...
    const target = path.join(
      ...
      ...classification.targetRelative.split("/"),
    );
...
async function classifyLegacyManagedFile(
  relative: string,
): Promise<
...
function legacyGeneratedMapping(
  relative: string,
): { targetRelative: string; assetSegments: string[] } | undefined {
