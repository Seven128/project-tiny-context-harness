...
function tagsFor(relative: string, content: string, language: string, routes: string[]): string[] {
  ...
  if (/schema|contract|types?\.|\.d\.ts$/.test(lower) || /\b(interface|type|schema)\b/.test(content)) tags.add("contract");
  if (/pages|views|screens|components|\.vue$|\.tsx$|\.jsx$/.test(lower)) tags.add("ui");
  if (/test|spec|verification|makefile/.test(lower)) tags.add("test");
  if (["json", "yaml", "toml", "make", "dockerfile"].includes(language)) tags.add("config");
  if (lines > OVERSIZED_LINES || content.length > OVERSIZED_CHARACTERS) tags.add("oversized");
  return [...tags].sort();
}

function scoreFor(tags: string[], relative: string): number {
  const weights: Record<string, number> = { entry: 50, api: 35, cli: 32, worker: 30, contract: 28, ui: 24, test: 18, config: 12, oversized: -15 };
...
function extractSymbolSummary(content: string, language: string): string[] {
  ...
  for (const pattern of patterns) {
    ...
    while ((match = pattern.exec(content)) && symbols.size < 10) symbols.add(match[1]);
  }
...
function describeFilePurpose(relative: string, language: string): string {
  const lower = relative.toLowerCase();
  if (lower.includes("/test") || lower.includes(".test.") || lower.includes(".spec.")) return `Contains ${language} tests for ${path.posix.basename(relative)}`;
  if (lower.includes("/commands/")) return `Implements ${language} command handling for ${path.posix.basename(relative)}`;
  if (lower.includes("/cli") || lower.endsWith("/cli.ts") || lower.endsWith("/cli.js")) return `Implements ${language} CLI behavior for ${path.posix.basename(relative)}`;
  if (lower.includes("/components/") || lower.includes("/pages/") || lower.includes("/views/")) return `Implements ${language} UI behavior for ${path.posix.basename(relative)}`;
...
function languageFor(relative: string): string {
  ...
  for (const [extension, language] of [[".tsx", "tsx"], [".ts", "typescript"], [".jsx", "jsx"], [".js", "javascript"], [".mjs", "javascript"], [".py", "python"], [".go", "go"], [".vue", "vue"], [".sql", "sql"], [".json", "json"], [".jsonc", "jsonc"], [".yaml", "yaml"], [".yml", "yaml"], [".toml", "toml"], [".sh", "bash"], [".ps1", "powershell"], [".cmd", "batch"], [".bat", "batch"], [".graphql", "graphql"], [".gql", "graphql"], [".proto", "protobuf"]] as const) {
