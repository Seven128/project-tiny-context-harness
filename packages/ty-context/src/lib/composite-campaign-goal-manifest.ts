...
export async function createSliceGoalManifest(
  ...
  void input;
  throw new Error(
    "Campaign V4 is audit-only; automatic Goal creation requires Slice Goal Manifest V2",
  );
}
...
export async function createSliceGoalManifestV2(
  ...
  const identity = {
    schema_version: "slice-goal-manifest-v2" as const,
    ...input,
  ...
  const manifest: SliceGoalManifestV2 = {
    ...identity,
    manifest_sha256: sha256Hex(canonicalJson(identity)),
  };
  const objective = renderSliceGoalObjectiveV2(manifest);
...
export async function readSliceGoalManifest(
  file: string,
): Promise<SliceGoalManifestV1 | SliceGoalManifestV2> {
  const parsed = JSON.parse(await readFile(file, "utf8"));
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed))
    throw new Error("Invalid Slice Goal manifest schema");
  const value = parsed as unknown as SliceGoalManifestV1 | SliceGoalManifestV2;
  if (
...
export function renderSliceGoalObjective(
  ...
  void manifest;
  throw new Error(
    "Campaign V4 is audit-only; automatic Goal rendering requires Slice Goal Manifest V2",
  );
}
...
function validateManifest(
  value: SliceGoalManifestV1 | SliceGoalManifestV2,
): void {
  const base = [
    "schema_version",
    "campaign_id",
  ...
  const expected =
    value.schema_version === "slice-goal-manifest-v2" ? [...base, ...v2] : base;
  validateExactManifestFields(value, expected);
  validateCommonManifest(value);
  if (value.schema_version === "slice-goal-manifest-v2")
    validateV2Manifest(value);
}

function validateExactManifestFields(
  value: SliceGoalManifestV1 | SliceGoalManifestV2,
  expected: string[],
): void {
...
function validateCommonManifest(
  value: SliceGoalManifestV1 | SliceGoalManifestV2,
): void {
  for (const key of [
    "campaign_id",
...
function validateV2Manifest(value: SliceGoalManifestV2): void {
