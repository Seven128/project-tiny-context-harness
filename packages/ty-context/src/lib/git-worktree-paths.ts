...
export async function resolveGitWorktreePaths(directory: string): Promise<GitWorktreePaths> {
  ...
  if (lines.length !== 3) throw new Error("git_worktree_paths_unavailable:unexpected_rev_parse_output");
...
export async function resolveGitWorktreePath(directory: string, gitPath: string): Promise<string> {
  if (!gitPath || path.isAbsolute(gitPath) || gitPath.split(/[\\/]/u).includes("..")) {
    throw new Error("git_worktree_path_invalid");
  }
  return normalizeAbsolutePath(await gitRevParse(path.resolve(directory), [
    "--path-format=absolute",
    "--git-path",
    gitPath
  ]));
...
export async function ensureLongTaskRuntimeExcludes(directory: string): Promise<void> {
  const excludeFile = await resolveGitWorktreePath(directory, "info/exclude");
  await mkdir(path.dirname(excludeFile), { recursive: true });
  let current = "";
  try { current = await readFile(excludeFile, "utf8"); }
  catch (error) {
    if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
  }
  const existing = new Set(current.split(/\r?\n/u).map((line) => line.trim()));
  const missing = LONG_TASK_RUNTIME_EXCLUDES.filter((entry) => !existing.has(entry));
...
async function gitRevParse(cwd: string, args: string[]): Promise<string> {
  try {
    const result = await execFileAsync("git", ["-C", cwd, "rev-parse", ...args], {
      encoding: "utf8",
      windowsHide: true,
