...
} from "./composite-campaign-graph.js";
import {
...
interface RepairStateV1 {
  schema_version: "campaign-repair-state-v1";
  repair_id: string;
  wave_id: string;
  kind:
...
export async function advanceCampaignV4(
  ...
  if (graph.graph_sha256 !== campaign.graph.graph_sha256)
    throw new Error("Immutable Scope Fit graph hash mismatch");
  ...
  if (
    ...
    return { action: "decision_required", decision: scope.decision_required };
  if (!campaign.base_commit) {
    ...
    campaign = await mutateCampaignV4(
      ...
      "git_baseline_ready",
      async (_root, value) => {
        value.base_commit = baseline.baseCommit;
        value.integration_head = integration.headCommit;
        value.campaign_status = "authoring";
        return value;
      },
    );
  }
  const integration = await createIntegrationWorktree({
    repositoryRoot: projectRoot,
    campaignId: campaign.campaign_id,
    baseCommit: campaign.base_commit!,
    branchName: campaign.integration_branch,
  });
  const repair = await resumeRepairIfReady(
    projectRoot,
    root,
    campaign,
    integration.path,
  ...
  if (activeWave) {
    ...
    const waiting = wave.slice_ids.filter(
      (sliceId) => campaign.slices[sliceId].status === "goal_running",
    );
    if (waiting.length)
      return {
        action: "wait_goals",
        goal_ids: waiting
          .map((sliceId) => campaign.slices[sliceId].goal_id!)
          .sort(asciiCompare),
      };
    if (
      wave.slice_ids.every(
        (sliceId) =>
          campaign.slices[sliceId].status === "accepted" ||
          campaign.slices[sliceId].status === "merged",
      )
    ) {
      const merged = await mergeAcceptedWave(
        projectRoot,
        root,
        campaign,
        integration.path,
      ...
      await mutateCampaignV4(
        ...
        async (_root, value) => {
          const current = value.waves[waveId];
          current.status = "integration_verified";
          current.integration_result_sha256 = integrationResult.result_sha256;
          value.integration_head = integrationResult.integration_head;
          for (const sliceId of current.slice_ids)
            value.slices[sliceId].status = "integration_verified";
          value.campaign_status = "authoring";
          return value;
        },
      );
  ...
  const integrated = Object.entries(campaign.slices)
    ...
    .map(([sliceId]) => sliceId);
  if (
    integrated.length === Object.keys(campaign.slices).length &&
    integrated.length
  )
    return finalizeCompletedGraph(
      projectRoot,
      root,
      campaign,
      scope,
  ...
  const packetPending = frontier
    .filter(
      (slice) =>
        campaign.slices[slice.slice_id].status !== "packet_ready" ||
        !v5.slices[slice.slice_id].thread.thread_id ||
        v5.slices[slice.slice_id].thread.phase !== "packet_validation",
    )
    .map((slice) => slice.slice_id);
  if (packetPending.length)
    return { action: "author_packets", slice_ids: packetPending };
  ...
  if (stalePackets.length > 0) {
    await mutateCampaignV4(
      ...
      async (_root, value) => {
        const current = assertCampaignV5(value);
        for (const sliceId of stalePackets) {
          value.slices[sliceId].status = "packet_pending";
          current.slices[sliceId].thread = transitionThreadPhaseV5(
            current.slices[sliceId].thread,
            "authoring",
          );
        }
        value.campaign_status = "authoring";
    ...
    return {
      action: "author_packets",
      slice_ids: stalePackets.sort(asciiCompare),
    };
  }
  const profiles = await Promise.all(
    frontier.map((slice) => readProfile(root, campaign, slice.slice_id)),
  );
  const schedule = selectDeterministicWaveV4(profiles, { max_concurrency: 4 });
  if (!schedule.slice_ids.length)
  ...
  await mutateCampaignV4(
    ...
    async (_root, value) => {
      ...
      for (const launch of launches) {
        ...
        slice.branch = manifest.branch;
        slice.worktree = manifest.worktree;
        slice.base_commit = base;
        slice.status = "worktree_ready";
        const current = assertCampaignV5(value);
        current.slices[launch.slice_id].thread = markWorktreeReadyV5(
          current.slices[launch.slice_id].thread,
        );
      }
...
export async function bindCampaignGoalV4(
  ...
  return mutateCampaignV4(
    ...
    async (root, campaign) => {
      ...
      if (manifest.launch_token !== launchToken)
        throw new Error("Goal launch token mismatch");
      ...
      if (
        ...
        goalId !== manifest.thread_id
      )
        throw new Error(
          "Goal must bind to the persisted SFC App Server thread",
        );
      const objective = await readFile(
        path.join(manifest.contract_workdir, "goal-objective.txt"),
        "utf8",
      );
      current.slices[sliceId].thread = bindThreadGoalV5(
        thread,
        sha256Hex(objective),
        launchToken,
      );
      slice.goal_id = goalId;
      slice.status = "goal_running";
      campaign.waves[slice.wave_id].status = "running";
...
export async function bindCampaignRepairGoalV4(
  ...
  if (state.launch_token !== launchToken)
    throw new Error("Repair Goal launch identity mismatch");
  if (state.goal_id && state.goal_id !== goalId)
    throw new Error("Repair is already bound to a different Goal");
...
export async function recordCampaignResultV4(
  ...
  const recorded = await recordSliceExecutionReceipt({
    campaignRoot: root,
    campaignId: campaign.campaign_id,
    sliceId,
    ...
    contractWorkdir: workdir,
    branch: slice.branch,
    baseCommit: slice.base_commit,
    forbiddenChangedPaths: manifest.forbidden_campaign_state_paths,
  ...
  await mutateCampaignV4(
    ...
    async (_root, value) => {
      const current = value.slices[sliceId];
      current.status = "accepted";
      current.head_commit = recorded.receipt.head_commit;
      current.final_receipt_sha256 = recorded.receipt.receipt_sha256;
      const v5 = assertCampaignV5(value);
      v5.slices[sliceId].thread = acceptThreadV5(v5.slices[sliceId].thread);
      return value;
    },
  );
  return recorded;
...
export async function statusCampaignV4(
  ...
  return {
    campaign,
    campaign_path: root,
    derived_status: deriveCampaignStatus(campaign),
    next_action: await previewNextAction(root, campaign),
  };
}
...
async function mergeAcceptedWave(
  ...
  const result = await mergeWaveIntoIntegration({
    campaignRoot: root,
    campaignId: campaign.campaign_id,
    waveId,
  ...
  if (result.status === "repair_required")
    return {
      action: await prepareRepairAction(
        projectRoot,
        root,
        campaign,
        integration,
  ...
  await mutateCampaignV4(
    ...
    "wave_merged",
    async (_root, value) => {
      value.waves[waveId].status = "merged";
      value.integration_head = result.integration_head;
      for (const record of result.merges) {
        value.slices[record.slice_id].status = "merged";
        value.slices[record.slice_id].merge_commit = record.merge_commit;
      }
      value.campaign_status = "integrating";
      return value;
    },
  );
  return {};
...
async function materializeSliceLaunch(
  ...
  if (
    !thread?.thread_id ||
    !thread.authoring_profile ||
    !thread.execution_profile ||
    !thread.routing_reason ||
    !thread.routing_freeze ||
    thread.phase !== "packet_validation"
  )
    throw new Error(`campaign_thread_not_ready_for_worktree:${sliceId}`);
  const worktree = await createSliceWorktree({
    repositoryRoot: projectRoot,
    campaignId: campaign.campaign_id,
    sliceId,
    baseCommit: base,
  ...
  const task = path.join(
    ...
    `${sliceId}-r${campaign.slices[sliceId].packet_revision}`,
  );
  await rm(task, { recursive: true, force: true });
  await mkdir(task, { recursive: true });
  for (const file of Object.values(LONG_TASK_SOURCE_FILES))
    await cp(path.join(revision, file), path.join(task, file));
  ...
  const created = await createSliceGoalManifestV2(root, {
    ...
    base_commit: base,
    packet_revision: campaign.slices[sliceId].packet_revision!,
    packet_sha256: campaign.slices[sliceId].packet_sha256!,
    contract_workdir: task,
    integration_branch: campaign.integration_branch,
    allowed_implementation_bindings: [
      ...profile.write_paths,
      ...profile.contract_keys,
    ],
    ...
    authoring_model: thread.authoring_profile.model,
    authoring_effort: thread.authoring_profile.effort,
    execution_model: thread.execution_profile.model,
    execution_effort: thread.execution_profile.effort,
    routing_reason: thread.routing_reason,
    routing_policy_id: thread.routing_freeze.policy_id,
    routing_switched: thread.routing_freeze.routing_decision.switched,
    routing_policy_sha256: thread.routing_freeze.policy_sha256,
    routing_catalog_sha256: thread.routing_freeze.catalog_sha256,
    routing_decision_sha256: thread.routing_freeze.decision_sha256,
  });
  return {
    slice_id: sliceId,
    thread_id: thread.thread_id,
    worktree: worktree.path,
    objective_path: created.objective_path,
    launch_token: created.manifest.launch_token,
  };
}
...
async function finalizeCompletedGraph(
  ...
  const constraints = scope.global_constraints.map(
    (constraint): CampaignGlobalConstraintBindingV1 => {
      ...
      return {
        ...
        requirement_ids: unique(rows.flatMap((item) => item.requirement_ids)),
        acceptance_criterion_ids: unique(
          rows.flatMap((item) => item.acceptance_criterion_ids),
        ),
        verification_spec_ids: unique(
          rows.flatMap((item) => item.verification_spec_ids),
        ),
      };
    },
  );
  ...
  const target = await finalizeCampaignTarget({
    repositoryRoot: projectRoot,
    campaignId: campaign.campaign_id,
    campaignRoot: root,
    integrationWorktree: integration,
  ...
  if (target.status === "revalidation_required") {
    await mutateCampaignV4(
      ...
      async (_root, value) => {
        value.integration_head = target.integration_head;
        value.campaign_status = "finalizing";
        return value;
      },
    );
    return advanceCampaignV4(projectRoot, root);
...
async function prepareRepairAction(
  ...
  const worktree = await createRepairWorktree({
    ...
    campaignId: campaign.campaign_id,
    repairId,
    baseCommit: await currentHead(integration),
  });
  if (
    replayBranch &&
    (await gitStatus(worktree.path)).clean &&
    (await currentHead(worktree.path)) === worktree.baseCommit
  )
    await runGit(
      worktree.path,
      [
  ...
  const objective = path.join(
    root,
    "repairs",
    repairId,
    "repair-objective.txt",
  );
  await mkdir(path.dirname(objective), { recursive: true });
  await writeFile(
    objective,
    `Campaign V5 repair Goal\nCampaign: ${campaign.campaign_id}\nRepair: ${repairId}\nWave: ${waveId}\nKind: ${kind}\nAffected Slices: ${affectedSliceIds.join(", ")}\nAllowed write paths: ${changeEnvelope.allowed_write_paths.join(", ") || "none"}\nAllowed supporting paths: ${changeEnvelope.allowed_supporting_paths.join(", ") || "none"}\nForbidden paths: ${changeEnvelope.forbidden_paths.join(", ") || "none"}\nUndeclared change policy: ${changeEnvelope.undeclared_change_policy}\nWorktree: ${worktree.path}\nBranch: ${worktree.branch}\nBase: ${worktree.baseCommit}\nManifest/result: ${manifest}\nPreserve every affected Packet, requirement and AC. Resolve the conflict or regression inside this union Change Envelope, verify, commit, and leave the worktree clean. Do not weaken contracts or modify Scope Fit, Packets, or Campaign state.\n`,
  );
  const launchToken = sha256Hex(
    canonicalJson({
      campaign_id: campaign.campaign_id,
      repair_id: repairId,
  ...
  const state: RepairStateV1 = {
    schema_version: "campaign-repair-state-v1",
    repair_id: repairId,
    wave_id: waveId,
    kind,
    affected_slice_ids: affectedSliceIds,
    change_envelope: changeEnvelope,
    worktree: worktree.path,
    branch: worktree.branch,
    base_commit: worktree.baseCommit,
  ...
  if (campaign.waves[waveId])
    await mutateCampaignV4(
      ...
      "repair_required",
      async (_root, value) => {
        value.waves[waveId].status = "repair_required";
        value.campaign_status = "integrating";
        return value;
      },
    );
...
async function resumeRepairIfReady(
  ...
  const candidates = [
    ...Object.entries(campaign.waves)
      ...
      .map(([id]) => `${id}-merge_conflict`),
    ...Object.entries(campaign.waves)
      .filter(([, wave]) => wave.status === "repair_required")
      .map(([id]) => `${id}-integration_regression`),
    "CAMPAIGN-FINAL-campaign_final_regression",
  ];
  for (const repairId of candidates) {
    const file = path.join(root, "repairs", repairId, "repair-state.json");
    let state: RepairStateV1;
    try {
      state = assertRepairState(
        parseStrictJson(await readFile(file, "utf8")),
        root,
        repairId,
      );
    ...
    const action: CampaignAdvanceActionV4 = {
      ...
      objective_path: state.objective_path,
      launch_token: state.launch_token,
      goal_id: state.goal_id,
      reason: state.kind,
    };
    if (!state.goal_id) return { action };
    const status = await gitStatus(state.worktree);
    const head = await currentHead(state.worktree);
    if (!status.clean || head === state.base_commit) return { action };
    const changed = splitLines(
      (
        await runGit(state.worktree, [
          "diff",
          "--name-only",
          "--no-renames",
          state.base_commit,
    ...
    assertChangedPathsWithinEnvelopeV1(changed, state.change_envelope);
    if ((await currentHead(integration)) !== state.base_commit)
      throw new Error("Integration head changed while repair was running");
    ...
    await mutateCampaignV4(
      ...
      "repair_applied",
      async (_root, value) => {
        value.integration_head = await currentHead(integration);
        if (value.waves[state.wave_id])
          value.waves[state.wave_id].status = "accepted";
        return value;
      },
    );
    return { applied: true };
...
async function affectedSlices(
  ...
  const waveEntry = Object.entries(campaign.waves).find(
    ([, wave]) =>
      ...
      wave.slice_ids.every((id) => waveSliceIds.includes(id)),
  );
  const waveId = waveEntry?.[0];
  if (!integration || !scope || !coverage || !waveEntry) {
    if (waveId) {
      try {
        const prior = parseStrictJson(
          await readFile(
  ...
  const profileEntries = await Promise.all(
    candidateIds.map(
      async (id) => [id, await readProfile(root, campaign, id)] as const,
    ),
  );
  ...
  const changed = splitLines(
    (
      await runGit(integration, [
        ...
        "--name-only",
        "--no-renames",
        waveEntry[1].base_commit,
        await currentHead(integration),
      ])
    ).stdout,
  );
...
async function launchAction(
  ...
  return goals.length
    ? { action: "launch_wave", wave_id: waveId, goals }
    : null;
}
...
function deriveCampaignStatus(campaign: CampaignV4): string {
  ...
  if (
    Object.values(campaign.slices).every(
      (slice) => slice.status === "integration_verified",
    )
  )
    return "finalizing";
  if (
    Object.values(campaign.slices).some(
      (slice) => slice.status === "goal_running",
    )
  )
    return "executing";
...
async function previewNextAction(
  ...
  return await readFile(path.join(root, "scope-fit.json"), "utf8").then(
    () => "advance",
    () => "apply_scope",
  );
}
...
function assertRepairState(
  ...
  const state = value as RepairStateV1;
  assertPortableId(repairId, "repair");
  if (
    state.schema_version !== "campaign-repair-state-v1" ||
    state.repair_id !== repairId ||
    !/(?:^WAVE-[0-9]{3}$|^CAMPAIGN-FINAL$)/u.test(state.wave_id) ||
    ![
      "merge_conflict",
      "integration_regression",
      "campaign_final_regression",
    ].includes(state.kind) ||
    !/^[a-f0-9]{40,64}$/u.test(state.base_commit) ||
    !/^[a-f0-9]{32}$/u.test(state.launch_token) ||
    typeof state.applied !== "boolean" ||
    (state.goal_id !== null && typeof state.goal_id !== "string")
  )
    throw new Error("Campaign repair state is invalid");
  if (
    !Array.isArray(state.affected_slice_ids) ||
    state.affected_slice_ids.length === 0 ||
    state.affected_slice_ids.some((id) => !/^SFC-[0-9]{3,}$/u.test(id)) ||
    new Set(state.affected_slice_ids).size !== state.affected_slice_ids.length
  )
    throw new Error("Campaign repair affected Slice set is invalid");
  validateChangeEnvelopeV1(state.change_envelope);
  const expected = path.join(root, "repairs", repairId, "repair-objective.txt");
  if (
    path.resolve(state.objective_path) !== path.resolve(expected) ||
    typeof state.worktree !== "string" ||
    typeof state.branch !== "string" ||
    (state.replay_branch !== null && typeof state.replay_branch !== "string")
  )
    throw new Error("Campaign repair state path or branch is invalid");
  return state;
}
async function assertOwnedRepairWorktree(
  projectRoot: string,
  campaignId: string,
  state: RepairStateV1,
): Promise<void> {
  const registered = (
    await listCampaignWorktrees({ repositoryRoot: projectRoot, campaignId })
  ).find((item) => samePath(item.path, state.worktree));
  if (!registered || registered.branch !== state.branch)
    throw new Error("Campaign repair worktree is not registered and owned");
}
