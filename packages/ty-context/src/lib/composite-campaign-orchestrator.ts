...
import { createSliceGoalManifest, readSliceGoalManifest } from "./composite-campaign-goal-manifest.js";
...
import { parseScopeFitResultV3, type CampaignV4, type CampaignWaveV4, type ScopeFitResultV3 } from "./composite-campaign-schema-v4.js";
import { assertGlobalConstraintPacketCoverageV1, parseSourceCoverageV1, validateSourceCoverageAgainstScopeV3, type SourceCoverageV1 } from "./composite-campaign-source-coverage.js";
import { currentPacketRevisionPathV4, loadCampaignV4, mutateCampaignV4, verifyCampaignPacketV4 } from "./composite-campaign-v4.js";
import { computeReadyFrontierV3, validateScopeFitGraphV3 } from "./composite-campaign-graph.js";
...
export interface GoalLaunchV4 { slice_id: string; worktree: string; objective_path: string; launch_token: string }
...
export async function advanceCampaignV4(projectRoot: string, campaignPath: string): Promise<CampaignAdvanceActionV4> {
  ...
  const frontier = computeReadyFrontierV3(scope, integrated);
  const packetPending = frontier.filter((slice) => campaign.slices[slice.slice_id].status !== "packet_ready").map((slice) => slice.slice_id);
  ...
  await mutateCampaignV4(projectRoot, root, "wave_scheduled", async (_root, value) => {
    ...
    for (const launch of launches) { const slice = value.slices[launch.slice_id]; const manifest = await readSliceGoalManifest(path.join(root, "waves", waveId, "goals", launch.slice_id, "goal-manifest.json")); slice.wave_id = waveId; slice.branch = manifest.branch; slice.worktree = manifest.worktree; slice.base_commit = base; slice.status = "worktree_ready"; }
...
export async function bindCampaignGoalV4(projectRoot: string, campaignPath: string, sliceId: string, goalId: string, launchToken: string): Promise<CampaignV4> {
  return mutateCampaignV4(projectRoot, campaignPath, "goal_bound", async (root, campaign) => {
    ...
    const manifest = await readSliceGoalManifest(path.join(root, "waves", slice.wave_id, "goals", sliceId, "goal-manifest.json"));
...
export async function recordCampaignResultV4(projectRoot: string, campaignPath: string, sliceId: string, goalId: string, workdir: string): Promise<unknown> {
  ...
  await mutateCampaignV4(projectRoot, root, "slice_result_recorded", async (_root, value) => { const current = value.slices[sliceId]; current.status = "accepted"; current.head_commit = recorded.receipt.head_commit; current.final_receipt_sha256 = recorded.receipt.receipt_sha256; return value; });
  return recorded;
}
...
async function materializeSliceLaunch(projectRoot:string,root:string,campaign:CampaignV4,_integration:string,waveId:string,base:string,sliceId:string):Promise<GoalLaunchV4>{
  const worktree=await createSliceWorktree({repositoryRoot:projectRoot,campaignId:campaign.campaign_id,sliceId,baseCommit:base});const revision=currentPacketRevisionPathV4(root,campaign,sliceId);const task=path.join(worktree.path,"tmp","ty-context","plan-acceptance",campaign.campaign_id,`${sliceId}-r${campaign.slices[sliceId].packet_revision}`);await rm(task,{recursive:true,force:true});await mkdir(task,{recursive:true});for(const file of Object.values(LONG_TASK_SOURCE_FILES))await cp(path.join(revision,file),path.join(task,file));const profile=await readProfile(root,campaign,sliceId);const created=await createSliceGoalManifest(root,{campaign_id:campaign.campaign_id,slice_id:sliceId,wave_id:waveId,worktree:worktree.path,branch:worktree.branch,base_commit:base,packet_revision:campaign.slices[sliceId].packet_revision!,packet_sha256:campaign.slices[sliceId].packet_sha256!,contract_workdir:task,integration_branch:campaign.integration_branch,allowed_implementation_bindings:[...profile.write_paths,...profile.contract_keys],forbidden_campaign_state_paths:[path.join(".codex","composite-long-task","campaigns",campaign.campaign_id)]});return{slice_id:sliceId,worktree:worktree.path,objective_path:created.objective_path,launch_token:created.manifest.launch_token};
}

async function finalizeCompletedGraph(projectRoot:string,root:string,campaign:CampaignV4,scope:ScopeFitResultV3,coverage:SourceCoverageV1,integration:string):Promise<CampaignAdvanceActionV4>{
  const analysis=validateSourceCoverageAgainstScopeV3(scope,coverage);const verified=await Promise.all(Object.keys(campaign.slices).sort(asciiCompare).map((sliceId)=>verifyCampaignPacketV4(projectRoot,root,sliceId)));const indexes=verified.map((result)=>result.packet_index);assertGlobalConstraintPacketCoverageV1(scope,coverage,indexes);
...
async function launchAction(root:string,campaign:CampaignV4,waveId:string,wave:CampaignWaveV4):Promise<CampaignAdvanceActionV4|null>{const goals:GoalLaunchV4[]=[];for(const sliceId of wave.slice_ids){const slice=campaign.slices[sliceId];if(slice.status!=="worktree_ready")continue;const manifest=await readSliceGoalManifest(path.join(root,"waves",waveId,"goals",sliceId,"goal-manifest.json"));goals.push({slice_id:sliceId,worktree:manifest.worktree,objective_path:path.join(manifest.contract_workdir,"goal-objective.txt"),launch_token:manifest.launch_token});}return goals.length?{action:"launch_wave",wave_id:waveId,goals}:null;}
...
async function readScope(root:string):Promise<ScopeFitResultV3>{return parseScopeFitResultV3(await readFile(path.join(root,"scope-fit.json"),"utf8"));}
