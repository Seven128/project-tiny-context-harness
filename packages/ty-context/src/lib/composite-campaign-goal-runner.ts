...
export async function launchCampaignWaveV5(
  input: GoalRunnerInput,
  action: LaunchWaveAction,
): Promise<{ wave_id: string; turn_ids: string[] }> {
  const queue = new CampaignMutationQueue();
  const settled = await Promise.allSettled(
    action.goals.map((goal) =>
      launchSlice(input, action.wave_id, goal.slice_id, queue),
    ),
  );
  const started = fulfilledOrThrow(
    settled,
    "campaign_wave_launch_failed_after_all_goals_settled",
  );
  return { wave_id: action.wave_id, turn_ids: started };
}

export async function waitCampaignGoalsV5(
  input: GoalRunnerInput,
): Promise<{ accepted_slice_ids: string[] }> {
  const campaign = (await loadCampaignV5(input.projectRoot, input.campaignPath))
    .campaign;
  const sliceIds = Object.entries(campaign.slices)
    .filter(([, slice]) => slice.status === "goal_running")
    .map(([sliceId]) => sliceId)
    .sort(ascii);
  const queue = new CampaignMutationQueue();
  const settled = await Promise.allSettled(
    sliceIds.map((sliceId) => runSliceUntilAccepted(input, sliceId, queue)),
  );
  fulfilledOrThrow(
    settled,
    "campaign_goal_wait_failed_after_all_goals_settled",
  );
  return { accepted_slice_ids: sliceIds };
}

async function launchSlice(
  input: GoalRunnerInput,
  waveId: string,
  sliceId: string,
  ...
  if (
    ...
    slice.thread.phase !== "worktree_ready" ||
  ...
  if (existing && existing.objective !== objective)
    throw new Error("app_server_goal_objective_mismatch");
  if (!existing)
    await input.client.setGoal({
...
async function runSliceUntilAccepted(
  input: GoalRunnerInput,
  sliceId: string,
  queue: CampaignMutationQueue,
): Promise<void> {
  for (let attempt = 0; attempt < 50; attempt += 1) {
    ...
    let turnId = slice.thread.active_turn_id;
    if (!turnId)
      turnId = await startExecutionTurn(
        input,
        sliceId,
        attempt === 0
          ? await readFile(
          ...
          : "Resume this Goal after host recovery. Re-read the Goal and current worktree, then continue the required verify/repair/commit/final-gate loop.",
        queue,
      );
    const completion = await currentOrWait(
      input.client,
      manifest.thread_id,
      turnId,
    );
    ...
    await queue.run(() =>
      updateSliceThreadV5(
        ...
        input.campaignPath,
        sliceId,
        "execution_turn_completed",
        (state) => completeThreadTurnV5(state, outcome),
      ),
    );
    if (completion.status === "interrupted") {
      await input.client.resumeThread(manifest.thread_id);
      await startExecutionTurn(
        input,
        sliceId,
        "The prior Execution Turn was interrupted. Resume the same Goal from the current worktree without resetting or duplicating work. Re-run required verification and finish with a fresh final-gate.",
        queue,
      );
      continue;
    }
    ...
    if (result.workflow_status === "accepted") {
      try {
        ...
        await startExecutionTurn(
          input,
          sliceId,
          continuationPrompt(sliceId, {
            workflow_status: "needs_work",
            findings: [
              {
                category: "receipt_recording_failed",
                next_action: errorText(error),
              },
            ],
          }),
          queue,
    ...
    await startExecutionTurn(
      input,
      sliceId,
      continuationPrompt(sliceId, result),
      queue,
    );
  }
...
async function startExecutionTurn(
  ...
  if (
    ...
    throw new Error(`execution_thread_not_ready:${sliceId}`);
  const profile = slice.thread.execution_profile;
  const explicit = profile.model !== "unknown" && profile.effort !== "unknown";
  const turn = await input.client.startTurn({
    threadId: slice.thread.thread_id,
    input: message,
    cwd: slice.worktree,
    ...(explicit ? { model: profile.model, effort: profile.effort } : {}),
    sandboxPolicy: {
      type: "workspaceWrite",
      writableRoots: [slice.worktree],
      networkAccess: false,
  ...
  await queue.run(() =>
    updateSliceThreadV5(
      ...
      (state) => recordExecutionTurnV5(state, turn.id),
    ),
  );
  return turn.id;
}
...
async function currentOrWait(
  ...
  const current = thread.turns.find((turn) => turn.id === turnId);
  if (!current)
    throw new Error(`active_execution_turn_missing:${threadId}:${turnId}`);
  if (current.status === "inProgress")
    return client.waitForTurn(threadId, turnId);
  return {
    threadId,
    turn: current,
    status: current.status,
    outputText: finalText(current),
  };
}
  ...
  for (const file of ["final-result.json", "current-status.json"]) {
    try {
      const row = parseStrictJson(
        await readFile(path.join(workdir, file), "utf8"),
...
function continuationPrompt(
  sliceId: string,
  result: { workflow_status: string; findings: unknown[] },
): string {
