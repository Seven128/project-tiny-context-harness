export type LongTaskWorkflowStatus = "needs_work" | "accepted";
export interface LongTaskFindingV2 {
  category: string;
  requirement_id?: string;
  obligation_id?: string;
  ac_id?: string;
  verification_spec_id?: string;
  expected: unknown;
  actual: unknown;
  evidence_path: string;
  forbidden_shortcut?: string;
  next_action: string;
  reverify_command: string;
}
export interface SnapshotFileV2 {
  path: string;
  type: "file";
  mode: number;
  size: number;
  sha256: string;
}
export interface SnapshotManifestV2 {
  repository_root: string;
  git_head: string | null;
  submodules: Record<string, string>;
  snapshot_sha256: string;
  files: SnapshotFileV2[];
}
export interface EnvironmentManifestV2 {
  node: string;
  platform: NodeJS.Platform;
  arch: string;
  release: string;
  executable: string;
  executable_sha256: string;
  cwd: string;
  environment_keys: string[];
}
export interface SnapshotHandle {
  root: string;
  manifest: SnapshotManifestV2;
  dispose(): Promise<void>;
}
export interface CommandRunV2 {
  spec_id: string;
  executable: string;
  argv: string[];
  cwd: string;
  exit_code: number;
  stdout_path: string;
  stdout_sha256: string;
  stderr_path: string;
  stderr_sha256: string;
  started_at: string;
  completed_at: string;
}
export interface ArtifactManifestV2 {
  spec_id: string;
  artifacts: Array<{
    path: string;
    sha256: string;
    size: number;
    mtime: string;
  }>;
}
export interface VerificationSpecResultV2 {
  spec_id: string;
  status: "passed" | "failed" | "blocked";
  assertion_results: Record<string, boolean>;
  population_results: Record<
    string,
    import("./long-task-population-evaluator.js").PopulationEvaluationV3
  >;
  observations: Record<
    string,
    import("./long-task-observation-v2.js").ObservationV2Record
  >;
  findings: LongTaskFindingV2[];
}
export interface VerificationRunResultV2 {
  schema_version: "long-task-verification-run-v2";
  run_id: string;
  contract_sha256: string;
  verification_scope:
    "targeted_repair" | "impact_repair" | "full_repair" | "full_acceptance";
  acceptance_authorized: boolean;
  snapshot: SnapshotManifestV2;
  environment: EnvironmentManifestV2;
  spec_results: VerificationSpecResultV2[];
  findings: LongTaskFindingV2[];
  started_at: string;
  completed_at: string;
}
export interface CurrentStatusV3 {
  schema_version: "long-task-current-status-v3";
  workflow_status: "needs_work";
  acceptance_authorized: false;
  verification_scope: VerificationRunResultV2["verification_scope"];
  contract_sha256: string;
  latest_run_id: string;
  findings: LongTaskFindingV2[];
}
export type LongTaskEntityStatusV3 = "passed" | "failed" | "blocked";
export interface BindingResultV3 {
  binding_id: string;
  status: "passed" | "failed" | "unobservable";
  evidence_refs: string[];
  finding_codes: string[];
}
export interface CounterfactualMutationEffectV3 {
  path: string;
  before_sha256: string | null;
  after_sha256?: string;
  after?: "deleted";
}
export interface CounterfactualAssertionFlipV3 {
  assertion_id: string;
  real: boolean;
  counterfactual: boolean;
}
export interface CounterfactualResultV3 {
  control_id: string;
  obligation_id: string;
  status: LongTaskEntityStatusV3;
  mutation_effects: CounterfactualMutationEffectV3[];
  assertion_flips: CounterfactualAssertionFlipV3[];
  finding_codes: string[];
}
export interface LongTaskEntityResultV3 {
  status: LongTaskEntityStatusV3;
  upstream_ids: string[];
  evidence_refs: string[];
  finding_codes: string[];
}
export interface FinalResultV3 {
  schema_version: "long-task-final-result-v3";
  workflow_status: LongTaskWorkflowStatus;
  contract_sha256: string;
  run_id: string;
  final_snapshot_sha256: string;
  source_hashes: Record<string, string>;
  context_hashes: Record<string, string>;
  oracle_hashes: Record<string, string>;
  verifier_identity: unknown;
  requirement_results: Record<string, LongTaskEntityResultV3>;
  plan_item_results: Record<string, LongTaskEntityResultV3>;
  obligation_results: Record<string, LongTaskEntityResultV3>;
  acceptance_results: Record<string, LongTaskEntityResultV3>;
  proof_requirement_results: Record<string, LongTaskEntityResultV3>;
  binding_results: Record<string, BindingResultV3>;
  counterfactual_results: Record<string, CounterfactualResultV3>;
  spec_results: Record<string, "passed" | "failed" | "blocked">;
  workspace_hash_before: string;
  workspace_hash_after: string;
  findings: LongTaskFindingV2[];
  started_at: string;
  completed_at: string;
  atomic_write_complete: true;
}
