...
export class GitCommandError extends Error {
  ...
  constructor(result: GitCommandResult) {
    ...
    super(
      `git_${safeCommandName(result.args[0])}_${reason}${detail ? `:${detail}` : ""}`,
    );
...
export type GitSyncAction =
  "no_upstream" | "unchanged" | "fast_forward" | "local_ahead" | "diverged";
...
export async function runGit(
  ...
  const maxOutputBytes = boundedPositive(
  ...
  if (
    (result.exitCode !== 0 || result.timedOut || result.outputLimited) &&
...
export async function currentBranch(root: string): Promise<string> {
  ...
  if (result.exitCode !== 0 || !result.stdout.trim())
    throw new Error("git_detached_head_not_supported");
...
export async function gitStatus(root: string): Promise<GitStatus> {
  ...
  for (let index = 0; index < fields.length; index += 1) {
    ...
    if (entry.indexStatus === "R" || entry.indexStatus === "C")
      entry.originalPath = fields[++index] || undefined;
    entries.push(entry);
  }
...
export async function assertNoGitOperation(root: string): Promise<void> {
  ...
  if (active.length > 0)
    throw new Error(`git_operation_in_progress:${active.join(",")}`);
}

export async function assertNoApparentSecretsInDirtyFiles(
  root: string,
  status?: GitStatus,
): Promise<void> {
  ...
  for (const entry of currentStatus.entries) {
    ...
    if (!isInside(repositoryRoot, candidate))
      throw new Error("dirty_path_escapes_repository");
    const metadata = await lstat(candidate).catch(() => null);
    if (!metadata?.isFile()) continue;
    if (await fileContainsApparentSecret(candidate))
      flagged.push(entry.path.replace(/\\/gu, "/"));
  }
  if (flagged.length > 0)
    throw new Error(
      `dirty_checkpoint_contains_apparent_secret:${flagged.sort().join(",")}`,
    );
}

export async function prepareGitBaseline(
  options: GitBaselineOptions,
  ...
  const targetHead = await currentHead(repositoryRoot, targetBranch);
  const status = await gitStatus(repositoryRoot);
  const statusIdentity = JSON.stringify(status);
  ...
  if (!status.clean) {
    ...
    const checkpoint = await createHiddenCheckpoint(
      repositoryRoot,
      options.campaignId,
      targetHead,
      options.timeoutMs,
  ...
  let baseCommit = checkpointCommit ?? targetHead;
  if (upstream) {
    const remote = (
      await runGit(
        repositoryRoot,
    ...
    if (options.fetch !== false && remote && remote !== ".")
      await runGit(repositoryRoot, ["fetch", "--prune", remote], {
    ...
    if (!Number.isInteger(ahead) || !Number.isInteger(behind))
      throw new Error("git_upstream_counts_invalid");
    if (ahead === 0 && behind === 0) syncAction = "unchanged";
    else if (ahead === 0) {
      syncAction = "fast_forward";
      if (!checkpointCommit)
        baseCommit = await currentHead(repositoryRoot, upstream);
    } else if (behind === 0) syncAction = "local_ahead";
    else syncAction = "diverged";
  }
  if (
    (await currentBranch(repositoryRoot)) !== branch ||
    (await currentHead(repositoryRoot)) !== originalHead ||
    JSON.stringify(await gitStatus(repositoryRoot)) !== statusIdentity
  ) {
    throw new Error("git_primary_worktree_changed_during_baseline");
  }
  return {
    repositoryRoot,
    targetBranch,
...
async function createHiddenCheckpoint(
  root: string,
  campaignId: string,
  parent: string,
  timeoutMs?: number,
  ...
  const temporaryRoot = await mkdtemp(
    path.join(os.tmpdir(), "ty-context-index-"),
  );
  const temporaryIndex = path.join(temporaryRoot, "index");
  const env = { GIT_INDEX_FILE: temporaryIndex };
  try {
    await runGit(root, ["read-tree", parent], { env });
    await runGit(root, ["add", "-A"], { env, timeoutMs: timeoutMs ?? 120_000 });
    await runGit(
      root,
      [
        "reset",
    ...
    const label = safeCommitLabel(campaignId);
    const commit = (
      await runGit(
        root,
        [
          ...
          parent,
          "-m",
          `ty-context hidden checkpoint for campaign ${label}`,
        ],
        { env },
      )
    ).stdout.trim();
    const timestamp = new Date().toISOString().replace(/[-:.TZ]/gu, "");
    const ref = `refs/ty-context/checkpoints/${label}/${timestamp}-${commit.slice(0, 12)}`;
    await runGit(root, ["update-ref", ref, commit]);
    return { commit, ref };
  } finally {
...
async function resolveRepositoryRoot(root: string): Promise<string> {
  ...
  const result = await runGit(supplied, ["rev-parse", "--show-toplevel"], {
    throwOnError: false,
  });
  if (result.exitCode !== 0 || !result.stdout.trim())
    throw new Error("git_repository_required");
  const repositoryRoot = path.resolve(result.stdout.trim());
  if (!samePath(supplied, repositoryRoot))
    throw new Error(`git_repository_root_required:${repositoryRoot}`);
...
function containsApparentSecret(value: string): boolean {
  return /-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----|\bauthorization\s*[:=]\s*bearer\s+[A-Za-z0-9._~+/=-]{8,}|\b(?:api[_-]?key|access[_-]?token|refresh[_-]?token|secret|password)\s*[:=]\s*["']?[A-Za-z0-9_./+=-]{12,}/iu.test(
    value,
  );
}
...
function sanitizeGitOutput(value: string): string {
  return value
    ...
    .replace(
      /(\b(?:authorization|api[_-]?key|access[_-]?token|refresh[_-]?token|secret|password)\s*[:=]\s*)([^\s]+)/giu,
      "$1<redacted>",
    );
}
