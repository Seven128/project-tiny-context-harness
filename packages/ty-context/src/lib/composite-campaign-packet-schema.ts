...
export function compositeAuthoringPacketOutputSchemaV3(
  ...
  const technical = namespace(
    COMPOSITE_V3_SCHEMAS["technical-plan-v3"] as unknown as Record<
      string,
      unknown
    >,
    "technical",
  );
  const acceptance = namespace(
    COMPOSITE_V3_SCHEMAS["acceptance-checklist-v3"] as unknown as Record<
      string,
      unknown
    >,
    "acceptance",
  ...
  return {
    ...
    properties: {
      schema_version: { const: "composite-authoring-packet-v3" },
      campaign_id: { const: input.campaignId },
      slice_id: { const: input.sliceId },
      revision: { const: input.revision },
      previous_packet_sha256:
        input.previousPacketSha256 === null
          ? { type: "null" }
          : { const: input.previousPacketSha256 },
      authorities: {
        type: "object",
        additionalProperties: false,
        required: [
        ...
        properties: {
          product_architecture_source: product.schema,
          technical_realization_plan: technical.schema,
          acceptance_checklist: acceptance.schema,
        },
      },
      source_unit_bindings: {
        type: "array",
        minItems: input.sourceUnitIds.length,
        maxItems: input.sourceUnitIds.length,
        uniqueItems: true,
        items: {
          type: "object",
          additionalProperties: false,
          ...
          properties: {
            source_unit_id: { enum: input.sourceUnitIds },
            requirement_ids: idArray,
            obligation_ids: idArray,
            acceptance_criterion_ids: idArray,
...
function namespace(
  ...
  const root = Object.fromEntries(
    Object.entries(source).filter(
      ([key]) => !["$schema", "$id", "$defs", "title"].includes(key),
    ),
  );
...
function rewrite(value: unknown, prefix: string): PacketJsonValue {
  ...
  if (!value || typeof value !== "object")
    throw new Error("packet_output_schema_invalid:non_json_value");
  return Object.fromEntries(
    Object.entries(value as Record<string, unknown>).map(([key, item]) => [
      key,
      key === "$ref" && typeof item === "string" && item.startsWith("#/$defs/")
        ? `#/$defs/${prefix}_${item.slice(8)}`
        : rewrite(item, prefix),
    ]),
  );
}
