...
export type CampaignWorktreeKind =
  "integration" | "slice" | "repair" | "target";
...
export function portablePathSlug(value: string, maximumLength = 48): string {
  if (
    ...
    throw new Error("portable_slug_length_invalid");
  ...
  slug =
    slug.slice(0, maximumLength - reservedForSuffix).replace(/[._-]+$/gu, "") ||
    "item";
...
export function campaignWorktreesRoot(
  options: ListCampaignWorktreesOptions,
): string {
...
export async function createSliceWorktree(
  ...
  return createManagedWorktree(
    options,
    "slice",
    path.join("slices", sliceSlug),
    options.branchName ?? `tyctx/campaign/${campaignSlug}/${sliceSlug}`,
  );
}

export async function createRepairWorktree(
  options: RepairWorktreeOptions,
): Promise<CampaignWorktree> {
  const campaignSlug = portablePathSlug(options.campaignId);
  ...
  return createManagedWorktree(
    options,
    "repair",
    path.join("repair", repairSlug),
    options.branchName ?? `tyctx/campaign/${campaignSlug}/repair/${repairSlug}`,
  );
}
...
export async function createTargetWorktree(
  ...
  return createManagedWorktree(
    ...
    "target",
    "target",
    options.branchName ?? `tyctx/campaign/${campaignSlug}/target`,
  );
}

export async function listCampaignWorktrees(
  options: ListCampaignWorktreesOptions,
...
export async function listRepositoryWorktrees(
  ...
  const output = (
    await runGit(repositoryRoot, ["worktree", "list", "--porcelain", "-z"])
  ).stdout;
...
export async function removeCampaignWorktree(
  options: RemoveCampaignWorktreeOptions,
): Promise<void> {
  const ownedRoot = campaignWorktreesRoot(options);
  const candidate = path.resolve(options.worktreePath);
  if (!isInside(ownedRoot, candidate) || samePath(ownedRoot, candidate))
    throw new Error("worktree_cleanup_path_not_owned");
  const registered = (
    await listRepositoryWorktrees(options.repositoryRoot)
  ).find((item) => samePath(item.path, candidate));
  ...
  if (!registered.branch?.startsWith(branchPrefix))
    throw new Error("worktree_cleanup_branch_not_owned");
  const args = ["worktree", "remove"];
  if (options.force) args.push("--force");
  args.push(candidate);
  await runGit(options.repositoryRoot, args, { timeoutMs: 120_000 });
  if (options.deleteBranch)
    await runGit(options.repositoryRoot, ["branch", "-D", registered.branch]);
  await removeEmptyParents(path.dirname(candidate), path.dirname(ownedRoot));
}

export async function removeAllCampaignWorktrees(
  options: ListCampaignWorktreesOptions & {
    force?: boolean;
...
async function createManagedWorktree(
  ...
  if (!branch.startsWith(branchPrefix))
    throw new Error("worktree_branch_not_campaign_owned");
  ...
  if (!isInside(ownedRoot, target) || samePath(ownedRoot, target))
    throw new Error("worktree_target_not_owned");
  ...
  if (byPath || byBranch) {
    if (!byPath || !byBranch || !samePath(byPath.path, byBranch.path))
      throw new Error("worktree_resume_identity_mismatch");
  ...
  if (branchExists)
    await assertDescendant(
      repositoryRoot,
      baseCommit,
      await currentHead(repositoryRoot, branch),
    );
  ...
  if (!branchExists && headCommit !== baseCommit)
    throw new Error("worktree_exact_base_mismatch");
...
async function assertDescendant(
  ...
  if (result.exitCode !== 0)
    throw new Error("worktree_branch_not_descendant_of_base");
}

function complete(
  value: Partial<ListedCampaignWorktree>,
): ListedCampaignWorktree {
  return {
