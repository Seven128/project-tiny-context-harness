...
export async function activateLongTask(contract: CompiledContractV3, hookBundleSha256: string, identity: ActiveLongTaskActivationIdentity = {}): Promise<void> {
  if ((identity.campaign_id === undefined) !== (identity.slice_id === undefined)) changed("campaign_slice_identity");
  if (identity.campaign_id !== undefined && (!identity.campaign_id.trim() || !identity.slice_id?.trim())) changed("campaign_slice_identity");
  const worktree = await resolveGitWorktreePaths(contract.repository_root);
  if (!sameFilesystemPath(worktree.worktree_root, contract.repository_root)) changed("repository_root");
  ...
  for (const file of await activePointerFiles(contract.repository_root)) await writeExclusive(file, content);
}

export async function readActiveLongTaskBinding(repositoryRoot: string): Promise<ActiveLongTaskBindingV4 | null> {
  const [project, git] = await Promise.all((await activePointerFiles(repositoryRoot)).map(readPointer));
...
export async function clearAcceptedLongTaskBinding(repositoryRoot: string, workdir: string): Promise<boolean> {
  ...
  await Promise.all((await activePointerFiles(repositoryRoot)).map((file) => rm(file, { force: true })));
  return true;
}

async function readPointer(file: string): Promise<ActiveLongTaskBindingV4 | null> {
  let text: string;
...
function isActiveBinding(value: unknown): value is ActiveLongTaskBindingV4 {
  ...
  const campaignSlice = typeof item.campaign_id === "string" && item.campaign_id.length > 0
    && typeof item.slice_id === "string" && item.slice_id.length > 0;
  return item.schema_version === "active-long-task-binding-v4"
...
async function activePointerFiles(repositoryRoot: string): Promise<string[]> {
  const worktree = await resolveGitWorktreePaths(repositoryRoot);
  if (!sameFilesystemPath(worktree.worktree_root, repositoryRoot)) changed("repository_root");
  return [path.join(worktree.worktree_root, ".codex", ACTIVE_POINTER), await resolveGitWorktreePath(repositoryRoot, ACTIVE_POINTER)];
}
