...
export async function createLongTaskSnapshot(projectRoot: string, contract: CompiledContractV2, runId: string): Promise<SnapshotHandle> {
  ...
  async function copyDirectory(from: string, relative = ""): Promise<void> {
    for (const entry of await readdir(from, { withFileTypes: true })) {
      ...
      const info = await stat(sourcePath); const inode = `${info.dev}:${info.ino}`; if (info.nlink > 1 || inodeSeen.has(inode)) throw new Error(`snapshot_hardlink_rejected:${rel}`); inodeSeen.add(inode);
      const content = await readFile(sourcePath); await mkdir(path.dirname(targetPath), { recursive: true }); await writeFile(targetPath, content, { mode: info.mode }); files.push({ path: rel, type: "file", mode: info.mode, size: content.length, sha256: sha256Hex(content) });
    }
  }
...
export async function hashLongTaskWorkspace(projectRoot: string, contract: CompiledContractV2): Promise<string> { const handle = await createLongTaskSnapshot(projectRoot, contract, "workspace-hash"); try { return handle.manifest.snapshot_sha256; } finally { await handle.dispose(); } }
