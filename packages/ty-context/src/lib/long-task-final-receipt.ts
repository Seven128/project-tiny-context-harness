...
export async function writeLongTaskFinalAuthority(
  contract: CompiledContractV3,
  workdir: string,
  result: FinalResultV3,
): Promise<void> {
  ...
  for (const file of await receiptFiles(contract.repository_root))
    await atomic(file, receipt);
}

export async function assertLongTaskFinalAuthority(
  contract: CompiledContractV3,
  resultText: string,
  result: FinalResultV3,
): Promise<void> {
  const receipts = await Promise.all(
    (await receiptFiles(contract.repository_root)).map(readReceipt),
  );
  if (canonicalJson(receipts[0]) !== canonicalJson(receipts[1]))
    invalid("receipt_mismatch");
...
export async function invalidateLongTaskFinalAuthority(
  repositoryRoot: string,
  workdir: string,
): Promise<void> {
  const files = [
    path.join(workdir, "final-result.json"),
    ...(await receiptFiles(repositoryRoot)),
  ];
...
async function readReceipt(file: string): Promise<FinalResultReceiptV3> {
  ...
  try {
    ...
    invalid("receipt_missing_or_invalid");
  }
...
function isReceipt(value: unknown): value is FinalResultReceiptV3 {
  ...
  return (
    item.schema_version === "long-task-final-result-receipt-v3" &&
    typeof item.repository_root === "string" &&
    typeof item.workdir === "string" &&
    typeof item.contract_sha256 === "string" &&
    typeof item.run_id === "string" &&
    (item.workflow_status === "accepted" ||
      item.workflow_status === "needs_work") &&
...
async function receiptFiles(repositoryRoot: string): Promise<string[]> {
  ...
  return [
    path.join(worktree.worktree_root, ".codex", RECEIPT),
    await resolveGitWorktreePath(repositoryRoot, RECEIPT),
  ];
}
