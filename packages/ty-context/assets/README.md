...
Tiny Context has three capabilities. Minimal Context is the durable fact layer in `project_context/**` or `DESIGN.md`. The default Workflow Contract reads only relevant Context, decides one `Context Delta`, updates durable facts when required, uses the platform's internal plan, implements, runs project-owned verification and finishes with Contract Conformance plus a Context drift check. Composite Long-Task is an explicit Codex profile for strict multi-SFC execution; it inherits Context rules but replaces ordinary planning and completion calculation with Contract V3 and Campaign gates.
...
For ordinary long-running acceptance planning, explicitly invoke `/normal-long-task`. For a discussed multi-SFC plan that should be prepared and executed end to end, explicitly invoke `/prepare-composite-long-task`. That explicit invocation authorizes Campaign V5 to preserve the plan and complete Source Coverage V2, build one maximal-cohesion Scope Fit V4 DAG, use one persistent Codex App Server thread per SFC to author Contract V3 inputs and execute its Goal, schedule conservative worktree waves, integrate/repair them, reverify the merged state and finalize the target branch without ordinary scheduling prompts.
...
The workdir contains the three YAML authorities, `compiled-contract.json`, `goal-objective.txt`, verifier-owned status/results and `runs/**`; it has no duplicate Context artifact, plan matrix or verdict. The agent cannot submit pass results or completion status. Ordinary questions are unaffected because the Hook is a no-op without an active task and Composite is explicit-only. An identical compile is idempotent; changed frozen identities fail with `active_contract_changed`.
...
This pre-stable guarantee prevents declared requirement/obligation omissions, changed compile authorities, stale final results and post-final workspace drift in the normal CLI/Hook flow. It is not a hostile-Host boundary: it does not resist deliberate same-user/admin state or Hook deletion, Credential Manager/Registry attacks, system-level Hook bypass or kernel/sandbox escape. Focused tests must finish within 5 minutes, the Composite suite within 15 minutes and one test normally within 2 minutes; default tests do not install VMs, containers, browser matrices or administrator environments. Local `npm test` excludes Composite self-tests; GitHub package PR/main and publish gates run both the complete default and Composite suites. Maintainers can also invoke Composite explicitly with `npm run test:composite-workflow --workspace project-tiny-context-harness`. The six real CLI/final-gate black boxes are `happy_path_real_implementation`, `missing_obligation`, `source_changed_after_compile`, `oracle_or_verifier_changed_after_compile`, `stale_or_missing_final_result` and `drift_repair_end_to_end`.
...
After explicit `ty-context enable composite-codex`, `/prepare-composite-long-task` and `ty-context composite-campaign` provide a clean-start Campaign V5 orchestration plane over Contract V3; mutating, compiling, running and status CLI operations fail closed while that profile is disabled. `create --plan-file` preserves immutable source; Source Coverage V2 records `context_resolution: existing|updated|task_local` with exact Context references or a task-local reason, while terminal dispositions use `null`. `apply-scope` freezes a graph/file-hash Context baseline and accepts a maximal-cohesion Scope Fit V4 DAG: SFCs never split merely for parallelism, files, layers, agents or duration; capacity splits require persisted evidence and the first Goal freezes the graph and Context baseline.

Before any Goal, Campaign records the Context graph and baseline hashes. Unchanged graphs reread only Packet-referenced Context; relevant changes force reauthoring. `run --campaign` launches Codex App Server over stdio JSONL. Each ready SFC receives one persistent thread and a Goal Manifest V3 carrying a closed Change Envelope: Technical Plan `file`/`path_glob` bindings become `allowed_write_paths`, Scope Fit generated artifacts and package-manager manifests become `allowed_supporting_paths`, non-file bindings become `allowed_contract_keys`, and the frozen undeclared-change policy is `reject`. Every Obligation must still have a file carrier; symbol/schema/route/runtime bindings cannot be its sole implementation carrier. `project_context/**` and `.codex/composite-long-task/**` stay hard-forbidden after Goal creation even if a Packet tries to allow them, every undeclared write fails, and Receipt V2 binds the final `base..head` changed paths to the Envelope hash. Model routing is read from a versioned YAML policy and host catalog; the policy id/hash, catalog hash and complete routing decision are frozen and hash-checked. A missing or invalid policy never guesses a downgrade: it records a deterministic safe-passthrough policy decision.

The scheduler derives conflicts from dependencies, writes, verification inputs, bindings, API/schema/route/runtime/Context contracts, resources, migrations, generated artifacts, package manifests and environment profiles; unknown overlap is serial. Primary worktree state is preserved even when dirty by using temporary-index/`commit-tree` hidden refs and explicit target worktrees; target synchronization and finalization run only in Campaign-owned worktrees, with `preserve_primary_worktree: true` invariant and `auto_push: true` default. Campaign mutation uses a lease lock, root `.campaign-transaction.json` WAL intent, staged/fsynced artifacts, atomic durable writes, hash-chained events, deterministic startup recovery and quarantine. Concurrent App Server turns are observed with settled-result semantics and reconciliation, so the host never exits while a mutation may still be running.
...
Target finalization fetches the authoritative upstream and converges immediately only for the exact verified commit or tree. Any different Target tree receives one complete Target Snapshot Gate over current Packets, Source Coverage, Slice contracts and global constraints. Missing implementation uses a non-force fast-forward push/local-ref update or a matching open PR; a successful push is refetched and verified, and merged/closed PRs are not reused. Campaign `accepted` then commits state, accepted Final Result, hash-validated Target Finalization Receipt and event atomically. Reruns validate that frozen authority and return finished before App Server, Gate, PR or worktree work. Idempotent cleanup follows acceptance and can fail or resume without revoking it. Same-host live lease owners cannot be stolen after expiry; heartbeat and operation-id checks fail closed before each transaction replacement. V4 Campaigns remain inspectable audit data but cannot execute automatically.
...
| `npx --yes --package project-tiny-context-harness@latest ty-context enable composite-codex` | Explicitly installs the Codex Hooks, Composite Skills and Composite CLI profile. |
| `npx --yes --package project-tiny-context-harness@latest ty-context disable composite-codex` | Removes only package-owned Composite Hooks and surfaces while preserving user Hooks and the portable/default profiles. |
...
| `npx --yes --package project-tiny-context-harness@latest ty-context composite-campaign <subcommand>` | Campaign V5 commands include pure/audit `contract`, `create`, `apply-coverage`, `apply-scope`, Packet/preflight, `advance`, Goal/receipt/status surfaces plus automatic `run`, `app-server-check`, `model-routing`, `threads` and `interrupt`. `run` owns App Server authoring, Goal execution, recovery, repair, integration and finalization; unavailable hosts never fall back. |
