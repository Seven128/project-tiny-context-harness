...
For ordinary long-running acceptance planning, explicitly invoke `/normal-long-task`. For a discussed multi-SFC plan that should be prepared and executed end to end, explicitly invoke `/prepare-composite-long-task`. That explicit invocation authorizes Campaign V5 to preserve the plan and complete source coverage, build one maximal-cohesion Scope Fit V4 DAG, use one persistent Codex App Server thread per SFC to author Contract V3 inputs and execute its Goal, schedule conservative worktree waves, integrate/repair them, reverify the merged state and finalize the target branch without ordinary scheduling prompts.
...
The generated consumer workflow runs only the selected Harness gate: `validate-context`, `validate-code-modularity` or composite `validate-harness`. `validate-plan-contract` no longer exists. `validate-plan-acceptance` remains only for explicitly retained legacy artifacts and is not part of the default or ordinary long-task workflow. Maintainer-only package tests stay out of consumer projects.
...
After explicit `ty-context enable composite-codex`, `/prepare-composite-long-task` and `ty-context composite-campaign` provide a clean-start Campaign V5 orchestration plane over Contract V3; mutating, compiling, running and status CLI operations fail closed while that profile is disabled. `create --plan-file` preserves immutable source; Source Coverage records `context_resolution: existing|updated|task_local` with exact Context references or a task-local reason. `apply-scope` accepts a maximal-cohesion Scope Fit V4 DAG: SFCs never split merely for parallelism, files, layers, agents or duration; capacity splits require persisted evidence and the first Goal freezes the graph.

Before any Goal, Campaign records the Context graph and baseline hashes. Unchanged graphs reread only Packet-referenced Context; relevant changes force reauthoring. `run --campaign` launches Codex App Server over stdio JSONL. Each ready SFC receives one persistent thread and a closed Change Envelope containing `allowed_write_paths`, `allowed_supporting_paths`, `forbidden_paths`, carrier paths for non-file bindings and the frozen `undeclared_change_policy: reject`. Contract V3 `supporting_paths` are projected into allowed supporting paths; `project_context/**` and `.codex/composite-long-task/**` stay hard-forbidden after Goal creation even if a Packet tries to allow them, and every undeclared write fails. Model routing is read from a versioned YAML policy and host catalog; the policy id/hash, catalog hash and complete routing decision are frozen and hash-checked.

The scheduler derives conflicts from dependencies, writes, verification inputs, bindings, API/schema/route/runtime/Context contracts, resources, migrations, generated artifacts, package manifests and environment profiles; unknown overlap is serial. Primary worktree state is preserved even when dirty by using temporary-index/`commit-tree` hidden refs and explicit target worktrees. Campaign mutation uses WAL intents, leases, atomic durable writes, hash-chained events, startup recovery and quarantine. Concurrent App Server turns are observed with settled-result semantics and reconciliation, so the host never exits while a mutation may still be running.

Accepted branches merge only to Integration. Wave impact combines merge diff, bindings, verification inputs, contract keys, Context refs and global constraints; unknown impact reruns everything. Campaign Final Gate uses one shared snapshot. Specs are deduplicated only when snapshot, normalized spec, oracle, executable, input paths, command and environment all match. Target movement triggers synchronization and revalidation before final integration. V4 Campaigns remain inspectable audit data but cannot execute automatically.
...
| `npx --yes --package project-tiny-context-harness@latest ty-context validate-plan-acceptance <dir>` | Legacy explicit-artifact consistency check. It is not a default workflow, ordinary long-task or Composite gate. |
