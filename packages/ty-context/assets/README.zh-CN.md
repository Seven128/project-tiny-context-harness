...
Long-Task Contract Authoring 会尽量保留 Source 中已有的稳定 Key 与 Anchor。保持产品含义的结构分解和有真实证据的仓库绑定可以继续；新增业务规则、默认值、恢复行为、权限或范围必须进入 `decision_required`，不能静默加入。缺少推荐标题、Key、Anchor 或类型标签本身不构成阻塞。
...
本轮 Authoring 优化继续使用 `long-task-delivery-v2`：完整展开的 V2 Contract 与既有 `outcome_files` 仍可解析，不新增 V3 或 Contract 迁移。可选 Source Plan Authoring 不新增 Schema、CLI、Preflight、Compile、Validator、Receipt、Authority 或状态。Contract Authoring Preflight 会在正式 Compile 前要求 Assertion 具有可读 `criterion`；直接 Compile 仍兼容缺失该字段的旧 V2 Assertion。
