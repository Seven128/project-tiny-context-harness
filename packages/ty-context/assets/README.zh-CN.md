...
- `compile` 生成 Global 与 Outcome Product/Control/Non-completing/Technical Claim，拒绝未覆盖 Claim，保留首次不可重置 baseline，按 Outcome 计算风险，并冻结 Source hash、Context topology/file、canonical Product/Global 语义、owner/binding、resolved runner 与 verification inputs；active binding 位于 Git common-dir，并由 Git config marker 双向校验。
...
Contract authority、Source hash/file set、Context topology/file set/hash、Product semantic projection 或 Global semantic projection 的任何变化都必须使用 `--revise`；普通 compile 不能静默重新冻结变化后的 Source 或 Context。执行开始后，Source/Context/Product/Global 语义变化、Product Claim 增删改、runner 或既有 verification input 变化、proof 削弱与 scope 扩张都必须批准绑定 previous/next 实际材料的精确 revision identity。只有机械 proof 增强与机器证明的范围收紧可自动 revise。Risk downgrade 直接拒绝，执行 Agent 不得自行批准，首次 baseline 始终保留，受影响 progress 与 Receipt 失效。

Repository pattern inclusion 统一使用保守 subset prover。完全相等、精确文件、受支持的 subtree、单 segment 与扩展名关系可以被证明；复杂或无法证明的关系一律 fail closed。`src/safe/*.ts` 改为 `src/safe/**` 即使静态前缀相同也属于 widening。
