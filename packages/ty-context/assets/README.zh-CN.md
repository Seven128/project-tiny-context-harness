...
已有讨论方案、需要多个 SFC 从准备一直自动执行到目标分支时，显式调用 `/prepare-composite-long-task`。该显式调用即授权完整 prepare-and-execute；`ty-context composite-campaign` 保存不可静默改写的 `source-plan.md` 和完整 source coverage，建立稳定 Scope Fit V3 DAG，对当前 ready frontier 统一编写 `CompositeAuthoringPacketV3`，并确定性投影三份 V3 YAML authority：

- `product-architecture-source.yaml`
- `technical-realization-plan.yaml`
- `acceptance-checklist.yaml`

preflight 必须通过 Requirement/PI/obligation/binding/AC/proof/spec/counterfactual 完整覆盖和 oracle 可用性检查。CLI 再根据依赖、写入/读取路径、API/schema/route/runtime/Context contract 和资源锁计算确定性 wave；无法证明安全并行时自动串行。所有 SFC 都走独立 worktree；同一 wave 先完整保留 worktree/objective/launch token，再批量启动并绑定 Goal。每个 Slice 先 commit 且 clean，再 final-gate；`record-result` 只验证现有结果和 receipt，不重跑 final-gate。

accepted Slice 只合入 Integration Branch。普通 merge conflict 和组合回归自动生成 repair worktree/Goal；每波通过 Integration Gate 后才解锁下游。全部 SFC integrated 后，Campaign Final Gate 在同一最终快照重跑所有 SFC、global constraints 和 source coverage；目标分支变化会自动同步并重验，成功 merge/push 后才推导 Campaign `accepted`。普通调度、Git 冲突、测试失败和可恢复 Goal 中断不询问用户；只有产品语义决策或真实外部阻塞才暂停。旧 V1/V2/Markdown campaign、旧 workdir 和历史 runtime 不提供 importer、alias 或静默迁移。
