# Current-SFC Packet Authoring V3

Author only the selected dependency-ready SFC as `CompositeAuthoringPacketV3`. Run `ty-context composite-campaign contract --json` immediately before authoring; code owns the three strict authority schemas and canonical YAML filenames.
...
```json
{
  ...
  "authorities": {
    "product_architecture_source": { "schema_version": "product-source-v3" },
    "technical_realization_plan": { "schema_version": "technical-plan-v3" },
    "acceptance_checklist": { "schema_version": "acceptance-checklist-v3" }
  }
}
```

Revision 1 uses `previous_packet_sha256: null`; later revisions bind the preceding immutable packet hash. V1/V2 packets are rejected without compatibility parsing.
...
```text
ty-context composite-campaign apply-packet --campaign <path> --sfc <id> --input <packet.json>
ty-context composite-campaign render --campaign <path> --sfc <id>
ty-context composite-campaign preflight --campaign <path> --sfc <id> --json
```
