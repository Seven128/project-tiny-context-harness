...
```text
ty-context composite-campaign handoff --campaign <path> --sfc <id>
```
...
```text
ty-context composite-campaign start --campaign <path> --sfc <id> --goal-id <id>
```
...
```text
ty-context composite-campaign record-result --campaign <path> --sfc <id> --workdir <path>
```
...
`next` is read-only: a `recommended` candidate is not yet selected and cannot accept a packet. If exactly one dependency-ready candidate exists, create a new `ScopeFitResultV3` with the same stable graph and IDs, preserve prior rationale plus the continuation reason, set `decision: "split_required"`, set `selected_sfc_id` to that candidate, clear `decision_required`, and run:
