...
Contract V2 compiles Global plus Product/Control/Non-completing/Technical Claims and rejects uncovered Claims. It freezes Source hashes, Context topology/files, canonical Product/Global semantic projections, owner/path/binding authority, an immutable initial base, resolved runners and explicit verification inputs. The complete active compiled authority lives in Git common-dir and is marker-bound by task, revision and identity; the workdir compiled file is only a rebuildable cache projection. After execution begins, semantic material changes, reduced input coverage or weakened expected outputs require exact user-approved Authority Revision; only mechanical proof additions and proven scope tightening revise automatically, and the executing Agent never approves its own pending revision. Repository pattern matching/subset/overlap share one restricted parser, unsupported syntax is rejected and unknown relations fail closed. Missing or type-incomparable Observations never prove Claims. Targeted verify accumulates scoped per-Check progress and never grants acceptance.

Status, progress, receipts and compiled cache are audit/recovery surfaces only. Status/resume/verify use the common-dir active snapshot when cache is missing or mismatched. Final Gate, Stop and close recompile the source Contract, require the common-dir authority snapshot/config marker to agree, require a clean candidate commit, create a Git-tree snapshot, and rerun every required Check. Successful Stop or close atomically clears the active binding. External confirmations remain explicit; machine acceptance never implies CI, deployment or human acceptance.
