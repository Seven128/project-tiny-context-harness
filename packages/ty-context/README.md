...
Distribution uses `core-portable`, `workflow-default` and explicit `composite-codex` profiles. Default Codex `init` installs the portable core and ordinary workflow; `ty-context enable composite-codex` installs Codex Hooks, Composite Skills and Composite CLI support. Non-Codex hosts do not receive Composite surfaces unless explicitly enabled. `validate-plan-contract` no longer exists; `validate-plan-acceptance` remains only for legacy explicit artifacts. `init` never creates Campaign data, lifecycle/plan state, business surface contracts or stage work-product trees.
...
| Plan acceptance validation | `npx --yes --package project-tiny-context-harness@latest ty-context validate-plan-acceptance <dir>` | Legacy explicit-artifact consistency check; not part of default Workflow, ordinary long-task output or Composite gates. |
...
After explicit `ty-context enable composite-codex`, `/prepare-composite-long-task` and `ty-context composite-campaign` provide a clean-start Campaign V5 plane over Contract V3; mutating, compiling, running and status CLI operations fail closed while that profile is disabled. Source Coverage embeds `context_resolution: existing|updated|task_local` with exact refs or task-local reason. Scope Fit V4 creates maximal coherent SFCs and never splits merely for parallelism, files, layers, agents or duration; capacity splits require persisted evidence and the first Goal freezes scope.

Before any Goal, Campaign freezes the Context graph and baseline hashes; unchanged graphs reread only Packet-referenced Context, while relevant changes force reauthoring. Every ready SFC receives one thread, one isolated worktree and a closed Change Envelope containing `allowed_write_paths`, `allowed_supporting_paths`, `forbidden_paths`, carrier paths for non-file bindings and `undeclared_change_policy: reject`; Contract V3 `supporting_paths` are projected into that closed envelope, while `project_context/**` and `.codex/composite-long-task/**` remain hard-forbidden after Goal creation. Model routing uses a versioned YAML policy plus host catalog and freezes the policy id/hash, catalog hash and complete hash-checked routing decision.

Preflight derives dependency/write/verification/binding/contract/Context/resource plus migration/generated/package/environment conflicts; unknown evidence is serial. Primary worktree state is preserved via explicit target worktrees and temporary-index hidden refs. Campaign writes use WAL intents, leases, atomic durability, hash-chained events, recovery and quarantine. App Server waits with settled-result semantics and records Turn observation/reconciliation so no mutation is abandoned.

Targeted `verify` is optional and non-accepting. Slice, impact-aware Wave and shared-snapshot Campaign gates remain mandatory. Final specs deduplicate only when snapshot, normalized spec, oracle, executable, input paths, command and environment are identical; moved targets are revalidated. Campaign V4 state is audit-only.
