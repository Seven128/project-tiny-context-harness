...
| Long-Task Workflow Campaign V5 orchestrator | `npx --yes --package project-tiny-context-harness@latest ty-context composite-campaign <subcommand>` | Exposes pure/audit Campaign commands plus automatic `run`, `app-server-check`, `model-routing`, `threads` and `interrupt`. `run` requires Codex App Server and drives one persistent thread per SFC from read-only Packet authoring through Goal execution, recovery, repair, integration and finalization. |
