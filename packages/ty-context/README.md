...
The workdir contains the three YAML authorities, compiled identity, verifier-owned status/results and `runs/**`; it has no duplicate Context artifact, plan matrix or verdict. The agent cannot submit pass results or completion status. Ordinary questions are unaffected because the Hook is a no-op without an active task and Composite is explicit-only. An identical compile is idempotent; changed frozen identities fail with `active_contract_changed`.
...
Distribution uses `core-portable`, `workflow-default` and explicit `composite-codex` profiles. Default Codex `init` installs the portable core and ordinary workflow; `ty-context enable composite-codex` installs Codex Hooks, Composite Skills and Composite CLI support, while `ty-context disable composite-codex` removes only package-owned Composite Hooks and surfaces. Non-Codex hosts do not receive Composite surfaces unless explicitly enabled. Plan Validator commands no longer exist; existing plan, matrix or verdict files remain ordinary user files. `init` never creates Campaign data, lifecycle/plan state, business surface contracts or stage work-product trees.
...
| Composite Codex profile | `npx --yes --package project-tiny-context-harness@latest ty-context enable composite-codex` | Explicitly installs Codex Hooks, Composite Skills and Composite CLI surfaces. |
| Disable Composite Codex | `npx --yes --package project-tiny-context-harness@latest ty-context disable composite-codex` | Removes only package-owned Composite Hooks and surfaces while preserving user Hooks. |
...
| Composite long-task workflow Skill | `<harnessRoot>/skills/composite-long-task-workflow/SKILL.md` | Explicit-only Contract V3 executor for the three YAML authorities, verifier-owned repair, fresh final-gate recomputation and project-level Stop freshness enforcement. |
...
After explicit `ty-context enable composite-codex`, `/prepare-composite-long-task` and `ty-context composite-campaign` provide a clean-start Campaign V5 plane over Contract V3; mutating, compiling, running and status CLI operations fail closed while that profile is disabled. Source Coverage V2 embeds `context_resolution: existing|updated|task_local` with exact refs or task-local reason, while terminal dispositions use `null`. Scope Fit V4 creates maximal coherent SFCs and never splits merely for parallelism, files, layers, agents or duration; capacity splits require persisted evidence and the first Goal freezes scope and the Context baseline.

Before any Goal, Campaign freezes the Context graph and baseline hashes; unchanged graphs reread only Packet-referenced Context, while relevant changes force reauthoring. Every ready SFC receives one thread, one isolated worktree and Goal Manifest V3 with a hashed Change Envelope: file/path-glob bindings define writes, Scope Fit generated/package-manager paths define support and non-file bindings define contract keys while every Obligation still requires a file carrier. `project_context/**` and `.codex/composite-long-task/**` remain hard-forbidden after Goal creation, and Envelope-bound Receipt V2 rejects any undeclared `base..head` path. Goal Manifest V2 and Receipt V1 are audit-only. Model routing uses a versioned YAML policy plus host catalog and freezes the policy id/hash, catalog hash and complete hash-checked routing decision; a missing or invalid policy records deterministic safe passthrough instead of guessing a downgrade.

Preflight derives dependency/write/verification/binding/contract/Context/resource plus migration/generated/package/environment conflicts; unknown evidence is serial. Primary worktree state is preserved via explicit target worktrees and temporary-index hidden refs; all upstream/finalization work stays in Campaign-owned worktrees, `preserve_primary_worktree` is invariant and `auto_push` defaults true. Campaign writes use a lease lock, root transaction intent, staged/fsynced artifacts, atomic durability, hash-chained events, deterministic recovery and quarantine. App Server waits with settled-result semantics and records Turn observation/reconciliation so no mutation is abandoned.
