...
- Version: `agent-project-sdlc@0.1.30`
- Milestone: `MVP`
- Date: `2026-06-01`
...
|---|---|---|
| npm package | `agent-project-sdlc` | `0.1.30` |
| dry-run tarball | `npm pack --dry-run --json --workspace agent-project-sdlc` | `9c3e5aa8f57e1d18cbb167c8d938c99b5af4bc69` |
| dry-run integrity | same | `sha512-wVPSPL5Obf1Lj41RNgysPOQ5tc5KI/is2SJtwfb43v6NlD5c9DgYdjkGb/+L9iD/0wpH3FZ4zdAaHUlmdh8e3A==` |
| package content | dry-run output | 116 files, 164.2 kB package size, 605.1 kB unpacked size |
| registry package | `npm view agent-project-sdlc version dist-tags.latest dist.integrity --json` | `version 0.1.30`, `latest 0.1.30`, `integrity sha512-wVPSPL5Obf1Lj41RNgysPOQ5tc5KI/is2SJtwfb43v6NlD5c9DgYdjkGb/+L9iD/0wpH3FZ4zdAaHUlmdh8e3A==` |
...
- Evidence:
  ...
  - `npm publish --workspace agent-project-sdlc`: PASS，registry 返回 agent-project-sdlc@0.1.30。
  - `npm view agent-project-sdlc version dist-tags.latest dist.integrity --json`: PASS，version 和 latest 均为 0.1.30。
  - Registry installed-consumer smoke: PASS，从 npm registry 安装 agent-project-sdlc@0.1.30 后，init 和 doctor 均通过，doctor 输出 `core package: agent-project-sdlc@0.1.30`。

## 5. Deployment Checklist（部署检查清单）

- [x] Confirm registry latest before publishing.
- [x] Bump package version to `0.1.30`.
...
- [ ] Create and push git tag `v0.1.30` after publish success.
