...
- Version: `agent-project-sdlc@0.1.22`
- Milestone: `MVP`
- Date: `2026-05-30`
...
|---|---|---|
| npm package | `agent-project-sdlc` | `0.1.22` |
| dry-run tarball | `npm pack --dry-run --json --workspace agent-project-sdlc` | `27648fa4bdae22a4b8617d7a14b65401439c412a` |
| dry-run integrity | same | `sha512-Kd09+1TKuPKrLLojdfRSFgEpLLqgbeYEk8Yrw+/tus/4COLoqcJ0D5HpIjsNyjaXhCe15nhvtqw/K80kl42CQQ==` |
| package content | dry-run output | 109 files, 115.6 kB package size, 429.2 kB unpacked size |
| registry package | `npm view agent-project-sdlc version dist-tags.latest dist.integrity --json` | `version 0.1.22`, `latest 0.1.22`, `integrity sha512-Kd09+1TKuPKrLLojdfRSFgEpLLqgbeYEk8Yrw+/tus/4COLoqcJ0D5HpIjsNyjaXhCe15nhvtqw/K80kl42CQQ==` |
...
- Evidence:
  ...
  - `npm publish --workspace agent-project-sdlc`: PASS，registry 返回 agent-project-sdlc@0.1.22。
  - `npm view agent-project-sdlc version dist-tags.latest dist.integrity --json`: PASS，version 和 latest 均为 0.1.22。
  - Registry installed-consumer smoke: PASS，从 npm registry 安装 agent-project-sdlc@0.1.22 后，init 和 doctor 均通过，doctor 输出 `core package: agent-project-sdlc@0.1.22`。

## 5. Deployment Checklist（部署检查清单）

- [x] Confirm registry latest before publishing.
- [x] Bump package version to `0.1.22`.
...
- [ ] Create and push git tag `v0.1.22` after publish success.
