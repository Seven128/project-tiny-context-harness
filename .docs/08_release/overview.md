...
- Version: `agent-project-sdlc@0.1.29`
- Milestone: `MVP`
- Date: `2026-06-01`
...
|---|---|---|
| npm package | `agent-project-sdlc` | `0.1.29` |
| dry-run tarball | `npm pack --dry-run --json --workspace agent-project-sdlc` | `9c841a2eb361bdb048d4b92179141574069e5d5d` |
| dry-run integrity | same | `sha512-/ikCy+47KhQAJY4xyUGfTYSDwSqcc2Oy6dclVLz/JGTG6b5CK+qCjBRk8fSBv1fQBkF+rC6WKOeV2IiGhp1z6w==` |
| package content | dry-run output | 116 files, 158.4 kB package size, 590.5 kB unpacked size |
| registry package | `npm view agent-project-sdlc version dist-tags.latest dist.integrity --json` | `version 0.1.29`, `latest 0.1.29`, `integrity sha512-/ikCy+47KhQAJY4xyUGfTYSDwSqcc2Oy6dclVLz/JGTG6b5CK+qCjBRk8fSBv1fQBkF+rC6WKOeV2IiGhp1z6w==` |
...
- Evidence:
  ...
  - `git diff --check`: PASS。
  - `npm publish --workspace agent-project-sdlc`: PASS，registry 返回 agent-project-sdlc@0.1.29。
  - `npm view agent-project-sdlc version dist-tags.latest dist.integrity --json`: PASS，version 和 latest 均为 0.1.29。
  - Registry installed-consumer smoke: PASS，从 npm registry 安装 agent-project-sdlc@0.1.29 后，init 和 doctor 均通过，doctor 输出 `core package: agent-project-sdlc@0.1.29`。

## 5. Deployment Checklist（部署检查清单）

- [x] Confirm registry latest before publishing.
- [x] Bump package version to `0.1.29`.
...
- [ ] Create and push git tag `v0.1.29` after publish success.
