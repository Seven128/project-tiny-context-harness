...
- Version: `agent-project-sdlc@0.1.31`
- Milestone: `MVP`
- Date: `2026-06-01`
...
|---|---|---|
| npm package | `agent-project-sdlc` | `0.1.31` |
| dry-run tarball | `npm pack --dry-run --json --workspace agent-project-sdlc` | `0605cc92e94335e2d4d48ad228cbafbd84e9d44d` |
| dry-run integrity | same | `sha512-PEvINJL72MueKeVF/P653YCK5RAK+nwnNXV3ohFG33K2mOi+ABOhlG410JhJb5s4AIpJSN7/j/99pu1QSa0T1A==` |
| package content | dry-run output | 116 files, 164.7 kB package size, 607.6 kB unpacked size |
| registry package | `npm view agent-project-sdlc version dist-tags.latest dist.integrity --json` | `version 0.1.31`, `latest 0.1.31`, `integrity sha512-PEvINJL72MueKeVF/P653YCK5RAK+nwnNXV3ohFG33K2mOi+ABOhlG410JhJb5s4AIpJSN7/j/99pu1QSa0T1A==` |
...
- Evidence:
  ...
  - `npm publish --workspace agent-project-sdlc`: PASS，registry 返回 agent-project-sdlc@0.1.31。
  - `npm view agent-project-sdlc version dist-tags.latest dist.integrity --json`: PASS，version 和 latest 均为 0.1.31。
  - Registry installed-consumer smoke: PASS，从 npm registry 安装 agent-project-sdlc@0.1.31 后，init 和 doctor 均通过，doctor 输出 `core package: agent-project-sdlc@0.1.31`。

## 5. Deployment Checklist（部署检查清单）

- [x] Confirm registry latest before publishing.
- [x] Bump package version to `0.1.31`.
...
- [ ] Create and push git tag `v0.1.31` after publish success.
