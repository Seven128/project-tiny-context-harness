...
- Version: `agent-project-sdlc@0.1.17`
- Milestone: `MVP`
- Date: `2026-05-29`
...
|---|---|---|
| npm package | `agent-project-sdlc` | `0.1.17` |
| dry-run tarball | `npm pack --dry-run --json --workspace agent-project-sdlc` | `4ce6c680b449404f40afc96057dee0570e576f6e` |
| dry-run integrity | same | `sha512-znjSA8DWPwhIiChT2FDiOFBLnBZ48iUI375DBYdh5T81sbRuYcGgFlrwIEQSTcrXtTN2Vijebs1+LkIC7O7OQQ==` |
| package content | dry-run output | 87 files, 76.1 kB package size, 268.3 kB unpacked size |
| registry package | `npm view agent-project-sdlc version dist-tags.latest dist.integrity --json` | `version 0.1.17`, `latest 0.1.17`, `integrity sha512-znjSA8DWPwhIiChT2FDiOFBLnBZ48iUI375DBYdh5T81sbRuYcGgFlrwIEQSTcrXtTN2Vijebs1+LkIC7O7OQQ==` |
...
- Evidence:
  ...
  - `git diff --check`: PASS。
  - `npm publish --workspace agent-project-sdlc`: PASS，registry 返回 agent-project-sdlc@0.1.17。
  - `npm view agent-project-sdlc version dist-tags.latest dist.integrity --json`: PASS，version 和 latest 均为 0.1.17。
  - Registry installed-consumer smoke: PASS，从 npm registry 安装 agent-project-sdlc@0.1.17 后，init 和 doctor 均通过，doctor 输出 `core package: agent-project-sdlc@0.1.17`。

## 5. Deployment Checklist（部署检查清单）

- [x] Confirm registry latest before publishing.
- [x] Bump package version to `0.1.17`.
...
- [x] Create and push git tag `v0.1.17` after publish success.
