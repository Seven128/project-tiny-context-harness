...
| 边界 | 责任 | 不负责 |
|---|---|---|
| Source Authoring Workspace | 当前 `ProjectTemplate` 仓库中维护工作流事实源、参考实现和包源同步输入 | 不保存业务项目状态 |
...
核心原则：包是分发源，工作区文件是 Agent 实际读取入口，项目状态和文档是不可覆盖事实源。
...
```txt
修改 AGENTS.md / .agents/skills / .harness/templates / .harness/policies / Makefile / workflow / validators
...
多数 Agent 的 Skill 和规则路由依赖工作区固定路径。因此 `.agents/skills/**/SKILL.md`、`AGENTS.md` managed block 和必要的 `.harness/**` 入口必须落在业务项目工作区内。npm 包中的 assets 只是 canonical source，不能替代工作区 materialized files。
...
| `.agents/skills/**/SKILL.md` | `generated` | Agent 需要本地 Skill 文件 |
| `.harness/templates/**` | `managed` | 模板来自包内 canonical source |
| `.harness/policies/*.yaml` | `merge-with-local` | 默认策略可升级，项目差异放 `.local.yaml` |
...
| Agent 无法读取包内 Skill | P0 | 所有 Skill materialize 到 `.agents/skills/**/SKILL.md` |
| Python validators 放入 npm 包后运行环境不一致 | P1 | 首版可保留 Python 脚本由 Node CLI 调用，后续评估迁移到 TypeScript |
