...
- 通过 `sdlc-harness sync` 将 Agent 必须读取的规则、Skill、模板和策略 materialize 到工作区固定路径。
...
| 同步 Agent 可读文件 | Harness 用户 | 运行 `npx sdlc-harness sync` | 将包内 canonical Skill、模板、策略、Makefile 接入片段 materialize 到工作区，并保留 local overrides |
...
| PRD-NPM-004 | 提供 `sync` 命令，将包内 canonical source 同步到工作区固定路径 | P0 | 重点覆盖 `AGENTS.md` 管理区块、`.agents/skills/**`、`.harness/templates/**`、`.harness/policies/**`、`.harness/make/sdlc-harness.mk` |
...
| PRD-NPM-008 | `Makefile` 不整体覆盖，优先插入 include 并生成 `.harness/make/sdlc-harness.mk` | P0 | 保护项目自己的 `lint`、`test`、`build` 命令 |
| PRD-NPM-009 | `.docs/**`、`.harness/state/**`、`src/**`、`tests/**` 永远不被 sync/upgrade 覆盖 | P0 | 这些是项目事实源或业务代码 |
| PRD-NPM-010 | 支持 local overrides 合成最终 Skill、模板或策略 | P1 | 推荐 `.harness/overrides/**` 和 `.harness/policies/*.local.yaml` |
...
- [ ] 执行 `npx sdlc-harness sync` 后，`.agents/skills/**/SKILL.md` 存在于工作区，Agent 可按本地固定目录读取。
...
| CLI 使用 TypeScript/Node 实现，还是保留 Python validators 并由 Node CLI 调用？ | 技术方案阶段 | open |
...
- 依赖 Agent 对工作区固定文件的读取约定，例如 `AGENTS.md` 和 `.agents/skills/**/SKILL.md`。
...
- 如果 validators 长期保留为 Python 脚本，需要决定 npm 包如何分发和调用 Python 依赖。
