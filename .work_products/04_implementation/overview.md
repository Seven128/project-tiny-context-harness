...
- Release evidence is written to `.work_products/08_release/CURRENT_RELEASE.md`; each release overwrites the current status instead of creating a versioned docs ledger.
...
| `.work_products/08_release/CURRENT_RELEASE.md` | Current release status | release notes, smoke evidence, deployment checklist, rollback plan, known issues |
...
| `make work-products-overview` | Release and implementation overview refresh | PASS for `TASK-069` |
...
| 2026-05-29 | `TASK-069` | Git history | Changed release docs to a single `.work_products/08_release/CURRENT_RELEASE.md` current-state contract with legacy validator compatibility. |

## 9. 后续维护注意事项

- Keep the release script conservative; publishing must remain explicit.
- If release evidence format changes, update `tools/release_npm.mjs`, `validate-release`, package validators, release Skill/template and `.work_products/08_release/CURRENT_RELEASE.md` expectations together.
...
- User-owned Markdown files may contain fixed package-managed sections. `sync` / `upgrade` update only `memory.md#Harness Guidance` and `.work_products/INDEX.md#Harness Maintenance Rules`, while preserving user memory entries, artifact maps and links outside those sections.
...
- DEV-043 removed the legacy implementation-doc directory as an active asset/documentation target; source sync does not treat `.work_products/04_implementation/npm_package/**` as a maintained path.
...
| 2026-05-26 | `DEV-043` | DEV-043 implementation commit | Consolidated source-sync implementation facts from legacy task work_products. |
...
- Linked PRD: `.work_products/01_product/npm_package_distribution.md` (`PRD-NPM-026`, `PRD-NPM-028`)
- Linked technical design: `.work_products/03_tech_plan/harness_package_distribution.md`
- Linked RFC: `RFC_015`
...
| `.work_products/01_product/npm_package_distribution.md` | 产品需求 | `PRD-NPM-026` |
| `.work_products/03_tech_plan/harness_package_distribution.md` | 技术方案 | Natural Language Control |
...
- Linked PRD: `.work_products/01_product/npm_package_distribution.md` (`PRD-NPM-025`)
- Linked technical design: `.work_products/03_tech_plan/harness_package_distribution.md`
- Linked RFC: none
...
- 修改（Changed）:
  ...
  - `pjsdlc_architect_design` 和 plan/tech templates 引导 future development task 指向模块级 implementation doc；非开发 task 使用 `result_work_products` 指向对应阶段产物。
  - AGENTS、PROJECT_SPEC、PRD 和 tech plan 使用同一套语义。
  - DEV-043 将历史 `.work_products/04_implementation/npm_package/dev_*.md` task log 合并为模块、子系统和核心数据流级 implementation docs，并从活跃实现文档图中移除 legacy 目录。
...
- Config contract: `.work_products/04_implementation/**/*.md`, `.work_products/INDEX.md`, implementation doc template and dev task `implementation_work_product` fields.
...
| `.codex/skills/pjsdlc_architect_design/SKILL.md` | 架构阶段任务规划规则 | task `implementation_work_product` 指向长期实现事实文档 |
...
| `.codex/pjsdlc_managed/templates/PLAN_TEMPLATE.yaml` | open task 模板 | `TASK-*`、`phase`、`result_work_products` 和 development `implementation_work_product` 示例路径 |
| `.codex/pjsdlc_managed/templates/TECH_DESIGN_TEMPLATE.md` | 技术方案模板 | task breakdown 中 implementation doc 的模块级说明 |
| `.work_products/04_implementation/harness_package/*.md` | package-facing module implementation docs | CLI lifecycle、source sync、release automation |
| `.work_products/04_implementation/harness_workflow/*.md` | workflow-facing module implementation docs | command routing、implementation model、state/task protocol、skills/prompt、docs validation |
| `packages/sdlc-harness/assets/**` | npm 包 canonical assets | 由 `package sync-source` 从 `.codex/**` 和 `AGENTS.md` 同步 |
| `.work_products/01_product/npm_package_distribution.md` | 产品约束 | `PRD-NPM-025` |
| `.work_products/03_tech_plan/harness_package_distribution.md` | 技术方案约束 | implementation doc model |

## 4. 核心数据流

```txt
Architecting
-> plan.draft.yaml task includes implementation_work_product path
-> SPRINTING executes task as bounded work unit
-> gates pass
-> implementation_work_product updates module/subsystem/core-flow fact slice
...
- 输入校验（Input validation）: development open task 使用 `implementation_work_product` 指向长期实现事实文档，而不是默认 `dev_*.md` task ledger；文档、Review、测试、发布和 RFC task 使用 `result_work_products`。
- 核心分支（Core branches）: 修改已有模块时更新已有 implementation doc；新增稳定模块或核心数据流时创建新文档；一个 task 可更新多份相关文档。
- 异常处理（Error handling）: 若真实代码边界与 architecture / tech plan 不一致，implementation doc 必须记录 deviation，并更新 `.work_products/INDEX.md`。
...
- DEV-043 完成历史合并迁移，删除活跃 `.work_products/04_implementation/npm_package/` task-log 目录，并将事实合并到模块级 implementation docs。
...
| 2026-05-27 | `TASK-057` | Git history | 明确只有 development task 使用 `implementation_work_product`，其它阶段 task 使用 `result_work_products`。 |

## 9. 后续维护注意事项

- 后续新 task 应优先更新相关模块级 implementation doc；不要默认新增 `dev_*.md`。
- 不要在 `.work_products/04_implementation/` 下重新建立 task-grain ledger；task 历史动作记录以 git commit/tag/release evidence 为准。
...
- Linked PRD: `.work_products/01_product/npm_package_distribution.md`
- Linked technical design: `.work_products/03_tech_plan/harness_package_distribution.md`, `PROJECT_SPEC.md`
- Linked RFC: `RFC_007`, `RFC_009`, `RFC_015`, `RFC_017`, `RFC_018`, `RFC_019`, `RFC_020`, `RFC_021`, `RFC_024`
- Linked ADR: `.work_products/05_decisions/ADR_009_visual_reconciliation.md`
...
- `UI_UX_DESIGNING` is now a first-class lifecycle phase between `REQUIREMENT_GATHERING` and `ARCHITECTING`. It uses `pjsdlc_uiux_design`, writes `.work_products/02_experience/**`, optionally writes root `DESIGN.md`, and exits through `validate-uiux`.
- `.work_products/02_experience/**` records UX flow, screen contracts, interaction states, responsive/a11y acceptance and handoff matrix. visual UI projects also use Google-format `DESIGN.md` as a design-system fact source; CLI/API/non-visual projects declare `Applicability: cli_or_api_experience` or `Applicability: not_applicable`.
...
- Architect prompt now treats semantic design slicing as a `make validate-design` hard gate: `plan.draft.yaml` development tasks must reference `work_products.tech_plan`, multiple draft tasks need distinct primary tech plan slices, generated `overview.md` cannot satisfy design deliverables, and explicit cross-cutting themes require dedicated architecture slices.
- Architect prompt now consumes `.work_products/02_experience/**` and `DESIGN.md`, requires `Experience Input Review`, and makes UI/frontend draft tasks reference `work_products.uiux` plus `work_products.design_system`.
...
- TESTING distinguishes `.work_products/07_test/TEST_STRATEGY.md`, `.work_products/07_test/TEST_CASES.md` and `.work_products/07_test/TEST_REPORT.md`; `TEST_REPORT.md` is execution-only evidence and `validate-test` no longer falls back to `TEST_PLAN.md`.
...
- RFC recalibration now records `Test Fact Source Impact` and removes superseded `.work_products/07_test/**` files from current test facts and `.work_products/INDEX.md` when a route, entry/exit or acceptance boundary changes.
...
- The authoring-only Harness design prompt now states the PROJECT_SPEC boundary: it describes stable zero-to-one project design, product/protocol rationale and canonical behavior, while version migration and upgrade instructions belong in README / package README or release/implementation work_products.
- The authoring-only Harness design prompt now requires future workflow changes to consult `memory.md#Harness Design Decisions` and related `.work_products/05_decisions/ADR_*.md` before changing existing design tradeoffs, and to add new durable rationale as ADR slices instead of expanding `PROJECT_SPEC.md`.
...
- `validate-uiux` requires a non-overview `.work_products/02_experience/**` deliverable, PRD / requirement references unless `Applicability: not_applicable`, applicable screen states, responsive expectations and a11y/focus/keyboard/touch expectations. visual UI slices require root `DESIGN.md` and run the local `@google/design.md` linter, failing on errors.
- `validate-design` now rejects UI/frontend draft tasks that omit `work_products.uiux` or `work_products.design_system`, or reference missing UX slices / missing `DESIGN.md`.
...
- RELEASING uses `.work_products/08_release/CURRENT_RELEASE.md` as the canonical current release status. `validate-release` keeps accepting legacy versioned release docs when the current file is absent, but new release work updates the current status file instead of creating a version ledger.
...
- Config contract: `AGENTS.md`, `.codex/pjsdlc_managed/**`, `packages/sdlc-harness/source-mappings.yaml`, `.work_products/07_test/TEST_REPORT.md`.
...
- Initialization / health: package CLI starts through Node and reads lifecycle/plan/implementation work_products.
...
- `DESIGN.md` is only mandatory for visual UI. The validator accepts explicit non-visual applicability in `.work_products/02_experience/**` to keep CLI/API/operator experience projects from manufacturing empty design systems.
...
- `validate-test` requires `.work_products/07_test/TEST_REPORT.md`, rejects placeholder report text, and requires test matrix, regression evidence, coverage gaps, runnable entry/exit coverage and PASS/BLOCKED decision text.
- `validate-test` lightly validates `.work_products/07_test/TEST_CASES.md` only when case facts are in scope: report `TC-*` refs, current TESTING task `result_work_products`, or an existing cases file. It checks unique `TC-*`, required case fields and report refs without proving execution.
- `validate-plan` rejects non-`TESTING`/`RFC_RECALIBRATION` tasks that target `.work_products/07_test/**`, so SPRINTING cannot create formal test facts before entry/exit delivery.
- `validate-rfc` checks `Test Fact Source Impact` and rejects RFCs that list superseded `.work_products/07_test/**` paths still present in current facts or `.work_products/INDEX.md`.
...
- High-risk runtime/live/remote-operator tasks now use resume-first recovery: Architect reserves `.work_products/09_runbooks/**`, Dev maintains `plan.yaml#resume_capsule`, strategy-changing decisions are promoted to `resume_capsule.do_not_retry` / runbook `Hard Constraints`, Implementation Doc keeps operation logs and evidence dumps out of the main implementation facts, and Review/Testing consume the capsule/runbook before reading exploration appendices.
- `validate-dev` requires high-risk current SPRINTING tasks to have concrete `resume_capsule` fields, current implementation doc recovery ref, `.work_products/09_runbooks/**` recovery ref, a short `Current Operator Path` with `Hard Constraints`, `Evidence Index Refs` under `.work_products/09_runbooks/**`, and a `Gate Breakdown` in `Development Self-Test Report`; it also scans notes/docs/runbooks for session / QR / canonical path / do-not-retry judgments and fails when those judgments are not promoted.
...
| `make validate-work-products-overviews` | Generated overview freshness | PASS for TASK-066 |
...
| `make work-products-overview` | Generated overview refresh after test report rename | PASS for TASK-067 |
...
| `make work-products-overview` | Generated overview refresh for RFC, implementation and test docs | PASS for TASK-070 |
...
| `make work-products-overview` | Generated overview refresh for RFC_020 and implementation docs | PASS for TASK-075 |
...
| `make work-products-overview` | Generated overview refresh after RFC_021 and implementation doc updates | PASS for TASK-076 |
...
| `make work-products-overview` | Generated overview refresh after RFC_023 and implementation doc updates | PASS for TASK-078 |
...
| `make work-products-overview` | Generated overview refresh after prompt redline implementation doc update | PASS on 2026-05-30 |
...
| `make work-products-overview && make validate-harness && make validate-plan` | Generated overview, prompt language and active plan consistency after self-test boundary changes | PASS on 2026-05-30 |
| `make work-products-overview && make validate-work-products-overviews && make validate-harness && make validate-plan && npm test --workspace agent-project-sdlc && git diff --check` | Authoring ADR lookup guidance, memory decision index and PROJECT_SPEC boundary update | PASS on 2026-05-31; package tests 10 passed |
...
| `make work-products-overview` | Generated overview refresh for new `.work_products/02_experience/` and ADR 007 | PASS on 2026-06-01 |
...
| 2026-05-27 | `TASK-057` | Git history | Generalized prompt rules so every phase main action is a `TASK-*` task governed by `plan.yaml`, with review/test/release/RFC outputs using `result_work_products`. |
...
| 2026-05-29 | `TASK-069` | Git history | Replaced versioned release document generation with canonical `.work_products/08_release/CURRENT_RELEASE.md` release status guidance and validator compatibility wording. |
...
| 2026-06-01 | UI/UX design stage | Git history | Added `UI_UX_DESIGNING`, `.work_products/02_experience/**`, optional visual UI `DESIGN.md`, `validate-uiux`, downstream consumption rules and package assets. |
...
- Document, review, test, release and RFC tasks use `result_work_products` for planned fact-source outputs; development tasks use `implementation_work_product`.
...
- Complete decision context, alternatives, rationale and consequences belong in `.work_products/05_decisions/` ADRs or other formal `.work_products/**` fact sources.
- `.work_products/05_decisions/` is not a lifecycle phase; it is an `ARCHITECTING`-produced ADR fact source for durable architecture decisions that may outlive a single architecture or tech plan slice.
...
- Release history is also cold archive. `.work_products/08_release/CURRENT_RELEASE.md` stores only the current release status; older release evidence is reconstructed from git tags, registry metadata, CI, release commits or external release systems.
...
- `.work_products/09_runbooks/**` stores runtime/live/remote-operator runbooks, evidence indexes and exploration appendices. `resume_capsule.recovery_refs` must include the current implementation doc and at least one runbook/evidence document.
...
- Config contract: task fields (`phase`, `allowed_paths`, `required_gates`, `result_work_products`, `implementation_work_product`) and lifecycle `current_phase`.
...
- Testing Handoff Readiness: ready for Review/Testing handoff after transition regression, package tests, source sync/check, work products overview, validate-harness and validate-rfc pass.
...
- Executed Gates: `node --test tests/sdlc-harness/transition.test.mjs`; `npm test --workspace agent-project-sdlc`; `node packages/sdlc-harness/dist/cli.js package sync-source`; `node packages/sdlc-harness/dist/cli.js package check-source`; `make work-products-overview`; `make validate-harness`; `make validate-rfc`.
...
- Evidence Index Refs: transition regression output; package regression output; package source sync/check output; work products overview and Harness/RFC gate output.
- Missing / Blockers: none.
...
| `.codex/pjsdlc_managed/templates/PLAN_TEMPLATE.yaml` | New-task template | open task fields, `result_work_products` and `implementation_work_product` examples |
...
| `packages/sdlc-harness/src/lib/init.ts` | Package init state seed | memory seed and `.work_products/09_runbooks` directory creation |
...
-> result_work_products or implementation_work_product points to the produced fact source
...
-> high-risk runtime/live tasks maintain resume_capsule and .work_products/09_runbooks recovery refs
...
- direct `validate-dev` requires lifecycle `current_phase: "SPRINTING"` and allows either an empty development queue or one valid current open `phase: "SPRINTING"` task with `current_task_id`, `docs`, `allowed_paths`, `required_gates`, `acceptance_criteria` and `implementation_work_product`.
- direct `validate-dev` requires `resume_capsule` for high-risk current SPRINTING tasks, validates its task id, concrete recovery fields, do-not-retry list, current implementation doc ref and `.work_products/09_runbooks/**` ref.
...
- direct `validate-dev` rejects self-test report headings that turn the report into a debug/operator/runbook/exploration log. High-risk tasks must link operator facts through `Current Operator Path` and `.work_products/09_runbooks/**`.
...
- ADRs solve the long-term "why this option, not another" problem. Architecture and tech plan slices may keep local rationale, but decisions with alternatives, cross-module or cross-stage impact, high reversal cost or future supersede semantics belong in `.work_products/05_decisions/`.
...
| `make work-products-overview` | Generated overview refresh after RFC_018 and implementation doc updates | PASS for TASK-071 |
...
| `make work-products-overview && make validate-harness && make validate-plan` | Generated overview, Harness scaffold and active plan contract after self-test boundary hardening | PASS on 2026-05-30 |
...
| 2026-05-30 | Resume-first runtime task protocol | Git history | Added high-risk runtime `resume_capsule`, `.work_products/09_runbooks` recovery docs and Gate Breakdown validation. |
...
| 2026-05-29 | `TASK-069` | Git history | Clarified that release history is cold archive while `.work_products/08_release/CURRENT_RELEASE.md` remains the active release status fact source. |
...
## harness_workflow/work_products_overview_and_validation.md

Source: [harness_workflow/work_products_overview_and_validation.md](harness_workflow/work_products_overview_and_validation.md)
