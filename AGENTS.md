...
- 需要追溯 done task 的完整执行合同时，不要从当前已压缩的 `plan.yaml` 中猜测或恢复旧字段；先读对应 implementation doc，再用 `git log --oneline --grep "<TASK_ID>"` 找到 task implementation commit，并用 `git show <implementation_commit>:.agent/state/plan.yaml` 查看当时未压缩的 open task 合同。
...
15. 如果信息缺失，或 gate 因基础设施原因失败，停止推进并报告 blocker。
