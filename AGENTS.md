...
- Harness authoring prompts（如果存在）：`.agent/prompts/authoring/`，只在维护 Harness/workflow/npm package 源码或本仓库自举规则时读取，不作为用户项目默认分发内容
...
- 后续更新 `.agent/prompts/workflow/*/PROMPT.md`、`.agent/prompts/authoring/*/PROMPT.md` 或 `.agent/pjsdlc_managed/templates/*.md` 时，遵循“中文解释 + 英文精确标识符”。Harness 根目录由 `package.json#sdlcHarness.harnessFolderName` 或 `sdlc-harness.config.json#harnessFolderName` 决定；未配置的项目默认使用 `.agent`。
...
1. 阶段约束优先：除非用户明确要求其它工作流动作，否则使用 `active_prompt` 指定的 workflow prompt，并服从当前阶段的 allowed paths、required gates 和交付物边界。
...
1. 选择任何角色或 prompt 前，先读取 `.agent/state/lifecycle.yaml`。
2. 除非用户明确要求其它工作流动作，否则使用 `active_prompt` 指定的 workflow prompt。
...
- `/next`：运行当前阶段映射的 workflow prompt。
