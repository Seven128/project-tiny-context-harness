...
│   └── archive/
│
├── .agents/skills/  # compatibility view generated by sync
├── tools/
├── .github/workflows/
└── src/ or services/
```
...
- `.agents/skills/`：由 `sdlc-harness sync` 生成的兼容视图，服务于只扫描固定 `.agents` 目录的 Agent。
- `tools/`：确定性脚本和校验工具。
- `Makefile`：统一命令入口。
...
- 默认 `.agents/skills/*/SKILL.md` 兼容生成出口。
...
- `.agents/skills/**`，由 `sdlc-harness sync` 生成，作为 Agent 兼容读取出口。
