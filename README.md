...
- `.agents/skills/*/SKILL.md`（兼容生成出口，不提交为事实源）
- `.docs/**/*.md`
- `.docs/INDEX.md`
- `Makefile`
- `tools/*.py`
...
│   └── archive/
│
├── .agents/skills/  # gitignored compatibility view generated by sync
├── tools/
├── .github/workflows/
└── src/ or services/
```
...
- `.harness/agents/skills/`：阶段角色 Skill 的 canonical source。
...
- `.agents/skills/`：由 `sdlc-harness sync` 生成的兼容视图，服务于只扫描固定 `.agents` 目录的 Agent；该目录被 `.gitignore` 排除，不作为仓库事实源提交。
- `tools/`：确定性脚本和校验工具。
- `Makefile`：统一命令入口。
...
- Agent 入口和角色规则：`AGENTS.md`、`.harness/agents/skills/**/SKILL.md`，以及生成的 `.agents/skills/**/SKILL.md` 兼容出口。
...
- 默认 `.harness/agents/skills/*/SKILL.md`。
- 默认 `.harness/managed/templates/*`。
- 默认 `.harness/managed/policies/*`。
- 默认 `.agents/skills/*/SKILL.md` 兼容生成出口；它是 generated artifact，不应提交到仓库。
...
- `AGENTS.md`。
- `.harness/agents/skills/**`，由 `sdlc-harness sync` 从包内 materialize 到工作区，作为 Skill canonical source。
- `.agents/skills/**`，由 `sdlc-harness sync` 生成，作为 Agent 兼容读取出口；业务仓库可在本地保留，但不应把它当成 source authoring path。
