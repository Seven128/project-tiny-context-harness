...
- `compile --revise` auto-adopts proven-safe revisions. Protected revisions return `authority_revision_pending` on stdout plus the exact decision id, deterministic material approval summary and its human `decision_brief`, then fail closed until `approve-authority-revision` approves that exact id. Present the brief before asking for approval. Candidate edits produce a new id and invalidate the old approval. Adoption emits `authority_revision_adopted` and returns to rolling execution; it never means delivery completion.
