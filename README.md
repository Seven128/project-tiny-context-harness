...
`REVIEWING`、`TESTING` 或 `RELEASING` 阶段发现实现偏离既有 PRD、UI/UX 和技术方案时，才使用 `bugfix_implementation_gap` 走 `python3 tools/transition.py --to SPRINTING` 创建小的 bugfix task。TESTING 阶段发现 bug 时，先在 `.docs/07_test/TEST_REPORT.md` 记录 `Bugfix Route`；需求、验收标准、产品边界、UX contract、screen contract、handoff matrix、`DESIGN.md`、技术方案、接口契约、任务拆分、Development Self-Test Contract 或 Module Key Test Graph 需要变化时都走 RFC，而不是直接回设计或开发。
