...
The workdir contains the three YAML authorities, `compiled-contract.json`, `goal-objective.txt`, `current-status.json`, `final-result.json` and verifier-owned `runs/**`. The agent cannot submit pass results, evidence records, assertion results or entity completion status. Compile accepts only the exact package-managed Hook bytes and commands. Ordinary questions are unaffected because the Hook is a no-op without an active task, and the Skill policy disables implicit invocation. With an active task, an identical compile is idempotent; changed inputs, Context, oracle/verifier identity, graph or workdir fail with `active_contract_changed`. A later activation invalidates the preceding final result before it can be reused.
...
`/prepare-composite-long-task` and `ty-context composite-campaign` provide a clean-start V3 authoring plane. `CompositeAuthoringPacketV3` stores the structured authoring source and deterministically projects the three Contract V3 YAML authorities. Preflight requires complete Requirement/PI/Obligation/Binding/AC/Proof/Spec/Counterfactual coverage and oracle readiness before handoff. Handoff copies only the YAML authorities into a fresh execution workdir; it does not create a Goal, execution result or evidence. Explicit `start` binds an already-created Goal, and `record-result` mirrors only a complete current `final-result.json` identity.
