...
- **Long delivery:** initial proposal → explicitly initialize/adopt a design system with `/design-system-authoring` when the project has none → `/design-resource-authoring` generates/selects resources, optionally captures exact Figma-native facts, reconciles accepted decisions once and emits a validated residual `design-resource-handoff-v1` for implementation → pass the revised proposal plus selected immutable resources and the validated handoff to `/long-task-workflow`. Those inputs enter one Source-bound Contract Draft loop immediately in the same native Goal.
...
A design-specific purpose of the Long-Task Workflow is that Agent implementation, acceptance and testing fully conform to every material UI/UX fact selected design resources explicitly express within their declared scope and conditions. Direct visual inspection is lossy, so the input path has two complementary layers. Optional Figma-native acquisition can pin exact file/version/nodes/conditions and freeze metadata, design context, Variables, screenshots and applicable motion/assets/Code Connect as immutable repository input, reducing manual transcription while preserving native precision. The provider-neutral residual handoff still closes all eight dimensions and carries scope, condition meaning, product/technical semantics, gaps, blockers, Source-item mappings and verification bindings that Figma does not own.

Those inputs remain ordinary Source. The consuming workflow makes selected targets Context-reachable, maps them through Source Claims and applicable Controls/`surface_bindings` to production owners and cold-start journeys, then requires independently failing `design_conformance`, interaction and target-runtime evidence through current-snapshot Contract Conformance or the Long-Task Final Gate. Mutable links, missing version/node identity, truncated inputs and unresolved coverage fail closed. Connector/extraction success, screenshots, hashes and handoff preflight prove acquisition or integrity only—not production conformance. The optional hybrid path therefore improves authoring cost and fidelity without a Figma-specific schema, registry, second Authority or second Gate; non-Figma resources remain fully supported.
...
For a selected implementation handoff, both development paths first run `ty-context design-resource preflight <handoff.md>`. One marked project-native Markdown Source is the residual handoff: it references addressable native facts instead of manually duplicating them and closes every in-scope subject across surface/flow, visual/content, component/control, state/interaction, motion, adaptation/input, accessibility and assets, with immutable evidence, Source-item and verification-method bindings. Missing dimensions, unsupported evidence, unresolved meaning or stale digests fail closed. Exploration remains schema-free. Preflight proves semantic-input completeness and resource integrity only; each workflow must still open the resources and prove the production implementation on the real entry.
...
It commissions only the smallest sufficient set through structured MCP, with bounded CLI/daemon and UI fallback. One page/prototype or component-family workbench may cover many items when its conditions are addressable and inspectable; repeated controls map to shared variants, while unique or complex uncovered controls may need dedicated state/interaction studies. A static/default frame never silently covers unseen state, interaction, motion, responsiveness or accessibility. A prototype, low/high-fidelity pair, component board, Figma-native input, one-file-per-control rule, variant count or directory is never universally required, and Tiny Context never copies Open Design prompts/templates or vendors a provider catalogue. Designs may express user-visible interaction semantics and the presentation of product rules, but business/data/permission/algorithmic rules remain owned by product/technical Source.

When Figma is supplied, requested or selected, the Skill reads the dedicated Figma-native profile. It feature-detects operational tools and auth, pins exact file/version/nodes/conditions, starts with metadata and small logical node sets, then retrieves design context, Variables and screenshots plus applicable motion, assets and Code Connect. It prefers Components/Variants, Variables/token mappings, Auto Layout, semantic naming and Annotations/dev resources when the upstream can be improved. Every selected result is frozen to repository-readable immutable inputs with hashes and a separate editable-upstream route. Mutable-only links, metadata-only or flattened substitutes, oversized/truncated extraction and unavailable plan-dependent capabilities remain explicit and cannot authorize fidelity.

Exploration returns the requested visible candidate after minimal sanity review and requires no handoff schema. After explicit or delegated final selection for implementation, the Skill performs one consolidated idempotent proposal reconciliation and writes one provider-neutral marked Markdown Source containing exactly one strict residual `design-resource-handoff-v1` block. It references addressable Figma-native facts when present, then records scope/provenance, immutable resource paths/digests and editable upstreams, conditions, grouped subjects, selected targets, complete eight-dimension coverage, residual product/interaction/accessibility meaning, Source-item/verification-method bindings and acceptance blockers. Shared preflight cannot call `decision_required`, `unavailable`, unsupported or stale input ready. There is no fixed directory, provider pack or one-file-per-control rule. The adapter is ordinary Source, not Design Authority or acceptance, and the Skill never edits a Source Plan, `project_context/**`, `DESIGN.md`, production code or a Delivery Contract.
