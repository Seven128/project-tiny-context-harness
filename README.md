...
`/prepare-composite-long-task` and `ty-context composite-campaign` provide a clean-start Campaign V5 orchestration plane over the unchanged Contract V3 Slice worker. `create --plan-file` preserves immutable `source-plan.md`; `apply-scope` accepts complete source coverage plus a maximal-cohesion Scope Fit V4 DAG and control/capability-level Source Unit inventory. Each Source Unit must map through SFC, Requirement, PI Obligation, AC and Verification Spec. Scope can split only at six declared semantic/capacity boundaries, rejects obvious `over_split_sfc`, and freezes permanently when the first Goal is set.

`run --campaign` launches the local Codex App Server over stdio JSONL. Each ready SFC receives one persistent thread: a read-only Authoring Turn uses the known controller model/effort and strict Packet output schema; after preflight, the same thread receives Goal Manifest V2, switches to its isolated worktree and starts an explicit execution-profile `workspaceWrite` Turn. Only catalog-proven Sol `xhigh|max` routes to Sol `medium`; lower, unrelated, unknown or unavailable profiles pass through. The whole wave starts before waiting, `needs_work`/interruption continues in the same thread, and merge/integration/final regressions use independent execution-profile repair threads.
