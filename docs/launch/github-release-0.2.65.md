...
```text
Use the commit that bumps `packages/ty-context/package.json` to 0.2.65 and is published to npm.
```

Title:

```text
Project Tiny Context Harness 0.2.65
```
...
````markdown
Project Tiny Context Harness 0.2.65 is the current public release line under the renamed npm package:
...
- Publishes `project-tiny-context-harness@0.2.65` through npm Trusted Publishing.
- Keeps the install path on the renamed package: `project-tiny-context-harness`.
- Adds two-document upstream input guidance for long target-mode tasks: use `Development Plan` as execution direction and `Acceptance and Tests` as acceptance authority.
- Tightens plan acceptance completion discipline: AC status starts from `unknown / not_run`, fresh required evidence is needed for completion, contradictions invalidate affected ACs, and UI-facing acceptance needs the real page path unless explicitly scoped out.
...
- This release is marked `sync-only`: default upgrade diagnostics remain recommended, while direct `sync` is only the shortcut for managed-asset refresh without migration diagnostics.
...
```sh
node tools/github_release_publish.mjs --version 0.2.65
```
...
2. Choose tag `v0.2.65`.
3. Confirm the target is the commit that was published to npm for `project-tiny-context-harness@0.2.65`.
4. Use title `Project Tiny Context Harness 0.2.65`.
...
- Do not retarget `v0.2.65` after the npm publish; it should point to the commit used by the published package.
- Do not claim benchmark wins or adoption in the release.
- Do not mark this as a pre-release if npm `project-tiny-context-harness@0.2.65` remains live and installable.
