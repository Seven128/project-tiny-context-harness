...
```text
Use the commit that bumps `packages/ty-context/package.json` to 0.7.4 and is published to npm.
```

Title:

```text
Project Tiny Context Harness 0.7.4
```
...
````markdown
Project Tiny Context Harness 0.7.4 is the current public release line under the renamed npm package:
...
- Refines repository test reruns so repair loops use failed/affected coverage, proven environment-only local failures may defer only to a guaranteed downstream complete gate, and partial reruns never become a complete-pass claim.
- Publishes `project-tiny-context-harness@0.7.4` with the synchronized package assets and CLI build.
...
```sh
node tools/github_release_publish.mjs --version 0.7.4
```
...
2. Choose tag `v0.7.4`.
3. Confirm the target is the commit that was published to npm for `project-tiny-context-harness@0.7.4`.
4. Use title `Project Tiny Context Harness 0.7.4`.
...
- Do not retarget `v0.7.4` after the npm publish; it should point to the commit used by the published package.
- Do not claim benchmark wins or adoption in the release.
- Do not mark this as a pre-release if npm `project-tiny-context-harness@0.7.4` remains live and installable.
