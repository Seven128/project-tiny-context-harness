...
```text
Use the commit that bumps `packages/sdlc-harness/package.json` to 0.2.51 and is published to npm.
```
...
```sh
node tools/github_release_publish.mjs --version 0.2.51
```
...
2. Choose tag `v0.2.51`.
3. Confirm the target is the commit that was published to npm for `project-tiny-context-harness@0.2.51`.
4. Use title `Project Tiny Context Harness 0.2.51`.
...
- Do not retarget `v0.2.51` after the npm publish; it should point to the commit used by the published package.
- Do not claim benchmark wins or adoption in the release.
- Do not mark this as a pre-release if npm `project-tiny-context-harness@0.2.51` remains live and installable.
