...
```text
Use the commit that bumps `packages/ty-context/package.json` to 0.2.62 and is published to npm.
```

Title:

```text
Project Tiny Context Harness 0.2.62
```
...
````markdown
Project Tiny Context Harness 0.2.62 is the current public release line under the renamed npm package:
...
- Publishes `project-tiny-context-harness@0.2.62` as the current public npm package.
- Keeps the install path on the renamed package: `project-tiny-context-harness`.
- Tightens the plan acceptance checklist compiler so an explicit concrete checklist already present in a plan is reused verbatim as the full checklist instead of generating a competing checklist.
- Keeps the copied plan and reused full checklist as separate temporary files under `tmp/ty-context/plan-acceptance/**`.
- Documents the same plan-provided checklist behavior in the English and Chinese READMEs, package README, managed Skill and workflow Context.
...
- Keeps this release `sync-only`: no new migration is expected, but `ty-context upgrade --check` / `ty-context upgrade` remains the default post-update diagnostic path.
...
```sh
node tools/github_release_publish.mjs --version 0.2.62
```
...
2. Choose tag `v0.2.62`.
3. Confirm the target is the commit that was published to npm for `project-tiny-context-harness@0.2.62`.
4. Use title `Project Tiny Context Harness 0.2.62`.
...
- Do not retarget `v0.2.62` after the npm publish; it should point to the commit used by the published package.
- Do not claim benchmark wins or adoption in the release.
- Do not mark this as a pre-release if npm `project-tiny-context-harness@0.2.62` remains live and installable.
