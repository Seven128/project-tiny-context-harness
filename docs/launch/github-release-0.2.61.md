...
```text
Use the commit that bumps `packages/ty-context/package.json` to 0.2.61 and is published to npm.
```

Title:

```text
Project Tiny Context Harness 0.2.61
```
...
````markdown
Project Tiny Context Harness 0.2.61 is the current public release line under the renamed npm package:
...
- Publishes `project-tiny-context-harness@0.2.61` as the current public npm package.
- Keeps the install path on the renamed package: `project-tiny-context-harness`.
- Adds bounded Source Pack exports: `export-context --source-pack`, `--code-index` and `--task-context`, with manifest metadata, deterministic code indexes, redaction and a five-file cap for upload-ready packs.
- Tightens plan acceptance checklist prompts around the full checklist as the acceptance authority, compact prompt summaries as navigation, and current-state evidence ledgers before completion claims.
...
```sh
node tools/github_release_publish.mjs --version 0.2.61
```
...
2. Choose tag `v0.2.61`.
3. Confirm the target is the commit that was published to npm for `project-tiny-context-harness@0.2.61`.
4. Use title `Project Tiny Context Harness 0.2.61`.
...
- Do not retarget `v0.2.61` after the npm publish; it should point to the commit used by the published package.
- Do not claim benchmark wins or adoption in the release.
- Do not mark this as a pre-release if npm `project-tiny-context-harness@0.2.61` remains live and installable.
