...
| Prelaunch external blockers | Keep [prelaunch-external-blockers.md](prelaunch-external-blockers.md) aligned with current `npm-fetch` and `github-homepage` TODOs. | Broad launch is held until `node tools/launch_readiness_check.mjs --strict-external` no longer reports either blocker. |
