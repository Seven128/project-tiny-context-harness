...
```text
Use the commit that bumps `packages/ty-context/package.json` to 0.2.66 and is published to npm.
```

Title:

```text
Project Tiny Context Harness 0.2.66
```
...
````markdown
Project Tiny Context Harness 0.2.66 is the current public release line under the renamed npm package:
...
- Publishes `project-tiny-context-harness@0.2.66` through npm Trusted Publishing.
- Keeps the install path on the renamed package: `project-tiny-context-harness`.
- Tightens plan acceptance two-document input handling: incomplete `Development Plan` / `Acceptance and Tests` packets now stop with missing required fields instead of generating inferred checklist rows.
...
```sh
node tools/github_release_publish.mjs --version 0.2.66
```
...
2. Choose tag `v0.2.66`.
3. Confirm the target is the commit that was published to npm for `project-tiny-context-harness@0.2.66`.
4. Use title `Project Tiny Context Harness 0.2.66`.
...
- Do not retarget `v0.2.66` after the npm publish; it should point to the commit used by the published package.
- Do not claim benchmark wins or adoption in the release.
- Do not mark this as a pre-release if npm `project-tiny-context-harness@0.2.66` remains live and installable.
