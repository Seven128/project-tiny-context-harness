...
async function validateBinding(active, git) { const repositoryRoot=git.worktreeRoot; const standalone=active.campaign_id===null&&active.slice_id===null; const campaignSlice=typeof active.campaign_id==="string"&&active.campaign_id.length>0&&typeof active.slice_id==="string"&&active.slice_id.length>0; if (active.schema_version !== "active-long-task-binding-v4" || (!standalone&&!campaignSlice) || active.worktree_id!==gitWorktreeId(git.gitDir) || !samePath(active.repository_root || "",repositoryRoot) || !active.workdir || !active.contract_sha256 || !active.hook_bundle_sha256 || !active.verifier?.cli_path || !active.verifier?.cli_sha256) failClosed("Invalid composite long-task active binding"); const actualWorkdir=await realpath(active.workdir).catch(()=>null); if (!actualWorkdir || !samePath(actualWorkdir,active.workdir) || !inside(repositoryRoot,actualWorkdir)) failClosed("Composite workdir is missing, retargeted or outside its repository"); const contract=await optionalJson(path.join(actualWorkdir,"compiled-contract.json")); if (!contract || contract.contract_sha256!==active.contract_sha256 || !samePath(contract.repository_root||"",repositoryRoot) || !samePath(contract.workdir||"",actualWorkdir)) failClosed("Composite compiled contract does not match the active binding"); const cliHash=createHash("sha256").update(await readFile(active.verifier.cli_path).catch(()=>Buffer.alloc(0))).digest("hex"); if(cliHash!==active.verifier.cli_sha256) failClosed("Composite verifier identity changed after activation"); const scriptHash=createHash("sha256").update(await readFile(new URL(import.meta.url))).digest("hex"); const configHash=createHash("sha256").update(await readFile(path.join(repositoryRoot,".codex","hooks.json"))).digest("hex"); if(createHash("sha256").update(`${configHash}:${scriptHash}`).digest("hex")!==active.hook_bundle_sha256) failClosed("Composite Hook bundle changed after activation"); }
