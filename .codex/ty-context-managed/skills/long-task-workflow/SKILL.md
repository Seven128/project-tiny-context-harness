...
Map every marked Source item to exactly one canonical target of the same semantic kind and normalized text. A non-decision disposition owns exactly one target, and two Source items may not own the same target. Requirements, independently decided control fields, obligations and non-completing statements map respectively to Requirement, Control, Obligation and Non-completing non-Result Claims. Outcome acceptance maps to exactly one `<outcome>.<check>.<assertion>` whose criterion exactly matches the Source item and whose non-Result Claim set contains at least one Claim independently backed by a Requirement, Control, Obligation or Non-completing Source item. Global acceptance maps to `GLOBAL.<check>.<assertion>`, preserves the same exact criterion identity, proves only Global Claims, and must include at least one `non_goal.<key>`, `constraint.<key>` or `forbidden_shortcut.<key>` independently backed by another Source item. An explicit overall result uses `outcome_result`. A `risk_fact` marker must contain exact `fact=<fact-name> outcome=<outcome-key>` attributes and its disposition must name that same existing Fact/Affected-Outcome pair; each pair has one Source owner. If either value cannot be established, use `decision_required` rather than a weaker guess. Declared non-goals map to covered Global non-goal Claims; external confirmations and genuine decisions use their dedicated dispositions. `out_of_scope` is retired: an old occurrence is an error, and excluding an in-scope requirement requires `decision_required`.
...
The evidence adapter is derived from the runner: `playwright_test` is `playwright_json_v1` and may prove `ui_browser`; all other runners are `structured_json_v2` and may prove only non-browser surfaces. Across all Checks sharing one Raw Execution identity, one claim-bearing Observation belongs to one Assertion. A Playwright claim-bearing Assertion has exactly the canonical form `playwright.case.<ac-key>.passed equals true`; title each claim-bearing Test `[ac:<assertion-key>] ...`. One Test Instance may bind at most one declared AC. Undeclared tags such as `[smoke]` are ignored; legacy `[<key>]` is recognized only for a declared AC. Missing, skipped, flaky, unexpected, timed-out, interrupted, multi-AC or duplicate-per-project AC instances fail closed, while the same AC across distinct Playwright projects aggregates successfully only when every instance passes. Report-error, declared/unbound-unexpected and per-AC unexpected/timeout/interruption counts are decoder-only Counterfactual diagnostics and cannot prove Claims.

Every Claim-bearing `structured_json_v2` Check must demonstrate same-Check sensitivity for each related Claim through Counterfactuals, except Claims covered by that same Check's Population under normal observability. Under `weak_observability`, Population removes no sensitivity obligation. Counterfactual coverage may be the union of multiple controls, and one control may cover multiple related Claims, but controls on another Check and unrelated Claims or Artifacts do not count. A structured Result assertion additionally needs a same-Check failing Counterfactual rooted in `result` plus at least one related non-Result Requirement, Control, Obligation, non-completing or forbidden-shortcut Claim. Global structured Checks use `global.acceptance.counterfactual_controls`; each control names `binding_ref: <outcome-key>.<binding-key>`, covers Claims and Assertions from its own Global Check, and mutates only that owning Outcome Binding's carriers. Global Bindings do not exist.

Standard Playwright Check content is frozen trusted verifier input, so a normally observable `playwright_test` does not require a Counterfactual. When an affected Outcome declares `weak_observability`, every claim-bearing Playwright AC and each of its Claims requires same-Check Counterfactual sensitivity; another Check or a partial AC set cannot satisfy it. Changing frozen Playwright files or their resolved inputs after Authority Lock creates staleness/revision as applicable.
...
Outcome Counterfactual V2 names an Outcome `binding_key`; Global Counterfactual V2 names an Outcome-owned `binding_ref`. Both mutate only a proven subset of that Binding's carriers, never Source, Context, runners or verification inputs. `structured_json_v2` still requires completed exit zero plus only designated `assertion_value_mismatch` failures. A `playwright_json_v1` Counterfactual may instead accept exit one only when the parsed report has no root error and the complete unexpected Test Instance set is uniquely explained by the designated executed ACs; every unspecified declared AC must pass, and missing/skipped/flaky/timed-out/interrupted ACs, unbound or extra Test failures, artifact/population/environment/other Evidence failures, exit zero with unexpected ACs and exit codes above one all fail closed. Only after that exact classification may the generic non-Assertion `test_failed` be removed and designated `acceptance_case_unexpected` Findings normalize to `assertion_value_mismatch`. Ordinary Playwright Baseline Checks still require exit zero. An `existing` mutation target must exist at Preflight/Compile. A `planned` mutation target may be absent until implementation, but the Live Final Gate fails closed if it is still absent; once created, its changes invalidate targeted Progress. Population V2 proves exact eligible = observed + valid exclusions by entity id. Artifact collection remains useful evidence output but never substitutes for the per-Check sensitivity rule above.
