...
Do not hand-write the three YAML projections. Package rendering from the V3 packet is their only campaign projection path.

## Start And Continuation

- On explicit start with no bound Goal, read `goal-objective.txt`, call `create_goal` with the complete objective, and only after success run `start --campaign <path> --sfc <id> --goal-id <id>`.
