...
14. 长程任务不要靠宽泛关键词自动触发临时验收流程；需要普通长程任务包时，建议用户直接调用 `/normal-long-task`；只有原始需求、需要 Scope Fit / 稳定 SFC 拆分和三输入准备时，直接调用 `/prepare-composite-long-task`；已有 V2 Product Source、Technical Realization Plan 和 Acceptance Checklist 且需要独立验证器与完成硬门时，显式调用 `/composite-long-task-workflow`。准备 campaign 是 opt-in 的用户自有 authoring/provenance；运行状态、验证产物和最终结果只放 `tmp/ty-context/plan-acceptance/**`，不注册到 `project_context/**` / `project_context/context.toml`。
