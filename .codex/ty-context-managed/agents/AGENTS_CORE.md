...
14. 长程任务不要靠宽泛关键词自动触发临时验收流程；需要普通长程任务包时，建议用户直接调用 `/normal-long-task`；已有讨论方案、需要完整 Scope Fit V3 DAG、ready-frontier 三输入、保守并行 worktree/Goal、自动合并修复和最终同快照验收时，显式调用 `/prepare-composite-long-task`，该显式调用即授权完整 prepare-and-execute Campaign V4 循环；已有完整 Contract V3 Product Source、Technical Realization Plan 和 Acceptance Checklist 且只需执行一个 Slice 时，显式调用 `/composite-long-task-workflow`。Campaign 是 opt-in 的用户自有 source/orchestration/provenance，不是默认 lifecycle state；mutable contract/run/log/lock/workdir 只放 `tmp/ty-context/plan-acceptance/**` 等临时执行存储，Campaign 文件和任何临时产物都不注册到 `project_context/**` / `project_context/context.toml`。普通调度、Git 冲突、测试失败和可恢复 Goal 中断不询问用户，只有产品语义决策或真实外部阻塞才暂停。
