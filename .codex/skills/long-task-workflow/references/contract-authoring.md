...
- for each selected exact/constraint target inside a surface binding, declare immutable `source_paths`, condition keys, bound Control Claim refs, one root conformance Check/Assertion and distinct current actual/comparison artifact paths. The Assertion requires all-of `design_conformance`, `interaction_trace` and `target_runtime`, and its current proof includes a typed `design_conformance` record; `visual_render`, file hashes, counts or registry presence remain resource/render integrity and cannot substitute for implementation conformance;
