...
6. `upgrade` 只运行安全 migrations 和 `sync`；Schema v4 可自动创建缺失的 `project_context/context.toml` 并把 legacy `project_context/modules/**/*.md` 迁移到 `project_context/areas/**/*.md`，但不得自动改写用户 Context Markdown 或重新引入阶段式语义迁移入口。
