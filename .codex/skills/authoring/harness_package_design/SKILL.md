...
22. 修改 Long-Task Schema、Contract Authoring、Source Authority、Claim/Assertion、Evidence Kernel、Final Gate、Stop/close、status/resume、Authority Revision、scope/risk、Skill、Hook、profile/distribution、recovery 或 performance 机制前，必须先读 `PROJECT_SPEC.md` 的 Long-Task Workflow Controlling Objective、Authority Scope And Trusted Results、Mechanism Admission Rule，`project_context/areas/harness-package/decision-rationale/long-task-workflow.md`，以及 owning contract/verification Context。每个新增、删除或强化的机制都要在内部判断：关闭哪条具体 false-completion/delivery-drift path；建立什么 invariant；什么测试或当前快照证据证明它；已有机制是否 overlap；删除后重开什么路径；增加多少 Authoring/Runtime/State/Recovery/maintenance cost；独立防漂移收益是否高于成本；是否 fail closed；是否创建 second Authority、second plan 或 scheduling plane。这是设计与代码 Review 原则，不生成 Mechanism Matrix、Receipt、Registry 或 runtime state；文档措辞调整不得伪装成 runtime 机制。
...
- [ ] Long-Task 机制变更已读取 controlling objective、Authority Scope、Mechanism Admission、rationale 与 owning contract/verification Context，并完成 false-completion path、invariant、proof、overlap、deletion risk、cost/net benefit、fail-closed 和 second Authority/plan/scheduler 检查；未创建 matrix、Receipt、Registry 或 runtime state。
