...
- when a selected implementation handoff exists, run `ty-context design-resource preflight <handoff.md>` before fidelity implementation. Open the strict block and every indexed exact/constraint resource; route each covered Source Item and verification method into the internal implementation plan and project-owned checks. Missing dimensions, unsupported evidence, unresolved rows or stale digests fail closed. Preflight proves input completeness/integrity only, not the implementation;
