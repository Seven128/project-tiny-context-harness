...
- **Workflow contract**: prompt-level order of thought that the agent is expected to follow so Context, code, tests, temporary plans and one-off evidence are handled in the right authority order. It is a soft constraint, not a validator, phase gate or machine-enforced edit-order gate.
...
- **Temporary plan surface**: `plan.md` or an equivalent scratchpad used only when complex work needs visible execution state. It makes the workflow contract visible, executable and checkable for the current execution slice; it serves the workflow contract and Context without replacing either.
...
- Allow `plan.md` or an equivalent temporary plan surface to assist complex task-contract work as scratch space only; it must serve Context and the workflow contract without becoming either one, and it must not become a validator, gate or restored SDLC plan protocol.
...
- It is recommended through explicit `/superpowers-long-task` invocation when Product / Architecture Source, Technical Realization Plan and Acceptance Checklist already exist and the next step needs Superpowers execution.
...
- It requires `plan-conformance-matrix.*` to be initialized early and updated during execution. This matrix answers whether implementation followed the Technical Realization Plan across code, API/schema, UI/page, worker/runtime, artifact, data, tests and evidence. Status values distinguish `complete`, `partial`, `sampled_only`, `not_implemented`, `blocked`, `scope_changed_requires_user_approval` and `contradicted_by_current_state`.
- It requires `final-acceptance-verdict.*` to be generated only at the final gate. This verdict judges each AC as `complete`, `partial`, `blocked`, `not_run` or `invalidated` using fresh evidence and current contradictions.
...
- Overall completion requires both gates: no blocking implementation drift in the plan-conformance matrix and all required ACs complete in the final acceptance verdict.
