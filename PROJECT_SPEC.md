...
One design-specific Long-Task objective is that Agent implementation, acceptance and testing fully conform to every material UI/UX fact that selected design resources explicitly express within their declared scope and conditions. Open Design HTML, images and prototypes remain the primary visual resources, but passing them directly to an Agent or Contract is a lossy boundary: layout, control styling, states, interaction, motion, adaptation/input, accessibility and assets may be overlooked or reinterpreted. `design-resource-authoring` therefore adds a structured textual semantic handoff as part of the selected implementation design-resource set. It records only facts that the resources explicitly specify or demonstrate; missing meaning must be refined, decided or blocked rather than guessed. Long-Task then carries those facts through Source Claims, Controls, project-owned Checks and the current-snapshot Final Gate so development, acceptance and testing form one traceable conformance chain.
...
The shared design-resource handoff adapter satisfies the same admission rule one boundary earlier. Open Design HTML, images, prototypes and accompanying specifications can collectively carry rich layout, component, state, interaction, motion, responsive/input, accessibility and asset meaning, but direct visual-only consumption is not a reliable Contract interface: a prose summary or generic `condition_keys` can silently omit or flatten a category before implementation begins. One strict `design-resource-handoff-v1` block inside ordinary marked Markdown Source makes the textual supplement part of the selected implementation resource set and closes that independent false-completion path for both consumers. It expands grouped subjects across eight closed dimensions, binds covered facts to immutable evidence, Source Items and project verification methods, and fails on unresolved/stale/substituted evidence. Exploration pays no cost, and implementation handoffs pay one compact index rather than one artifact per control. It adds no Design Authority, registry, workflow state, Receipt or Gate; direct image inspection and a prompt-only reminder were rejected because neither makes omissions traceable or testable, while a provider-specific pack was rejected for duplicate ownership.
