...
`/design-resource-authoring` is an independent explicit-trigger upstream Skill, not a stage inside Source Plan or Long-Task authoring. It accepts the same raw drafts and mixed product/technical/design inputs directly or an optional existing Source Plan, fixes the requested output as a hard scope ceiling, discovers live Open Design agents/models/skills/templates/design systems/plugins/export routes and assigns each considered resource `selected`, `optional`, `not-needed`, `unavailable` or `decision-required`. It commissions only the smallest sufficient resources through structured MCP, then bounded CLI/daemon and UI fallbacks; it never copies provider prompts or vendors a template catalogue. Exploration stops after a visible scoped candidate and minimal sanity review; handoff adds declared coverage/provenance; selected-source preparation requires explicit human selection and immutable identity/snapshot but creates no authority. Candidate iteration does not rewrite the initial proposal. The Skill may retain task-local accepted/rejected/unresolved deltas and consolidate them once after the direction is final; a separately authorized owner decides whether and when to revise the proposal. A later explicit `/source-plan-authoring` call may consume the separately revised proposal and selected resources together, but neither Skill invokes the other.
