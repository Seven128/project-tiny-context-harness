...
Preparation and execution remain distinct. `/prepare-composite-long-task` owns Scope Fit and just-in-time authoring for one dependency-ready `SFC-###`; its V3 packet deterministically renders the three strict Contract V3 YAML inputs and stops at `handoff_ready`. `/composite-long-task-workflow` consumes those inputs without inferring or repairing missing product, plan, acceptance or oracle meaning.
...
The runtime therefore has one path with four costly operations, and each must reduce a named drift risk: contract compilation prevents omitted or unverifiable requirements and freezes authority hashes; implementation–verification iteration finds behavioral and scope drift before the end; final full recomputation prevents stale, partial and historical evidence; Codex Stop freshness enforcement prevents a model completion claim from bypassing final gate. Slice, epoch, progress-ledger, local-audit, review, TDD, next-slices, matrix/verdict and final-card processes are not package responsibilities.

Completion follows a strict separation of authority. The frozen three-input contract and Context define the complete Requirement/PI/Obligation/Binding/AC/Proof/Spec/Counterfactual graph. A pinned compiler and verifier execute frozen specs, collect their own observations and artifacts, and calculate findings. Final gate alone calculates `accepted` from a current full recomputation. Stop Hook checks that the accepted result and workspace identity are still current before deciding whether Codex may end. Agent prose, hand-written JSON, agent-selected pass results, old attempts, screenshots alone, file existence, generated summaries and post-compile modified acceptance tools never issue completion proof.
...
- Allow an explicit `/prepare-composite-long-task` Skill upstream of composite execution when work starts from a raw multi-SFC requirement. Its tracked campaign is limited to user-owned authoring/provenance, authors only the current SFC through `CompositeAuthoringPacketV3`, deterministically renders the three Contract V3 YAML authorities, requires complete graph and oracle-ready acceptance specs before handoff and never creates runtime proof or aggregate product completion.
...
- Only the current SFC is authored just in time. `CompositeAuthoringPacketV3` is the structured authoring source; package code, not a copied free-form prompt, deterministically renders the three strict Contract V3 YAML authorities and runs full-graph/oracle-ready stateless preflight.
...
- The storage model has two planes. Git-trackable user-owned campaign data preserves authoring/provenance; compiled contracts, command runs, artifacts, current status and final results stay in ordinary user/project execution storage. V3 intentionally has no aggregate campaign product-completion state.
...
- `record-result` mirrors only a matching current `final-result.json` identity and hashes. After an accepted result is recorded, or no active Goal remains, the next SFC is refreshed against current Context/code; the campaign never pre-authorizes stale downstream work. A user decision communicated outside the runtime does not record completion.
- No historical attachment, partial bundle, V1/Markdown campaign, old temporary workdir or campaign importer is added. Their authority/schema cannot be proven consistently and compatibility would permanently weaken the strict executor; only complete Contract V3 YAML bundles keep the direct `/composite-long-task-workflow` route.
