...
Preparation and execution remain distinct. `/prepare-composite-long-task` is explicit upstream authoring: it performs Scope Fit, gives selected slices stable `SFC-###` identities, authors only the current dependency-ready SFC through `CompositeAuthoringPacketV1`, deterministically renders the three inputs and stops at `handoff_ready`. `/composite-long-task-workflow` remains strict and does not infer or repair those inputs. Handoff freezes source identity but does not create a Goal; explicit start binds one Goal to one SFC.
...
Strict final completion is recompute-only, current-attempt-only and owned by the Trusted Evidence Kernel shared by final gate, `validate-superpowers-state`, state-backed `validate-plan-acceptance` and derived completion views, then by the completion-output resolver and blocker triage for official generated final-answer status. `compile` derives required command specs from AC proof layers, assertion commands, assertion artifacts, required test ids, positive/negative assertions, invalid completion signals and expected final evidence; `start-attempt --mode product_task|harness_task` records current source hashes, git/worktree identity and mode; `run-assertion` records command runs; `record-evidence` registers canonical EvidenceRecordV2 artifacts into state; and `final-gate` recomputes AC/PI/final status from current v2 proof plus contradiction scans. EvidenceRecordV2 carries attempt/source/product/plan/checklist hashes, git head, worktree fingerprint, command spec/run ids, command line and exit code, artifact path/SHA/mtime, target AC ids, target PI ids, target proof layers, assertion status/exit code, positive assertions, negative assertions, invalid completion signals, negative evidence scan and required test ids. Legacy v1 evidence remains audit/history material unless registered into the current attempt with source/worktree/command/artifact/assertion fields. `derived/**`, `events.ndjson`, matrix/verdict/evidence-index/final-summary, validator pass, final cards, auditor prose, summary-only AC proof, unregistered JSON, old final/gates/meta transient findings and hand-written status files cannot substitute for current assertion-backed evidence or authorize `completion_output_status=accept`. Newer failed command runs, Playwright/JUnit/test-result artifacts, owner DOM forbidden states, task-state false/partial values, source/worktree drift and derived/state contradictions block completion for the affected AC/layer.

Final-gate recomputation has a fixed order: load the three inputs, recompute source hashes, load task state, snapshot previous final/gates/meta transient bookkeeping as audit-only, resolve the current attempt, load required command specs, load command-run records, load registered EvidenceRecords, discard stale evidence, scan unregistered assertion JSON, run contradiction scan, run AC-010 bootstrap prevention, run under-specified AC checks, run Harness Drift Lock, run protected baseline guard, recompute every AC, recompute every PI, recompute `acceptance_target_status`, recompute `product_goal_complete`, build the current candidate state, resolve candidate `completion_output_status`, regenerate current `derived/**`, scan generated output in current-candidate mode, classify `blocker_triage`, self-recover once when the mismatch is transient/bookkeeping or regenerable generated output, write current final state and append an event. Old `completion_output_status=blocked` or old `generated_output_mismatch=true` cannot drag a current kernel accept back to blocked, while current user-visible false completion claims still block. Machine-blocking ACs that cannot provide a concrete assertion command, artifact, positive/negative assertions, invalid completion signal checks, required UI/browser proof or assertion result become `under_specified`; their PI is blocked and `product_goal_complete=false`. AC-010/final-gate summary proof is never bootstrap authority: if it passes while another required AC is missing, failed or stale, AC-010 is invalidated with `final_gate_cannot_bootstrap_from_summary_only`.

Harness Drift Lock prevents a product task from proving completion with a modified acceptance toolchain. In `product_task` mode, changes to Playwright specs, tests, assertion generators, AC010 helpers, evidence writers, final-gate, validator, derive, task-state reducer, composite workflow Skill/protocol or related Makefile/package test targets block completion with `harness_drift_detected`, `acceptance_target_status=blocked`, `product_goal_complete=false` and the required split-task message. In `harness_task` mode, harness changes require adversarial fixtures with explicit expected final-gate results covering stale evidence, historical complete, derived contradiction, AC010 summary-only, target mismatch, API-only-for-UI, negative evidence after pass, source hash mismatch, dirty worktree mismatch, missing assertion_result, test weakening and a happy path; passing those fixtures proves the harness only. HFC-004 adds a durable final-gate blocker-triage suite for old transient self-lock, candidate-driven scanner mode, blocker categories, next actions and one-pass self-recovery. `protected-harness-baseline.json` records the protected final-gate, validator, derive, evidence registration, assertion schema, fixture expected outcomes, workflow protocol, Skill markdown and test runner script surfaces; product tasks changing them are blocked, while harness tasks changing them need a baseline reason and fixture verification.
...
- Allow an explicit `/prepare-composite-long-task` Skill upstream of composite execution when work starts from a raw multi-SFC requirement. Its tracked campaign is limited to user-owned authoring/provenance, it authors only the current SFC through a structured packet, package code renders the three inputs, preflight is stateless, handoff never starts a Goal and execution/evidence remains under `tmp`.
...
- Only the current SFC is authored just in time. `CompositeAuthoringPacketV1` is the semantic source; package code, not a copied free-form prompt, deterministically renders Product / Architecture Source, Technical Realization Plan and Acceptance Checklist and runs stateless preflight.
...
- The authority ladder is fixed: immutable sanitized `request.md`; current `campaign.yaml` coordination; append-only campaign `events.ndjson`; `authoring-packet.json` as the only editable authoring source before handoff, with every change creating a new immutable revision; deterministic rendered three-input projections; post-start `tmp` `task-state.json`; then the current final gate as the only SFC product-completion authority. Validators, audit, matrix/verdict views, campaign status and recorded prose cannot promote completion.
- The storage model has two planes. Git-trackable user-owned campaign data preserves authoring/provenance; Goal attempts, runtime state, logs, evidence and derived views stay under `tmp`. v1 intentionally has no aggregate campaign product-completion state.
- Preparation ends at `handoff_ready`. Handoff freezes matching hashes and never creates a Goal. Only explicit `start` or `prepare and execute` authorization creates an idempotent one-Goal/one-SFC binding; Goal execution rereads current Context/code and resolves any candidate Context Delta before implementation.
- `record-result` mirrors only a matching current final-gate identity and hashes. After the result is recorded or no active Goal remains, the next SFC is refreshed against current Context/code; the campaign never pre-authorizes stale downstream work.
- No historical attachment, partial bundle, old temporary workdir or campaign importer is added. Their authority/schema cannot be proven consistently, an importer would impose permanent compatibility cost, and already-complete three-input bundles keep the existing `/composite-long-task-workflow` route.
...
- Its final wording separates `audit_task_complete`, `acceptance_target_status`, `product_goal_complete` and `completion_output_status`. A Superpowers implementation / execution Goal mode objective may use an accept/completion final answer only at `product_goal_complete=true` and `completion_output_status=accept`; a completed audit workflow or generated verdict is not product completion. A deliberately read-only audit / reporting goal may end at `audit_task_complete`, but when the verdict is not accepted/complete final responses must say `Audit workflow completed; acceptance target not complete.`, include available AC counts and avoid unqualified `Goal achieved` or treating `update_goal(status="complete")` as acceptance of the user target.
