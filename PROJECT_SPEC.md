...
2. **Workflow Contract** defines the lightweight default loop: core/default Context, manifest routing plus one bounded Context search, one `Context Delta`, platform-internal planning, implementation, project verification, Contract Conformance and Context drift checking.
...
The optional **Source Plan** authored by `source-plan-authoring` is an upstream Source-quality aid. It accepts refinement, mixed-input synthesis and hybrid authoring without requiring a structured intake form. It inventories and fully accounts for supplied artifacts, preserves direct requirements and qualifiers, makes necessary derivations traceable, and may record low-impact reversible product choices as `delegated` when the user explicitly asks it to synthesize or elaborate with its own judgment. Permissions, destructive behavior, pricing/quota, security/legal policy, persistence/retention, external automation, business thresholds, platform scope and population scope remain `decision_required` unless an authoritative input decides them.
...
- Product, acceptance or architecture semantic conflicts pause for user decision and are not disguised as implementation failures.
...
`/source-plan-authoring` is an explicit-trigger upstream Skill. It accepts a near-complete plan or a sparse brief with mixed attachments and needs no structured intake form. It produces one self-contained Markdown document with complete input coverage; direct requirements retain qualifiers, necessary derivations cite their source, explicitly delegated low-impact/reversible elaboration records its instruction and basis, and unsupported high-impact product semantics become `DEC`/`decision_required`. Outcomes split by independently decidable results, interactive work reaches surface/region/control/state/feedback granularity, NCOMP preserves non-completing meaning, Risks use exact Runtime Fact names and ACs name their accepted REQ/CTRL/OBL/NCOMP keys. It creates no Source Plan Schema, CLI, Preflight, Compile, Receipt, cache, Authority, state, repository binding, implementation or completion result.
...
`context_uiux_design` owns portable visual-design judgment without creating a visual workflow. For material design-system, redesign or high-fidelity work it keeps a task-local, risk-proportional **Visual Coverage Set** across production surface/component, viewport, theme/mode, interaction/state, content stress and accessibility/motion conditions. The set is internal planning, not a required file, Cartesian matrix, Context role or authority. Durable surface and interaction responsibility remains in `project_context/**`; durable visual-system semantics and rationale remain in `DESIGN.md`. The project identifies one authored exact token source and generation direction so generated CSS/theme files do not become competing authored truth.

`context_development_engineer` carries that intent into implementation by locating the production token/component/route owners, preventing an undeclared raw-value or detached-demo bypass, and running project-owned rendered verification on the combinations actually claimed. Static kits, mocks and marketing specimens may inform review but cannot substitute for production components or real product routes. Both Skills report only combinations actually checked; omitted combinations remain unproven.
...
Visual Long-Task delivery is likewise an authoring/evidence specialization, not a new mechanism. Material visual expectations are written as existing atomic Requirements, applicable Controls and named Assertions, with the surface, viewport, theme/state/content condition and observable result made explicit. Existing `ui_browser` Checks prove declared browser ACs; acceptance-affecting screenshot baselines are verification inputs, generated screenshots/diffs remain review artifacts, and subjective direction or approval stays an external confirmation or genuine decision. This adds no visual Schema, Claim type, risk level, lifecycle state, baseline authority, Gate or required artifact, and it cannot infer completeness beyond declared coverage.
