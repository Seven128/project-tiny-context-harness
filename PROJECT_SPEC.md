...
- For two-document input, the acceptance-and-tests document is reused directly only when it is already complete enough for target-mode execution; otherwise it is source material for generated AC rows with required evidence, verification methods, fail conditions, invalid evidence and state-machine rules.
- If the source plan contains explicit test requirements, it treats them as plan-provided acceptance evidence inside the full checklist. Test requirements belong to acceptance evidence, not a fourth artifact, so the compiler does not create `<plan-slug>-test-requirements.md`.
