...
-> explicit Long-Task Workflow Preparation Skill (`/prepare-composite-long-task`) can preserve a discussed multi-SFC plan as one complete Campaign V6 graph, use one foreground scheduler plus bounded ephemeral Codex Exec workers, schedule detached worktrees and drive integration/finalization
-> Long-Task Workflow Skill (`/composite-long-task-workflow`) compiles Product / Architecture Source + Technical Realization Plan + Acceptance Checklist into one immutable machine contract
...
Campaign orchestration and Slice execution remain distinct authorities. `/prepare-composite-long-task` owns immutable plan/Source Unit coverage, one maximal-cohesion Scope Fit V4 DAG, just-in-time read-only Packet authoring, routed detached SFC execution, positive-evidence wave scheduling, durable-stage worktree recovery, deterministic repair, Wave Integration Gates, one-snapshot Campaign Final Gate and target finalization. Packet, three Contract V3 inputs, Change Envelope and Receipt provide the authoring/execution authority handoff; conversational continuity and process identity do not. `/composite-long-task-workflow` consumes one Slice's three inputs without inferring or repairing missing product, plan, acceptance or oracle meaning.
...
Each Slice has three mandatory safety operations: contract compilation prevents omitted or unverifiable requirements and freezes authority hashes; Slice final full recomputation prevents stale, partial and historical evidence; Codex Stop freshness enforcement prevents a model completion claim from bypassing the Slice gate. Targeted `verify` is an optional repair accelerator and never an acceptance operation. Campaign V6 adds only composition/scheduling operations that close named risks: Scope Fit V4 prevents semantic over-splitting; alias-aware, minimum-effort policy-frozen routing prevents guessed downgrades; direct bounded children with PID/start-identity ownership and durable-stage reconciliation prevent abandoned work or PID-reuse mis-kills; a canonical state-derived worktree set plus pure budget assertion and explicit reconcile prevents implicit deletion; committed Slice heads and persisted Integration HEAD preserve recovery authority; monotonic Slice/Wave state prevents accepted re-execution; conservative scheduling prevents unsafe parallelism; hard managed-worktree budgets prevent resource growth; explicit terminal abandon bounds destructive cleanup to package-owned local assets; Integration Gates catch merged-wave regressions; and one-snapshot Campaign Final Gate plus target resynchronization prevents historical Slice results from masquerading as final delivery. Generic epoch/progress-ledger/local-audit/review/TDD/matrix/verdict/final-card processes are not package responsibilities.
...
- **Validator / gate**: machine checks for Context recoverability/path safety, generated-asset consistency, fake verification-claim prevention, touched-source modularity and explicit Composite authority. Both fixed Plan Validator commands are removed; validators do not prove product quality or edit order.
...
- Allow an explicit `/prepare-composite-long-task` Skill upstream of composite Slice execution when work starts from a discussed multi-SFC requirement. Explicit invocation authorizes the full Campaign V6 loop. Tracked user-owned orchestration/provenance preserves immutable source/Source Unit coverage, maximal Scope Fit V4 graph, Packet/schedule revisions, `codex-exec-v1` policy/profile/worker observations, commit/Receipt identities and verifier-derived integration/final results. One foreground scheduler uses bounded ephemeral authoring/execution/repair workers, preflights Contract V3 before execution, conservatively schedules detached worktrees, integrates/repairs them, and derives completion only from current Slice/Wave/final gates plus target integration. Accepted V5 is audit-only and unfinished V5 cannot execute.
...
- Remove both fixed Plan Validator commands and their matrix/verdict parsers. Existing plan, matrix and verdict files remain ordinary user files; the default workflow ignores them and Composite proves its own contract only through `compile`, optional targeted `verify`, Slice/Wave/Campaign gates and `stop-check`.
...
The Long-Task Workflow Preparation Skill (`/prepare-composite-long-task`) is the separate explicit Campaign V6 authoring/orchestration workflow for discussed raw plans that need one or more dependency-ordered SFCs. Its campaign is a narrow user-owned orchestration/provenance exception, not default lifecycle state. Existing `composite-*` commands and Skill paths remain compatibility identifiers; the executable Campaign schema is V6 and the public product name is Long-Task Workflow:

- Explicit invocation authorizes the complete prepare-and-execute loop. Scope Fit V4 inventories every control/capability-level `SRCU-###` and creates maximal coherent, never-renumbered `SFC-###` units. It may not split for parallelism, file/layer boundaries, agents or duration. A capacity split requires persisted evidence of the authoring limit and appends stable IDs before the first execution worker; execution freezes the graph permanently.
- Preserve the exact discussed plan as immutable `source-plan.md` with hash/kind/target metadata and complete Source Coverage V2 in `source-coverage.json`. Every delivery-significant item maps to SFCs, a global constraint, explicit out-of-scope entry or genuine decision. Non-terminal items record `context_resolution: existing|updated|task_local`; Packet Requirements carry the same resolution. Accepted V5 audit data is readable, but unfinished V5 returns `campaign_v5_execution_retired_recreate_required` and is never migrated.
- Before the first execution worker, the Campaign records the Context graph and baseline hashes. Just-in-time authoring rereads only referenced Context when graph/hashes are unchanged; relevant Context change forces reauthoring. Contract V3 defaults to `context_snapshot_mode: referenced`; `full` is explicit.
- Author the complete current dependency-ready frontier just in time through bounded ephemeral read-only `codex exec` workers using the controller profile and strict `CompositeAuthoringPacketV3` output schema. The Packet maps Source Units through Requirement, PI Obligation, AC and Verification Spec and is the authority handoff to independent execution workers; authoring and execution do not share a session.
- Every executable SFC has a closed **Change Envelope** whose file/path-glob carriers define allowed writes, explicit support paths cover generated/package-manager effects and non-file bindings remain contract keys. `project_context/**`, Campaign state and other forbidden paths remain unwritable. Receipt V3 binds detached base/head, changed paths, Packet/Contract/Envelope hashes and current accepted final result. Repair envelopes are explicit affected-envelope unions and cannot widen Campaign scope.
- Parallel placement requires positive independence evidence after Packet preflight. Dependencies, overlapping writes or verification inputs, shared API/schema/route/runtime/state/Context contracts, generated artifacts, package manifests, migrations and non-isolatable resources force serialization; unknown overlap defaults to serial. Wave Impact V2 uses the actual merge diff plus binding carriers, verification paths, contract keys, Context references and global constraints to produce qualified affected Slice/Spec identities with reason evidence. It runs only proven affected Specs, always includes global-constraint Specs and falls back to every candidate Spec when impact cannot be proved.
- One foreground scheduler owns Git/process/state sequencing. It starts all conflict-free SFC workers in a Wave before waiting, observes every child as settled and never leaves a child after CLI return. Managed fixed paths enforce one Integration worktree/branch, at most four detached current-wave SFC worktrees and one reusable detached repair worktree; retry reuses paths and no SFC/repair branch exists. Exact Sol `xhigh|max|ultra` routes execution/repair to Sol `medium`; high-and-below, non-Sol, unknown and invalid-policy cases pass through. Explicit target unavailability permits one controller-profile retry.
- Primary-worktree cleanliness is not required. When the target worktree is dirty, baseline and hidden-ref operations use a temporary index plus `commit-tree` without mutating the user's index, branch, working tree or stash. Every Git operation names its Campaign-owned worktree, including fetch, upstream synchronization and target finalization. `preserve_primary_worktree: true` is invariant, `auto_push: true` is the default, protected branches use pull-request mode, and primary state is checked before and after the Campaign.
- Campaign mutation uses a simple `campaign-lock-v1` with PID/process-start identity/operation id, optimistic generations, atomic writes, append-only events and bounded intent/result records around worktree, worker, Integration, delivery and accepted effects. There is no lease, heartbeat, persistent host or distributed takeover. Restart reconciles stale locks, interrupted worker observations, fixed worktrees, Git heads, final results and Receipts, then resumes from a durable stage with a new bounded attempt.
- Authoring allows one initial plus two repair attempts; SFC execution allows one initial plus three repair attempts per run generation in the same worktree; Integration repair is serial in the one reusable repair worktree. SIGINT/SIGTERM/`interrupt` stops dispatch, terminates only known child trees and persists interrupted state. PID, exit code and worker text remain observations, never acceptance.
- Targeted `verify` is an optional repair accelerator and never accepts a Slice. The Slice Final Gate always executes all Slice Specs; Wave Integration Gate runs its non-accepting Impact V2 Spec selection; Campaign Final Gate recomputes all Slices on the shared final snapshot. All three layers remain mandatory. Accepted detached Slice commits merge by SHA to Integration in stable order; ordinary conflicts and regressions create bounded repair workers rather than user decisions.
- Campaign Final Gate evaluates one shared final snapshot. Identical verification specs may be deduplicated only when snapshot, normalized spec, oracle, executable, input paths, command and environment are all identical. It reruns all required bindings, counterfactuals, global constraints and source-coverage rules; historical Slice results never prove Campaign completion. Finalization fetches the authoritative Target and accepts exact verified commit/tree identity. Any different tree receives one complete Target Snapshot Gate over current Packets, Source Coverage, Slice contracts and global constraints. A Target missing implementation proceeds only through non-force fast-forward delivery, matching-open-PR fallback or Integration repair; push success is refetched and identity-checked, and merged/closed PRs are not reused.
- Campaign `accepted` is a stable V6 terminal state committed in one mutation with `campaign.yaml`, accepted Campaign Final Result, hash-validated Target Finalization Receipt and event. Reruns validate frozen authority and return finished before workers, Gates, PR or worktree recreation. Idempotent cleanup runs only after acceptance; failure can resume but cannot revoke acceptance.
- The authority ladder is immutable source → Source Coverage/Scope Fit graph → Context baseline → Packet revisions → frozen Exec/Git identities → Contract V3 → Receipt V3 → Wave gates → shared-snapshot Campaign final result → Target Finalization Receipt and accepted transaction. Worker prose/status, PR state or handwritten state cannot promote any layer.
- Git-trackable Campaign data may retain immutable source/graph/Packet/schedule revisions and compact engine/profile/worker/worktree/commit/results. Mutable contracts, command runs, raw artifacts and workdirs stay temporary. Campaign status is CLI-derived, never worker-written.
- No historical attachment, partial bundle, V1/V2/Markdown campaign, old temporary workdir or Campaign importer is added. Accepted V5 remains inspectable audit data but automatic execution requires V6; unfinished V5 is rejected without silent migration. Complete Contract V3 YAML bundles retain the direct `/composite-long-task-workflow` route.
...
The Long-Task Workflow Skill (`/composite-long-task-workflow`) is a separate explicit-only lightweight executor for a complete Contract V3 three-input packet. Its design purpose is to allow implementation freedom while preventing false completion inside the declared contract and normal CLI/Hook boundary:
...
`init` creates the selected profile assets. The default Codex installation includes `core-portable` and `workflow-default`; `ty-context enable composite-codex` enables the explicit Composite profile and `ty-context disable composite-codex` removes only package-owned Composite Hooks/surfaces while preserving user Hooks:
...
The package-managed `.github/workflows/harness.yml` is a consumer project workflow. It installs the Node runtime required by the Harness CLI and runs the selected `validate-context`, `validate-code-modularity` or composite `validate-harness` target. In this source repository, direct `npm test` and `npm run test:long-task-workflow --workspace project-tiny-context-harness` invocations run the Long-Task Workflow suite, and package PR/main plus Trusted Publish CI run it through `npm test`. Release preparation, local fallback publication, Hooks and consumer Harness gates use default-only paths and never invoke that suite implicitly. Trusted publication packs once, installs that exact tarball into an empty repository, then proves `init`, `doctor`, `validate-context` and a minimal Contract V3 compile/final-gate path against the installed artifact. Its V2 attestation binds tarball SHA-256, Node version, pinned npm version and lockfile SHA-256; publication reuses the verified path without rebuilding another tarball.

Historical accepted V5 Campaign inspection lives under `composite-audit-v5/**`. Executable Packet/storage operations live under `composite-runtime-v6/**` and accept Campaign V6/Scope Fit V4/Source Coverage V2. V6 status/control exposes canonical worktree drift, exact Integration HEAD authority, PID/start-identity Worker ownership, interrupt acknowledgement, explicit abandon cleanup and one lightweight non-accepting run summary per generation. Legacy AppServer modules and their runtime black-box tests are isolated historical/audit code and are unreachable from V6 create/run or the active Long-Task suite.
