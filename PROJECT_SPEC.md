...
-> explicit Composite Long-Task Preparation Skill (`/prepare-composite-long-task`) can preserve a discussed multi-SFC plan as one complete Campaign V4 graph, author the current ready frontier, schedule isolated worktree Goals and drive integration/finalization
...
Campaign orchestration and Slice execution remain distinct authorities. `/prepare-composite-long-task` owns immutable plan/source coverage, one stable Scope Fit V3 DAG, just-in-time authoring for every current dependency-ready `SFC-###`, positive-evidence wave scheduling, worktree/Goal coordination, deterministic merge/repair, Wave Integration Gates, one-snapshot Campaign Final Gate and target finalization. `/composite-long-task-workflow` consumes one Slice's three inputs without inferring or repairing missing product, plan, acceptance or oracle meaning.
...
Each Slice still has one execution path with four costly operations, and each must reduce a named drift risk: contract compilation prevents omitted or unverifiable requirements and freezes authority hashes; implementation–verification iteration finds behavioral and scope drift before the end; Slice final full recomputation prevents stale, partial and historical evidence; Codex Stop freshness enforcement prevents a model completion claim from bypassing Slice final gate. Campaign V4 adds only the composition operations that close additional named risks: conservative scheduling prevents unsafe parallelism, Integration Gates catch merged-wave regressions, and one-snapshot Campaign Final Gate plus target resynchronization prevents historical Slice results from masquerading as final delivery. Generic epoch/progress-ledger/local-audit/review/TDD/matrix/verdict/final-card processes are not package responsibilities.
...
- **Final acceptance verdict**: an optional temporary ordinary-long-task reading artifact; Composite Contract V3 replaces it with verifier-generated `final-result.json` from one current full final recomputation.
...
- The ordinary long-task Skill (`/normal-long-task`) can help before long-running execution by externalizing the acceptance bar once. A Web GPT or external planning pass can produce the source plan, and `/normal-long-task` turns that plan into a full checklist and optional generic target-mode prompt. Composite Contract V3 is different: `/composite-long-task-workflow` accepts only the complete three YAML authorities, compiles their hashes and complete obligation/verification graph, runs active verification and controls final completion through final gate plus project-level Stop freshness checks. It does not generate the technical plan or checklist, does not bind a model execution methodology and does not use ordinary local-audit/matrix/verdict artifacts as proof. A genuine user decision is communicated through ordinary task interaction and leaves the Composite task unaccepted.
...
- Allow an explicit `/prepare-composite-long-task` Skill upstream of composite Slice execution when work starts from a discussed multi-SFC requirement. Explicit invocation authorizes the full Campaign V4 loop. Its tracked user-owned orchestration/provenance preserves the immutable source plan, complete coverage, stable full DAG, immutable `CompositeAuthoringPacketV3` and schedule revisions, Goal/commit/receipt identities and verifier-derived integration/final results. It centrally authors every current ready-frontier packet, deterministically renders/preflights the three Contract V3 authorities, conservatively schedules worktrees/Goals, integrates and repairs them, and derives completion only from current Slice/Wave/final gates plus target integration.
...
The Composite Long-Task Preparation Skill (`/prepare-composite-long-task`) is the separate explicit Campaign V4 authoring/orchestration workflow for discussed raw plans that need one or more dependency-ordered SFCs. Its campaign is a narrow user-owned orchestration/provenance exception, not default lifecycle state:

- Explicit invocation authorizes the complete prepare-and-execute loop. Scope Fit V3 returns `fit_for_three_inputs`, `split_required`, `blocked_for_decision` or `not_long_task`, persists every stable never-renumbered `SFC-###`, dependency edge, source reference, global constraint, produced/consumed contract, conflict domain and resource lock, and never reduces the campaign to one `selected_slice_id`.
- Preserve the exact discussed plan as immutable `source-plan.md` with hash/kind/target metadata and complete `source-coverage.json`. Every delivery-significant source item maps to one or more SFCs, a global constraint, explicit out-of-scope entry or genuine decision; uncovered source meaning blocks final acceptance.
- Author the complete current dependency-ready frontier just in time. `CompositeAuthoringPacketV3` remains each SFC's structured authoring source; package code, not copied prompt prose, deterministically renders the three strict Contract V3 YAML authorities and runs full-graph/oracle-ready stateless preflight. Downstream packets are authored only after predecessors pass integration so current Context/code informs them.
- The three execution inputs remain separate authorities: product/architecture owns intent and scope, the technical plan owns executable obligations, and the checklist owns completion semantics. Centralized campaign authoring prevents independent Goals from reinterpreting the discussed plan without collapsing those authorities or changing the strict downstream executor.
- Parallel placement requires positive evidence after packet preflight: no semantic/dependency/produced-consumed-contract edge, no overlapping implementation or verification path, no shared API/schema/route/runtime/state/Context contract, no shared non-isolatable resource and one common Integration Branch base. Unknown overlap defaults to serial. Priority, stable key and SFC ID provide deterministic ordering; ordinary ties never ask the user.
- All SFCs use isolated Git worktrees. A wave materializes every worktree and complete Goal objective, then creates/binds all Goal IDs before waiting, bounded by four and available resource capacity. Goal objectives freeze campaign/SFC/wave/worktree/branch/base/packet/contract/integration identities, allowed bindings and forbidden campaign paths. Goals implement, commit, verify and Slice-final-gate only their own packet; commit and a clean worktree precede final-gate, and accepted Goals do not mutate afterward.
- The authority ladder is immutable source plan → complete source coverage/Scope Fit graph → campaign coordination/audit → immutable packet revisions → deterministic YAML projections and schedule revisions → per-worktree Contract V3 compiled state/runs → Slice receipts → Wave Integration Gate results → one shared-snapshot Campaign Final Gate → target merge identity. Goal prose or hand-written campaign status cannot promote any layer.
- Accepted Slice branches merge only to the Campaign Integration Branch in stable SFC order. Receipt/head/base/clean/frozen-authority checks precede merge. Ordinary code conflicts produce repair worktrees/Goals; merged-wave regressions produce integration-repair Goals. Neither asks the user unless packets are semantically contradictory.
- Every wave passes an impact-aware Integration Gate for its ACs, affected prior ACs, global build/typecheck/smoke and cross-Slice contracts. After all SFCs integrate, Campaign Final Gate rematerializes/recompiles every packet and reruns every verification spec, binding, counterfactual, global constraint and source-coverage rule on one shared final snapshot. Historical Slice acceptance cannot prove Campaign completion.
- Target finalization fetches and compares the current target tip, resynchronizes and revalidates a moved target, then merges/pushes or opens/reuses an allowed automatic PR. An opened PR remains external-waiting rather than accepted until the target contains the integration and final revalidation succeeds. Only credentials/MFA/permission/external approval, an unautomatable protected-branch rule or genuine product/scope/architecture contradiction may block; ordinary Git conflicts, test failures and Goal interruption stay automatic repair/recovery work.
- Git-trackable campaign data may retain source/graph/packet/schedule revisions, Goal/branch/commit identities, receipt hashes, integration results, final result and target merge identity. Mutable compiled contracts, command runs, raw artifacts, active resource locks and workdirs stay in ordinary temporary execution storage. Campaign status is CLI-derived, never Goal-written.
...
- `compile` rejects every missing Requirement -> PI -> Obligation -> Binding -> AC -> Proof -> Spec -> Counterfactual edge and every boundary/non-completing/forbidden rule that lacks an executable negative assertion. It freezes the three inputs, registered Context, complete graph, oracle/verifier identities and workdir into `compiled-contract.json`. An identical active compile is idempotent; any changed identity is rejected as `active_contract_changed`.
...
- `verify` selects impacted frozen specs, collects verifier-owned observations/artifacts and writes actionable `needs_work` findings. The agent cannot submit pass results, evidence, assertion JSON or completion state.
...
- `<harnessRoot>/skills/prepare-composite-long-task/SKILL.md`
...
The package-managed `.github/workflows/harness.yml` is a consumer project workflow. It installs the Node runtime required by the Harness CLI and runs the selected `validate-context`, `validate-code-modularity` or composite `validate-harness` Make target. For pull requests it provides `TY_CONTEXT_MODULARITY_BASE` so the modularity target audits PR/base changes instead of full historical debt. Maintainer-only checks must not be copied into consumer projects. In this source repository, the two complete-suite npm commands and the focused Campaign E2E/mechanism profile remain explicit maintainer gates only. Package CI and trusted publication do not run those tests; they retain their narrower build, source-drift, Context, version and prepared-artifact integrity checks.
