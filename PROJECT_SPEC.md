...
- Allow a plan acceptance checklist compiler for long-running or handoff-heavy plans when the user provides a plan-like source and asks for acceptance criteria or a goal/target-mode prompt. It must copy the plan and checklist into `tmp/ty-context/plan-acceptance/**`, read relevant Context, stop for confirmation when durable assumptions are unclear, and avoid executing the plan or claiming completion. If the generated prompt requires a target-mode local audit, that audit records acceptance progress and blockers only; it must not replace the workflow-contract `plan.md`, Task Contract, Context or project quality proof.
