...
Visual Long-Task delivery is an authoring/evidence specialization, not a new mechanism or second workflow. External design briefs or indexes may enter `source_paths` as ordinary marked Source; acceptance-affecting selected targets, token sources and fixed prototype fixtures enter `verification_inputs`; production owners enter `input_paths`/Technical Bindings; generated implementation renders/diffs/reports remain `artifact_globs`. Before Compile, material production UI performs UI Authority Closure, classifies references as exact targets, partial constraints or inspiration and resolves an unconfigured/candidate/style-only gap through existing Source delegation or a genuine `decision_required` item. The Control projection preserves each applicable field as an independent Source-backed Claim and protected product semantic.

When Controls exist, Product `surface_bindings` are mandatory. Each aggregates Control keys for one declared owner surface and required product target, references existing route/component Bindings, names the root success Check and entry action, and requires target-local Control proof. Selected exact/constraint targets within that binding identify frozen files, conditions, Claim refs, conformance Check/Assertion and actual/comparison artifacts; the existing Final Gate evaluates their typed `design_conformance`. Explicit blocker dispositions reuse target-local Claims or blocking External Confirmations. Combined work may author candidates in ordinary Outcomes/Stages, but candidate/planned targets authorize no fidelity Claim; selection becomes real marked Source/registry input and, after lock, an adopted protected revision. Browser Control proof uses `ui_browser`; native/desktop Control proof uses `runtime_behavior` on the declared target. This adds no `uiux_delivery` block, visual Claim type, risk level, lifecycle state, baseline authority or Gate, and Contracts without Controls or selected targets pay none of the corresponding authoring/conformance cost.
