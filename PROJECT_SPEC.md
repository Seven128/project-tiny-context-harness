...
- Allow `validate-plan-acceptance` to check `plan-conformance-matrix.*` and `final-acceptance-verdict.*` for contradictory complete claims, dangling evidence references, weak-proof language, missing cross-references and declared surface/architecture binding gaps. This keeps temporary acceptance artifacts honest without turning Tiny Context into the project's quality system.
...
- It requires `final-acceptance-verdict.*` to be generated only at the final gate. This verdict judges each AC as `complete`, `partial`, `blocked`, `not_run`, `invalidated` or explicit `out_of_scope_NA` using fresh evidence and current contradictions.
- Before any completion claim, the future executor runs `ty-context validate-plan-acceptance <dir>` after `superpowers:verification-before-completion`; validator failure prevents final complete.
