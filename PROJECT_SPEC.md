...
The package-managed `.github/workflows/harness.yml` is a consumer project workflow. It installs the Node runtime required by the Harness CLI and runs the selected `validate-context`, `validate-code-modularity` or composite `validate-harness` target. In this source repository, local `npm test` remains bounded to the default package suite, while every package PR/main CI job runs typecheck, the complete default suite, the complete Composite suite, source drift, `validate-harness` and Quickstart Smoke. Trusted publication repeats the complete suites, packs once, installs that exact tarball into an empty repository, then proves `init`, `doctor`, `validate-context` and a minimal Contract V3 compile/final-gate path against the installed artifact. Its V2 attestation binds tarball SHA-256, Node version, pinned npm version and lockfile SHA-256; publication reuses the verified path without rebuilding another tarball.
