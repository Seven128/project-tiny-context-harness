...
Each Slice still has one execution path with four costly operations, and each must reduce a named drift risk: contract compilation prevents omitted or unverifiable requirements and freezes authority hashes; implementation–verification iteration finds behavioral and scope drift before the end; Slice final full recomputation prevents stale, partial and historical evidence; Codex Stop freshness enforcement prevents a model completion claim from bypassing Slice final gate. Campaign V5 adds only composition/host operations that close named risks: Scope Fit V4 prevents obvious semantic over-splitting, exact catalog-bounded routing prevents guessed model downgrades, persisted App Server identities prevent duplicate/lost work, conservative scheduling prevents unsafe parallelism, Integration Gates catch merged-wave regressions, and one-snapshot Campaign Final Gate plus target resynchronization prevents historical Slice results from masquerading as final delivery. Generic epoch/progress-ledger/local-audit/review/TDD/matrix/verdict/final-card processes are not package responsibilities.
...
- Explicit invocation authorizes the complete prepare-and-execute loop. Scope Fit V4 inventories every control/capability-level `SRCU-###`, returns `fit_for_three_inputs`, `split_required`, `blocked_for_decision` or `not_long_task`, and persists every stable never-renumbered `SFC-###`, legal separation reason, dependency, source/global constraint and conflict input. One SFC is the maximum coherent authorable range inside one acceptance outcome; parallelism, files, layers, agents and duration cannot force a split. Obvious same-cohesion over-splitting is rejected, factual authoring-capacity splits append stable IDs before the first Goal, and the first Goal permanently freezes the graph.
...
- Author the complete current dependency-ready frontier just in time. Each SFC receives one persistent App Server thread whose read-only Authoring Turn uses the known Controller model/effort and strict `CompositeAuthoringPacketV3` output schema. The Packet must map every Source Unit through Requirement, PI Obligation, AC and Verification Spec. Shared Campaign writes/render/preflight are serialized; preflight passes before any Goal, and the same thread is retained for execution. Downstream packets wait for predecessor integration so current Context/code informs them.
...
- Parallel placement requires positive evidence after Packet preflight: no semantic/dependency/produced-consumed-contract edge; overlapping implementation/verification path; shared API/schema/route/runtime/state/Context contract or non-isolatable resource; Source Unit cohesion collision; migration sequence; generated artifact; package-manager manifest; or environment profile. Unknown overlap defaults to serial. Priority, stable key and SFC ID provide deterministic ordering; ordinary ties never ask the user.
- All SFCs use isolated Git worktrees. A wave materializes every worktree and Goal Manifest V2 before setting Goals on the existing SFC threads, then starts all initial execution Turns before waiting, bounded by four and available resource capacity. Goal Manifest/objective freezes thread, exact authoring/execution profiles, routing, campaign/SFC/wave/worktree/branch/base/Packet/contract/integration identities, allowed bindings and forbidden paths. Only catalog-proven Sol `xhigh|max` or its explicit successor routes to Sol `medium`; lower, unrelated, unknown or unavailable profiles pass through. `needs_work` and interruption reuse the same explicit-profile thread. Goals implement, commit, verify and Slice-final-gate only their own Packet; commit and a clean worktree precede final-gate, and accepted Goals do not mutate afterward.
- The authority ladder is immutable source plan → complete Source Unit/source coverage and Scope Fit V4 graph → persisted host/thread/Turn/Goal routing state → immutable Packet revisions → deterministic YAML projections/schedule revisions → per-worktree Contract V3 state/runs → Slice receipts → Wave Integration Gate results → one shared-snapshot Campaign Final Gate → target merge identity. Goal prose, server status or hand-written Campaign status cannot promote any layer.
- Accepted Slice branches merge only to the Campaign Integration Branch in stable SFC order. Receipt/head/base/clean/frozen-authority checks precede merge. Ordinary code conflicts and merged/final regressions produce independent execution-profile repair threads/worktrees/Goals that cannot change Scope Fit or Packets. They do not ask the user unless contracts are semantically contradictory.
...
- Git-trackable Campaign data may retain source/graph/Packet/schedule revisions, App Server catalog/controller/thread/Turn/Goal/routing/recovery state, branch/commit identities, receipt hashes, integration results, final result and target merge identity. Mutable compiled contracts, command runs, raw artifacts, active resource locks and workdirs stay in temporary execution storage. Campaign status is CLI-derived, never Goal-written.
- App Server stdio is the only automatic Host adapter. One reconnect resumes/reads/reconciles known SFC and repair threads, Turns and Goals; a second failure becomes `wait_external`. If thread/Turn creation may have occurred but cannot be uniquely correlated, fail closed rather than duplicate work. No manual Goal/subagent fallback is allowed.
