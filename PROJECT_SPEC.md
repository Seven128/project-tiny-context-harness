...
- Allow `validate-plan-acceptance` to check `plan-conformance-matrix.*` and `final-acceptance-verdict.*` for contradictory complete claims, dangling evidence references, weak-proof language, missing proof layers, material/critical drift, unapproved sibling substitution, blocking auditor findings, missing cross-references and declared surface/architecture binding gaps. This keeps temporary acceptance artifacts honest without turning Tiny Context into the project's quality system.
...
The Superpowers long-task Skill (`/superpowers-long-task`) is a separate adapter for a recommended three-input packet:
...
- It is Tiny Context's adapter layer, not a Superpowers official schema.
...
- It emits a visible Superpowers input packet and execution binding: install Superpowers through the current platform's official path when missing; record installation blockers in the local audit rather than treating them as completion; map Product / Architecture Source, Technical Realization Plan, full checklist, local audit, Context and required tests/core paths into execution roles; evaluate Product Context Delta and Technical Context Delta before implementation; use `superpowers:writing-plans` if the technical plan is not bite-sized, `superpowers:subagent-driven-development` when subagents are available, `superpowers:executing-plans` otherwise, plan/AC behavior gap -> TDD with `superpowers:test-driven-development`, and `superpowers:verification-before-completion` before any completion claim.
...
- Before any completion claim, the future executor runs `ty-context validate-plan-acceptance <dir>` after `superpowers:verification-before-completion`; validator failure prevents final complete. If auditor-driven fixes change artifacts or evidence, the validator runs again. The validator remains a structural consistency check: it can reject complete rows with missing required layers, material or critical drift, unapproved sibling substitution, blocking auditor findings or weak evidence summaries, but it does not prove product quality.
