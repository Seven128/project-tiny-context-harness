...
- Overall completion requires both gates plus the external-reviewer evidence discipline: no blocking implementation drift in the plan-conformance matrix, all required ACs complete in the final acceptance verdict, no missing required proof layers, assertion-backed evidence for every required machine-verifiable proof layer, no negative evidence contradiction, no material or critical drift, no unapproved sibling substitution and no blocking auditor gap when the subagent audit is available.
