...
One design-specific Long-Task objective is that Agent implementation, acceptance and testing fully conform to every material UI/UX fact that selected design resources explicitly express within their declared scope and conditions. Direct visual inspection is a lossy boundary: layout, control styling, state, interaction, motion, adaptation/input, accessibility and asset meaning can be overlooked, flattened or reinterpreted before it becomes Contract authority. Open Design outputs, human-authored resources and existing exact targets remain valid ordinary Source; Figma-native input is an optional higher-density route when native inspectability, editable collaboration, component/library reuse or organizational handoff has material value and its connector/auth/read path is operational.

The hybrid transformation deliberately has two complementary inputs. A selected Figma-native target pins the exact file, immutable version, node IDs and declared conditions; bounded small-node acquisition obtains metadata, design context, variables, screenshots and applicable motion/assets/Code Connect, then freezes readable repository inputs with hashes while preserving a separate editable-upstream owner and update route. This lowers manual transcription cost and improves the precision of facts Figma actually models. A provider-neutral residual handoff with exactly one `design-resource-handoff-v1` block still closes all eight dimensions and supplies what native design data does not own: scope/exclusions, condition meaning, not-applicable versus missing facts, product/data/permission/recovery semantics, unmodeled accessibility/input/motion, unresolved or unavailable items, blockers, Source-item mappings and verification bindings.

The downstream authority chain is therefore explicit: selected immutable resource plus residual handoff → Context-reachable target → Source Claims and applicable Controls/`surface_bindings` → production route/component owner and cold-start journey → independently failing `design_conformance`, interaction and target-runtime evidence → current-snapshot Contract Conformance or the sole Long-Task Final Gate. Connector or extraction success, a screenshot, resource hash or handoff preflight proves input acquisition/integrity only and cannot substitute for production conformance. Mutable-only links, missing version/node identity, truncated or unreadable input and unresolved coverage fail closed. Figma and the handoff remain ordinary Source, so the extension adds no provider-specific schema, registry, Claim kind, second Authority, second Gate or acceptance lifecycle.
...
The shared design-resource input adapter satisfies the same admission rule one boundary earlier. Direct visual-only consumption is not a reliable Contract interface: a prose summary or generic `condition_keys` can silently omit or flatten a category before implementation begins. Figma-native acquisition materially improves the input side when available because exact nodes, component/variant structure, Variables, Auto Layout relationships, annotations, screenshots, motion/assets and optional Code Connect mappings expose native facts without manually retranscribing them. It cannot express every product semantic or prove that all eight dimensions are resolved, and a live provider introduces permission, rate-limit, mutability and availability risk. The repository-readable immutable capture removes that runtime dependency; one residual `design-resource-handoff-v1` block then closes the independent semantic/coverage omission path for both consumers.

The residual handoff expands grouped subjects across eight closed dimensions, binds native or other covered facts to immutable evidence, Source Items and project verification methods, and fails on unresolved, stale, truncated or substituted evidence. Exploration pays no schema cost, non-Figma workflows remain valid, and selected implementation handoffs pay one compact residual index rather than duplicating every Figma property or creating one artifact per control. This is materially positive ROI: it reduces transcription effort and raises native fact precision while preserving omission detection and deterministic recovery. A Figma replacement of the handoff was rejected because it loses nonvisual semantics and downstream bindings; link-only and prompt-only approaches cannot be validated; manual duplication increases drift; and a provider-specific pack/schema/registry/Gate would duplicate ownership and add migration/runtime cost. The admitted extension therefore adds no Design Authority, workflow state, Receipt, second Authority or second Gate.
...
When Figma is supplied, requested or selected, the Skill feature-detects an operational native connector and follows a bounded Figma-native profile: exact file/version/nodes/conditions; metadata-first small-node acquisition; design context, Variables and screenshots; applicable motion, assets and Code Connect; repository-readable immutable capture; and a separate editable-upstream route. Reusable Components/Variants, Variables/token mappings, Auto Layout, semantic names and Annotations/dev resources are preferred source structure when applicable. Figma remains optional, plan-dependent capabilities stay explicit, Open Design remains the generation engine, and a mutable link, metadata-only response, flattened export or incomplete/truncated acquisition authorizes no fidelity.

When that selection is an implementation handoff, the Skill additionally creates one project-native marked Markdown Source with exactly one strict residual `design-resource-handoff-v1` block and runs `ty-context design-resource preflight`. The adapter references addressable native facts rather than manually duplicating them, then records residual scope/provenance/resources/conditions/subjects/targets/addressable evidence, closes all eight UI/UX dimensions, binds Source Items and verification methods, and refuses ready status for unresolved or stale input. Exploration remains schema-free. The default Workflow reruns the same preflight before UI Authority Closure; Long-Task reruns it during Preflight/Compile and requires each Contract `design_target` to match the handoff target/files/conditions, every covered Source Item to reach the root conformance Assertion and every declared blocker to reach the owning surface binding. Native extraction and the adapter prove semantic input completeness and resource integrity only; project checks and default Contract Conformance or Long-Task Final Gate prove current implementation.
