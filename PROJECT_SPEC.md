...
When Source is raw, mixed, attachment-heavy or incomplete, `/long-task-workflow` performs Source-quality authoring before Contract mapping in the same native Goal. It supports refinement, synthesis and hybrid modes without a fixed intake form; inventories every supplied artifact; preserves direct requirements/qualifiers; and records derived, delegated and evidence-backed meaning with traceable basis. An instruction to synthesize/refine/complete/use judgment delegates authoring but does not invent tradeoff priorities. Unknown preferences that would materially change comparative research or selection trigger one targeted clarification; known preferences and recommendation-insensitive reversible choices do not.

After the preference envelope is clear, current changing external claims use authoritative/primary evidence. A defensible recommendation is written into real Source with authoring instruction, basis and exact added meaning before Contract mapping. High impact alone is not unresolved. Delegation authorizes plan meaning, not payment/purchase, contracting, production deployment/publication, destructive production mutation, permission grants, sensitive-data transmission or required legal/security/human approval; those remain External Confirmations. Conflicting authority, user-reserved choices, missing material preferences or no defensible recommendation remain `decision_required`.
...
Integrated Source-quality authoring and Contract Draft authoring both remain inside `long-task-workflow`; the separation is progressive reference loading, not a service handoff. External design resources remain ordinary Source and do not replace repository-aware Contract authoring.
...
-> integrated Source inventory/synthesis/refinement when needed
...
Every Long Task has at least one real Source file, and every declared Source file contains at least one Material Source Item; background-only material belongs in Context or ordinary references. Integrated Source authoring may refine a writable initial proposal or materialize one conversation-only delivery Source before marker insertion. Every Material Source Item is wrapped in its original Markdown with a non-rendering `<!-- ty-source-item:start key=... kind=... -->` / `<!-- ty-source-item:end -->` pair without rewriting direct text. A research proposal, ordinary prose plan or legacy Source Plan remains ordinary input and does not need to become strict Contract YAML.
...
Typed dispositions preserve meaning. Every non-decision Source item owns exactly one canonical target of the same semantic kind and normalized text; a target has one Source owner. Outcome `acceptance` points to one stable `<outcome>.<check>.<assertion>` whose criterion is Source-text-identical and proves an independently Source-backed non-Result Claim. Integrated Source authoring uses the exact ten Runtime risk names, including `data_migration`, separate `critical_user_path` and `weak_observability`, and retained `multi_repository_change`. `out_of_scope` is retired.
...
`context_product_plan` keeps its existing responsibility: author product proposals and classify durable product facts in owning `project_context/**` when explicitly requested. It is not a required stage before integrated Long-Task Source authoring.
...
`/long-task-workflow` remains the only active long-task execution Skill. It consumes the revised initial proposal plus selected design resources directly, performs integrated Source-quality authoring when needed, asks before preference-sensitive research, records defensible delegated meaning in real Source, inserts markers, authors one Compact V2 Contract, Preflights/Compiles, stops once for model choice, executes rolling Frontiers and runs the Live Final Gate. It creates no automatic model switch, Source Plan handoff, model route/checkpoint state, proactive subagent scheduler, second plan, Source Inventory Receipt, questionnaire state, matrix or top-level split.
...
- Package build identity is a deterministic digest of declared source/configuration/package/lockfile inputs; `--no-build` fails before testing when that digest is missing or stale. Package-suite timing is ephemeral per-file diagnostic output only. Controlled Ubuntu CI supplies wide suite-specific ceilings to stop catastrophic cost regressions; local and cross-environment functional runs have no general narrow wall-clock gate. Timing and build identity create no result cache, workflow state, Receipt or reusable authority.
- Long-Task delivery fixtures may copy one immutable initialized seed per suite process instead of repeating Git initialization for every case, but each fixture owns a distinct common directory, worktree, local configuration and refs and has no remote. After bounded identity/outcome/side-effect A/B proof, file-level concurrency defaults to two only for explicit pure/static or proven isolated-temp-repository files; Hook/profile, environment, distribution, verifier-migration, Git-race and every unknown file remain serial. Environment-selected concurrency one remains the mechanical rollback.
