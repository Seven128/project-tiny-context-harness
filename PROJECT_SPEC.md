...
-> Superpowers long-task Skill (`/superpowers-long-task`) consumes Product / Architecture Source + Technical Realization Plan + Acceptance Checklist when Superpowers execution is needed
...
中文对照：短程任务直接用流程契约 + Context 层；普通长程 checklist 准备仍可用 Web GPT 或外部规划模型产出《开发方案》+《验收清单》两份输入并调用 `/normal-long-task`；需要 Superpowers 专用目标模式文本时推荐提供 Product / Architecture Source、Technical Realization Plan、Acceptance Checklist 三份输入后直接调用 `/superpowers-long-task` 适配，然后用 Superpowers 落成执行切片。执行前用 Product Context Delta / Technical Context Delta 判断是否要更新 Context，执行中用 `plan-conformance-matrix.*` 防实现漂移，最后用 `final-acceptance-verdict.*` 防验收漂移。原因是 Context 到代码 这一步在长程任务中更容易漂移。
...
- **Plan conformance matrix**: a temporary execution trace under `tmp/ty-context/plan-acceptance/**` that maps each behavior-affecting implementation-plan item to expected surfaces, implemented paths, tests, runtime/evidence, status and drift. It is initialized early and updated during Superpowers execution. It is not final acceptance proof.
- **Final acceptance verdict**: a temporary final-gate artifact under `tmp/ty-context/plan-acceptance/**` that judges every AC from the full acceptance checklist against fresh evidence and current contradictions. It is the acceptance proof surface for Superpowers long-task execution, while still not durable Context.
...
- Allow `/superpowers-long-task` to adapt a recommended three-input packet directly when Superpowers execution is requested: Product / Architecture Source, Technical Realization Plan and full Acceptance Checklist. It must not require `/normal-long-task` when those documents already exist, must not perform complexity routing, and must not translate product intent into the technical plan. The external Technical Realization Plan must already satisfy the Superpowers-ready Markdown implementation plan fields; otherwise the Skill stops with a Missing Fields Report before prompt generation. Two-document compatibility is allowed only when the first document explicitly contains both source and technical-plan sections. Missing required input produces a structured Missing Fields Report. Future execution artifacts still live under `tmp/ty-context/plan-acceptance/**`: optional `evidence-manifest.md/json` for per-slice synchronization, `plan-conformance-matrix.*` for implementation trace and `final-acceptance-verdict.*` for final AC-by-AC acceptance. The evidence manifest is not a fourth input, not durable Context, not proof by itself and not required by `validate-plan-acceptance`; it reduces stale wording between matrix, verdict and audit. Complete rows should be externally reviewable evidence claims, with checklist-derived proof chains, no missing required layers and traceability to fresh evidence directly or through an optional `evidence_id`; a separate Evidence Ledger / proof index remains optional and is not a fourth input.
...
- Allow `validate-plan-acceptance` to check `plan-conformance-matrix.*` and `final-acceptance-verdict.*` for contradictory complete claims, dangling evidence references, weak-proof language, missing proof layers, material/critical drift, unapproved sibling substitution, blocking auditor findings, raw secrets/tokens/cookies, optional evidence-manifest reference consistency, generated active-count consistency under complete verdicts, missing cross-references and declared surface/architecture binding gaps. Validator `errors` are blocking and drive non-zero exit codes; optional `warnings` and `hygiene` report non-blocking stale prose, unsupported milestone labels and cleanup hints. This keeps temporary acceptance artifacts honest without turning Tiny Context into the project's quality system.
...
- It is recommended through explicit `/superpowers-long-task` invocation when Product / Architecture Source, Technical Realization Plan and Acceptance Checklist already exist and the next step needs Superpowers execution. It does not perform complexity routing; invocation means Superpowers long-task execution was already selected.
...
- It recommends optional `evidence-manifest.md/json` entries for non-trivial execution slices. Each entry records slice identity, missing layer classes, touched plan/AC ids, commands, artifacts, runtime or browser evidence, what the slice proves, what it explicitly does not prove, closed and remaining missing layers, cleanup status, redaction/security status and freshness. Default slice sizing groups 2-4 strongly related missing layers that share an AC, runtime scenario, proof environment or verification path; single-gap slices are reserved for blockers, contradictions or tiny metadata cleanup.
- It requires `plan-conformance-matrix.*` to be initialized early and updated during execution. This matrix answers whether implementation followed the Technical Realization Plan across code, API/schema, UI/page, worker/runtime, artifact, data, tests and evidence. Surface/IA/architecture-migration rows carry declared owner surface, required user paths, forbidden primary surfaces, real page evidence, negative checks and Context fact refs when applicable. When plan or checklist evidence is layered, complete rows expose required proof layers, satisfied proof layers, missing proof layers and substitute-evidence use. Status values distinguish `complete`, `partial`, `sampled_only`, `not_implemented`, `blocked`, `scope_changed_requires_user_approval`, `contradicted_by_current_state` and `out_of_scope_NA`.
- It requires `final-acceptance-verdict.*` to be generated only at the final gate. This verdict judges each AC as `complete`, `partial`, `blocked`, `not_run`, `invalidated` or explicit `out_of_scope_NA` using fresh evidence and current contradictions. Complete AC rows must be externally reviewable: they expose the checklist-derived required proof chain, fresh evidence, missing required layers, drift severity, sibling-substitution state and any independent auditor finding when those fields are applicable. A separate Evidence Ledger / proof index is optional, but complete rows must trace to fresh evidence directly or through an optional `evidence_id`.
- Before any completion claim, the future executor syncs matrix/verdict/audit from the evidence manifest when used, runs `superpowers:verification-before-completion`, then runs `ty-context validate-plan-acceptance <dir>`; validator failure prevents final complete. If auditor-driven fixes change artifacts or evidence, the validator runs again. A final stale/overclaim scan removes closed-gap wording and rejects unsafe claims such as `accepted`, `complete`, `final passed`, `product_goal_complete=true`, unapproved scope narrowing or unapproved sibling substitution. Superpowers review and verification remain useful execution checks, but they cannot override Tiny Context gates: passing Superpowers review does not by itself prove plan conformance or checklist acceptance. The validator remains a structural consistency check: it can reject complete rows with missing required layers, material or critical drift, unapproved sibling substitution, blocking auditor findings or weak evidence summaries, but it does not prove product quality.
...
- Overall completion requires both gates plus the external-reviewer evidence discipline: no blocking implementation drift in the plan-conformance matrix, all required ACs complete in the final acceptance verdict, no missing required proof layers, no material or critical drift, no unapproved sibling substitution and no blocking auditor gap when the subagent audit is available.
