...
- Campaign mutation uses a transactional store whose callback returns one closed mutation plan instead of writing formal annexes directly. A `campaign-lock-v1` lease, staged/fsynced artifacts, root `.campaign-transaction.json` intent, atomic replacements, file/directory durability, before/after-hash events and startup recovery cover every multi-file mutation. All-Before state rolls back, provable partial/After state completes deterministically, and irreconcilable state or orphan revisions enter quarantine and fail closed; a crash cannot silently promote status.
