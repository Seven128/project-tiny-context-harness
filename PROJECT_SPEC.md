...
-> Composite Long-Task Workflow Skill (`/composite-long-task-workflow`) compiles Product / Architecture Source + Technical Realization Plan + Acceptance Checklist into one immutable Contract V3 graph
-> the trusted Host seals first authority outside the workspace; the agent implements freely and repeatedly invokes affected-spec verification
-> final gate runs every frozen spec, trusted probe and obligation counterfactual on one final isolated snapshot and recomputes the complete entity graph
-> the system-managed Codex Stop Hook repeats that Host-authorized final verification; release acceptance additionally requires the separately pinned external audit
```

Preparation and execution remain distinct. `/prepare-composite-long-task` owns Scope Fit and just-in-time authoring for one dependency-ready `SFC-###`; its `CompositeAuthoringPacketV3` deterministically renders the three strict Contract V3 YAML inputs and stops at `handoff_ready`. `/composite-long-task-workflow` consumes those inputs without inferring or repairing missing product, plan, acceptance or Oracle meaning.
...
Completion follows a strict separation of authority. The frozen three-input contract and Context define Requirement → PI → Obligation → Binding → AC → Proof → Spec → Counterfactual. A pinned compiler and Host execute frozen Oracle bundles and real dependency/browser commands in sealed sandboxes; the Harness evaluates Observation V2 actuals, population coverage and counterfactual sensitivity. Final gate alone calculates `accepted` from one complete run. The managed-only Stop Hook reads workspace-external Host authority and repeats the full final orchestrator before deciding whether Codex may end. Agent prose, hand-written JSON, Oracle status fields, agent-selected commands, old attempts, screenshots alone, file existence, generated summaries, repo Hooks and modified acceptance tools never issue completion proof.
...
中文对照：短程任务直接用流程契约 + Context 层；普通长程 checklist 准备仍可用 `/normal-long-task`；原始多 SFC 需求可显式调用 `/prepare-composite-long-task` 产出 Contract V3 三输入；已有完整三输入时显式调用 `/composite-long-task-workflow`。模型在实现中可能漂移，工作流通过 Host 外置首份权威、Harness 自算 Observation V2、中途主动验证、最终单快照全量反事实验证和 managed-only Stop Hook 拦截，使带已知漂移的产物不能交付。
...
- **Plan conformance matrix**: an optional temporary ordinary-long-task reading trace under `tmp/ty-context/plan-acceptance/**`; it is not used by Composite Contract V3 and is never final acceptance proof.
- **Final acceptance verdict**: an optional temporary ordinary-long-task reading artifact; Composite Contract V3 replaces it with the Host-committed signed `final-result.json` from one full final snapshot.
...
- The ordinary long-task Skill (`/normal-long-task`) can help before long-running execution by externalizing the acceptance bar once. A Web GPT or external planning pass can produce the source plan, and `/normal-long-task` turns that plan into a full checklist and optional generic target-mode prompt. Composite Contract V3 is different: `/composite-long-task-workflow` accepts only the complete three YAML authorities, seals their complete graph in the workspace-external Host registry, runs active verification and controls final completion through the 20-step final gate plus managed-only Stop Hook. It does not generate the technical plan or checklist, does not bind a model execution methodology and does not use ordinary local-audit/matrix/verdict artifacts as proof. Existing local apps, browser sessions, CLI auth and system credential helpers remain self-service resources; only current Harness probes plus alternatives can classify an external blocker without acceptance.
...
- Allow an explicit `/prepare-composite-long-task` Skill upstream of composite execution when work starts from a raw multi-SFC requirement. Its tracked campaign is limited to user-owned authoring/provenance, authors only the current SFC through `CompositeAuthoringPacketV3`, deterministically renders the three Contract V3 YAML authorities, requires exhaustive graph/binding/proof/counterfactual coverage and a bundle-ready Oracle before handoff, and never creates runtime proof or aggregate product completion.
- Allow `/composite-long-task-workflow` only through explicit invocation after the three Contract V3 YAML authorities exist. It does not generate or repair product requirements, obligations, ACs or Oracles; it seals the first frozen contract in the workspace-external Host registry, lets the agent choose its own implementation methods, actively verifies affected frozen specs and blocks Codex completion until one unchanged final snapshot is accepted by the complete Host-authorized orchestrator.
- Require structural coverage across Product Requirement -> PI -> atomic obligation -> implementation binding -> AC -> proof requirement -> executable spec -> obligation counterfactual, plus Product Boundary / non-completing outcome / forbidden shortcut -> related negative assertion -> executable spec. Any unmapped node, unobservable binding, unrelated proof, text-only negative rule, manual-only AC, summary-only AC, incomplete population contract or AC without a sound machine Oracle blocks compile.
...
- Keep all acceptance calculation inside the pinned verifier, Host and final gate. Oracles emit only typed Observation V2 actuals; Agent-written status, evidence, assertion JSON, commands, blockers and artifact registration do not exist. Workdir status/result files are signed thin mirrors of Host authority; `runs/**` contains verifier-owned command and artifact manifests for reproduction only.
...
- Treat `needs_work` as a non-terminal repair loop and `externally_blocked` as a trusted, narrowly classified condition produced only by current Harness-run probes plus alternatives. Ordinary product, test, evidence, scope and UI failures must continue as needs-work findings. A mandatory system-managed Codex Stop Hook prevents unaccepted or stale completion replies; missing, untrusted, locally overridden or conflicting Hook/Host capability makes strict workflow start fail with no repo/user/plugin fallback.
...
- Only the current SFC is authored just in time. `CompositeAuthoringPacketV3` is the semantic source; package code, not a copied free-form prompt, deterministically renders the three strict Contract V3 YAML authorities and runs graph/binding/proof/counterfactual and Oracle-bundle-ready stateless preflight.
...
- The authority ladder is fixed: immutable sanitized `request.md`; current `campaign.yaml` coordination; each `authoring-packet.json` preserved as an immutable revision; deterministic YAML projections; post-start verifier-owned `tmp` runs and signed mirrors; workspace-external Host authority; then the current Host-committed final result as the only SFC product-completion authority. There is no events ledger, and campaign status or recorded prose cannot promote completion.
- The storage model has three trust planes. Git-trackable user-owned campaign data preserves authoring/provenance; verifier-owned workdir runs and thin signed mirrors stay under `tmp`; immutable authority registry, keys, Oracle/dependency/browser layers and attestations live in workspace-external Host Managed State. V3 intentionally has no aggregate campaign product-completion state.
...
- No historical attachment, partial bundle, V1/V2/Markdown campaign, old temporary workdir or campaign importer is added. Their authority/schema cannot be proven consistently and compatibility would permanently weaken the strict executor; only complete Contract V3 YAML bundles keep the direct `/composite-long-task-workflow` route.
...
The Composite Long-Task Workflow Skill (`/composite-long-task-workflow`) is a separate strict executor for a complete Contract V3 three-input packet. Its design purpose is to allow implementation freedom while making false completion mechanically unavailable inside the declared contract boundary:
...
- `compile` rejects every missing Requirement -> PI -> obligation -> binding -> AC -> proof -> spec -> counterfactual edge and every boundary/non-completing/forbidden rule that lacks a related executable negative assertion. It bundles the complete transitive Node Oracle closure and seals source, registered Context, scope/population, verifier, sandbox, dependency, browser and managed Host identities in the first create-only Host authority.
- Oracle soundness is a declared trust prerequisite, not something graph coverage can infer. Oracles must be pre-existing package/project acceptance authorities or separately authored and frozen before product work; they emit actual-only Observation V2 records and cannot sign `passed`, completion or blocker status. The implementation task may change implementation tests but cannot author or modify the Oracle that proves the same task.
- `verify` creates an isolated content snapshot, selects impacted specs, runs frozen Oracle bundles and declared command steps against real read-only dependency/browser layers in the Host OS sandbox, and writes only actionable verifier-owned `needs_work` findings. The agent cannot submit commands, observations, evidence, blockers, assertion JSON, artifacts or completion state.
- `final-gate` discards intermediate acceptance value, follows one fixed 20-step sequence, revalidates every frozen identity, runs all specs, trusted probes and obligation-specific counterfactuals on one new final snapshot, projects Binding -> Proof -> AC -> Obligation -> PI -> Requirement, and durably commits one signed `long-task-final-result-v3`. Only `accepted` is product completion; `externally_blocked` is a narrowly machine-classified terminal condition, not completion.
- A mandatory system-managed Codex Stop Hook reads the workspace-external Host registry, verifies managed policy/runtime identity and real assistant final text, and repeats the same complete final orchestrator before exit. Needs-work returns a continuation prompt; accepted allows exit only after fresh re-acceptance; externally blocked must be freshly reproduced and permits only the frozen minimal blocked report. Repo/user/plugin Hooks are not authority or fallback, and Host/Hook trust failure has no degraded workflow mode.
- The workdir contains the three YAML inputs, signed thin UX mirrors, `goal-objective.txt` and verifier-owned `runs/**`. Registry authority, keys, bundles and content-addressed layers remain outside the workspace. There is no workflow protocol, execution binding, task-state reducer, events ledger, slice delta, matrix/verdict, progress ledger, evidence index, local audit or final card.
...
- This is an intentional breaking replacement. There is no hidden alias, dual-write, old workdir reader, importer, audit-only migration or automatic conversion. V1/V2 authorities, Observation V1, Markdown campaign data and old workdirs must be regenerated as Contract V3 and verified from a fresh snapshot.
- The implementation is complete only when every security case is a real installed-candidate CLI/managed-Hook black-box run, the current happy path reaches accepted with real implementation and a sensitivity flip, ordinary prompts remain a total no-op, and the byte-identical 0.4.0 candidate passes package source sync, real consumers, Windows/Linux Host smokes, macOS compatibility, an integrity-pinned independent external audit and release checks.
