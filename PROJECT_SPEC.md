...
-> explicit Long-Task Workflow Preparation Skill (`/prepare-composite-long-task`) can preserve a discussed multi-SFC plan as one complete Campaign V5 graph, use Codex App Server to author/execute one persistent thread per SFC, schedule isolated worktree Goals and drive integration/finalization
...
The Long-Task Workflow Preparation Skill (`/prepare-composite-long-task`) is the separate explicit Campaign V5 authoring/orchestration workflow for discussed raw plans that need one or more dependency-ordered SFCs. Its campaign is a narrow user-owned orchestration/provenance exception, not default lifecycle state. Existing `composite-*` commands, Skill paths, runtime types and schema ids remain stable compatibility identifiers; the public product name is Long-Task Workflow:
...
- Preserve the exact discussed plan as immutable `source-plan.md` with hash/kind/target metadata and complete Source Coverage V2 in `source-coverage.json`. Every delivery-significant item maps to SFCs, a global constraint, explicit out-of-scope entry or genuine decision. Non-terminal items record `context_resolution: existing|updated|task_local`; `existing` and `updated` require exact Context references, `task_local` requires a reason, and terminal items use `null`. Packet Requirements must carry the same resolution without adding a duplicate Context artifact. Campaign V4 audit data retains Source Coverage V1 and cannot enter V5 execution.
...
- Every executable Goal has a Goal Manifest V3 carrying a closed **Change Envelope** with `allowed_write_paths`, `allowed_supporting_paths`, `allowed_contract_keys`, `forbidden_paths`, the frozen `undeclared_change_policy: reject` and its hash. Technical Plan `file`/`path_glob` bindings provide write paths; Scope Fit generated artifacts and package-manager manifests provide supporting paths; symbol/schema/route/runtime-capability bindings provide semantic contract keys but cannot be an Obligation's only implementation carrier. `project_context/**` and `.codex/composite-long-task/**` remain hard-forbidden even if a Packet declares them writable, and every other undeclared write is rejected. Receipt V2 records the Envelope hash, `base..head` changed paths and an empty undeclared set. Repair envelopes are the explicit union of affected envelopes plus explicit conflict paths; they cannot silently widen Campaign scope. Goal Manifest V2 and Receipt V1 remain audit-only.
...
- All SFCs use isolated Git worktrees. Goal Manifest V3 freezes thread, exact model-routing decision, Campaign/SFC/wave/worktree/base/Packet/contract/integration identities and Change Envelope. Model routing comes from a versioned YAML policy and a host-discovered model catalog; current policy maps only catalog-proven Sol `xhigh|max` (or an explicitly named successor) to Sol `medium`, and passes through lower, unrelated, unknown or unavailable profiles. Missing or invalid policy bytes select a deterministic hashed safe-passthrough policy rather than guessing a downgrade. Campaign freezes the policy id/hash, catalog hash and complete routing decision, then verifies the decision hash.
...
- Campaign mutation uses a transactional store whose callback returns one closed mutation plan instead of writing formal annexes directly. A `campaign-lock-v1` lease, staged/fsynced artifacts, root `.campaign-transaction.json` intent, atomic replacements, file/directory durability, before/after-hash events and startup recovery cover every multi-file mutation. A same-host live PID remains authoritative after nominal expiry; remote ownership expires by time. The current operation heartbeats and asserts/renews ownership before intent, subordinate artifact, campaign and event replacement; ownership loss or heartbeat failure stops the commit, and close removes only the same operation's lock. All-Before state rolls back, provable partial/After state completes deterministically, and irreconcilable state or orphan revisions enter quarantine and fail closed; a crash cannot silently promote status.
...
- Targeted `verify` is an optional repair accelerator and never accepts a Slice. The Slice Final Gate always executes all Slice Specs; Wave Integration Gate runs its non-accepting Impact V2 Spec selection; Campaign Final Gate recomputes all Slices on the shared final snapshot. All three layers remain mandatory. Accepted Slice branches merge to the integration branch in stable order; ordinary conflicts and regressions create bounded repair Goals rather than user decisions.
...
- Campaign `accepted` is a stable V5 terminal state committed in one mutation with `campaign.yaml`, the accepted Campaign Final Result, a hash-validated Target Finalization Receipt and the corresponding event. Reruns validate this frozen authority and return finished before Scope Fit, App Server, Gate, PR or worktree work. Idempotent cleanup of validated Campaign-owned assets runs only after acceptance and records its own pending/complete status; failure or a crash can resume cleanup but cannot revoke acceptance, and later Target movement does not alter the frozen accepted identity.
- The authority ladder is immutable source → Source Coverage/Scope Fit graph → Context baseline → Packet revisions → frozen model/Git/transaction identities → Contract V3 → Slice receipts → Wave gates → shared-snapshot Campaign final result → Target Finalization Receipt and accepted transaction. Goal prose, server status, PR state or handwritten state cannot promote any layer.
...
Historical V4 Campaign inspection/fixture support lives under `composite-audit-v4/**`. Mutating Packet/storage operations live under `composite-runtime-v5/**`, accept only Campaign V5/Scope Fit V4/Source Coverage V2, and reject V4 before mutation.
