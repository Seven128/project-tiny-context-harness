...
-> ordinary long-task Skill (`/normal-long-task`) produces the full checklist and optional generic target-mode prompt
-> Superpowers long-task Skill (`/superpowers-long-task`) produces the Superpowers target-mode prompt when Superpowers execution is needed
-> Superpowers derives concrete implementation slices
-> each slice follows the workflow contract + Context layer
```

Here Superpowers means the specific [obra/Superpowers](https://github.com/obra/superpowers) plugin/workflow. The Superpowers long-task Skill is Tiny Context's adapter layer, not a Superpowers official schema. If the source plan or target-mode prompt is not executable enough, `superpowers:writing-plans` turns it into a bite-sized implementation plan; `superpowers:subagent-driven-development` is preferred when subagents are available; `superpowers:executing-plans` is the non-subagent fallback; `superpowers:test-driven-development` applies to behavior changes and `superpowers:verification-before-completion` gates completion claims.

The reason is practical rather than ceremonial. The workflow contract and Context layer are soft constraints. They work well for short tasks because the agent can keep Context, implementation and verification evidence aligned inside one manageable execution loop. In long-running tasks, Context can still capture the expected facts, but the Context-to-code step tends to drift as the context window grows, handoffs accumulate, subagents split work and verification loops multiply. The Web GPT plan, explicit long-task Skill invocation, full acceptance checklist, optional generic target prompt, optional Superpowers target prompt and Superpowers execution layer externalize the completion target so later execution can recover, audit and continue without turning Minimal Context back into a phase-gated workflow.

中文对照：短程任务直接用流程契约 + Context 层；长程任务先用 Web GPT 或外部规划模型产出方案，再显式调用 `/normal-long-task` 生成完整验收清单/普通目标模式文本；若要 Superpowers 专用目标模式文本，再显式调用 `/superpowers-long-task` 适配，然后用 Superpowers 落成执行切片。原因是 Context 到代码 这一步在长程任务中更容易漂移。
...
- The ordinary long-task Skill (`/normal-long-task`) can help before long-running execution by externalizing the acceptance bar once. A Web GPT or external planning pass can produce the source plan, `/normal-long-task` turns that plan into a full checklist and optional generic target-mode prompt, and the Superpowers long-task Skill (`/superpowers-long-task`) turns the complete packet into a Superpowers-specific prompt when needed. When the plan already contains an explicit concrete checklist, `/normal-long-task` reuses that checklist verbatim instead of generating a new one, while keeping the copied plan and checklist as separate temporary files. Test requirements belong to acceptance evidence, not a fourth artifact: required automated tests live in the full checklist, and plan-provided test requirements are preserved there rather than replaced with generic AC10 wording. Local audit may let a future executor resume acceptance progress, including each required test's command, result and failure reason, but the full checklist remains the acceptance authority and any compact prompt summary is only navigation and priority guidance. Superpowers review and finish skills remain useful execution checks, but they cannot override the full checklist. Each concrete execution slice still follows the repository's Tiny Context workflow contract and any Task Contract or `plan.md` in force.
...
The Superpowers long-task Skill (`/superpowers-long-task`) is a separate adapter for the completed acceptance packet:

- It is recommended through explicit `/superpowers-long-task` invocation after the implementation/source plan and full checklist exist.
- It is Tiny Context's adapter layer, not a Superpowers official schema.
- It stops when required packet fields are missing: original requirement source, implementation/source plan, full checklist, local audit path, relevant Context, required tests/core paths, evidence layer rules, invalid evidence rules, state-machine rules, UI-facing gate and blockers.
- It emits a visible Superpowers input packet and execution binding: install Superpowers through the current platform's official path when missing; record installation blockers in the local audit rather than treating them as completion; map original/source requirements, implementation plan, full checklist, local audit, Context and required tests/core paths into execution roles; use `superpowers:writing-plans` if the plan is not bite-sized, `superpowers:subagent-driven-development` when subagents are available, `superpowers:executing-plans` otherwise, AC gap -> TDD with `superpowers:test-driven-development` for behavior changes, and `superpowers:verification-before-completion` before any completion claim.
- It does not generate or repair the checklist and does not execute the plan; it only makes the Superpowers-specific target prompt explicit.
