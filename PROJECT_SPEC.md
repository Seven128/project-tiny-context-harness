...
Material production UI adds one conditional readiness question rather than a design phase or artifact chain. Relevant surface/interaction Context defines what the screen must let the user judge and do; `DESIGN.md` defines the visual system, one authored token source/generation direction and the interpretation of durable references; versioned authored targets remain project Source or verifier inputs. `exact-target` permits fidelity comparison only for its named conditions, `constraint` governs only its stated scope, and `inspiration` or an unconfigured starter never authorizes invented production layout. An explicitly delegated design task may author and select a target before implementation; a material unresolved preference remains a genuine decision.
...
`context_uiux_design` owns portable visual-design judgment and repairs missing or conflicting Design Authority without creating a visual workflow. It is explicitly routed for design artifacts/directions and when material production UI cannot be implemented from sufficient existing authority; ordinary implementation with an established target, local CSS fixes and throwaway prototypes remain lightweight. For material design-system, redesign or high-fidelity work it keeps a task-local, risk-proportional **Visual Coverage Set** across production surface/component, viewport, theme/mode, interaction/state, content stress and accessibility/motion conditions. The set is internal planning, not a required file, Cartesian matrix, Context role or authority. Durable surface and interaction responsibility remains in `project_context/**`; durable visual-system semantics, one authored exact token source/generation direction and the selected design-reference registry remain in `DESIGN.md`. Versioned target assets stay at project-native paths.
...
Visual Long-Task delivery is likewise an authoring/evidence specialization, not a new mechanism. Before Compile, material production UI must classify references as exact targets, partial constraints or inspiration and resolve an unconfigured/style-only gap through existing Source delegation or a genuine `decision_required` item. Material visual expectations are then written as existing atomic Requirements, applicable Controls and named Assertions, with the surface, viewport, theme/state/content condition and observable result made explicit. Existing `ui_browser` Checks prove declared browser ACs; acceptance-affecting screenshot baselines are frozen verification inputs, generated implementation screenshots/diffs remain review artifacts and cannot become their own targets, and subjective direction or approval stays an external confirmation or genuine decision. Native target proof uses a project-owned current-execution Check when representable and otherwise remains external; a browser proxy cannot prove a native target. This adds no visual Schema, Claim type, risk level, lifecycle state, baseline authority, Gate or required artifact, and it cannot infer completeness beyond declared coverage.
