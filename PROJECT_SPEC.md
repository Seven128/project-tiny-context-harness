...
- Before any completion claim, the future executor derives all views, runs `superpowers:verification-before-completion`, runs `ty-context validate-superpowers-state <dir>` and state-backed `ty-context validate-plan-acceptance <dir>`, then uses a read-only auditor when available. If auditor-driven fixes change state or evidence, derived views and both validators run again. A final stale/overclaim scan removes closed-gap wording and rejects unsafe claims such as `accepted`, `complete`, `final passed`, `product_goal_complete=true`, unapproved scope narrowing or unapproved sibling substitution. `ty-context superpowers final-gate <dir>` computes `product_goal_complete`; agents must not hand-set it. Superpowers review and verification remain useful execution checks, but they cannot override Tiny Context gates: passing Superpowers review does not by itself prove plan conformance or checklist acceptance. The validators remain structural consistency checks and do not prove product quality.
