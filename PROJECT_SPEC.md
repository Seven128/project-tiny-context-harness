...
The actual design-generation engine stays outside Tiny Context. Dedicated Open Design/Product Design, Figma, image-generation, prototype and human design systems may produce flows, low/high-fidelity targets, tokens, assets, components and prototypes in project-native formats. The base package includes `/design-resource-authoring`, a thin explicitly triggered commissioner that accepts a raw initial proposal, product/technical/design Source or an optional Source Plan independently; makes the requested output or development content its hard ceiling; subtracts sufficient selected-source coverage from the material user-visible UI/UX meaning inside that scope; discovers current Open Design capabilities; commissions/iterates only the smallest remaining resource set; and returns intent-sized previews or implementation-facing handoff provenance/coverage. A handoff may combine page/flow targets, shared component-family specifications, dedicated unique/complex-control studies, interaction/motion resources and necessary assets. No prototype, Figma route, artifact sequence, provider installation, one-file-per-control rule or pack is required. It never invokes Source Plan authoring, invents product/business/data/permission/algorithmic rules, or mutates an input proposal, Context, `DESIGN.md`, production code or a Delivery Contract.

The complete research and implementation Source Plan for this capability is indexed at [`docs/design-resource-authoring-source-plan.md`](docs/design-resource-authoring-source-plan.md). It records live provider findings, requirements, accepted boundaries, bounded experiments, failure semantics and verification impact. It remains optional upstream Source/navigation rather than registered Context, Design Authority or implementation approval. Generated candidates remain ordinary external Source and authorize no fidelity; the downstream default or Long-Task workflow performs selection reconciliation, UI Authority Closure, durable adoption, implementation and proof.
...
`/design-resource-authoring` is an independent explicit-trigger upstream Skill, not a stage inside Source Plan, default development or Long-Task authoring. It accepts the same raw drafts and mixed product/technical/design inputs directly or an optional existing Source Plan and fixes the explicit output/development content as a hard scope ceiling; broader Source is background and a partial feature may include only the surrounding context necessary to place it. For an implementation handoff it derives a task-local coverage set from every material in-scope surface/flow/region/component/control condition—layout and information hierarchy, visual/content treatment, anatomy/variants, static and dynamic states, interaction/feedback/recovery/motion, responsive/platform/input behavior, accessibility and necessary assets—then subtracts only selected Source that explicitly specifies or demonstrates those conditions. It maps repeated controls to shared component families, reserves dedicated studies for uncovered unique/complex controls and allows one inspectable artifact to cover many items; a static/default frame never silently covers unseen behavior. The task-local mapping is not a required pack, registry, readiness state or authority. The Skill discovers live Open Design agents/models/skills/templates/design systems/plugins/export routes and assigns each considered resource `selected`, `optional`, `not-needed`, `unavailable` or `decision-required`. It commissions only the smallest sufficient resources through structured MCP, then bounded CLI/daemon and UI fallbacks; it never copies provider prompts or vendors a template catalogue. Exploration stops after a visible scoped candidate and minimal sanity review; an implementation handoff stops when every material in-scope item is covered, non-applicable, excluded by the explicit scope or honestly unresolved/unavailable, so no material user-visible design decision is silently left to implementation. Selected-source preparation requires explicit human selection and immutable identity/snapshot but creates no authority. User-visible interaction semantics may be carried by designs, while product/business/data/permission/algorithmic rules remain owned by their Source. Candidate iteration does not rewrite the initial proposal. The Skill may retain task-local accepted/rejected/unresolved deltas and consolidate them once after the direction is final; a separately authorized owner decides whether and when to revise the proposal. A later explicit `/source-plan-authoring` call may consume the separately revised proposal and selected resources together, but neither Skill invokes the other.
...
Profiles are `core-portable`, `workflow-default` and explicit `long-task`. The base managed set installs `/design-resource-authoring`; it adds no Open Design runtime or provider dependency. `ty-context enable long-task` additionally installs the Source Plan Skill, Long-Task Skill, Stop Hook and templates. Disable removes only those Long-Task-owned surfaces and preserves the base Skill plus user entries. Upgrade removes package-owned retired assets and leaves user historical files untouched.
