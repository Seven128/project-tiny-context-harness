...
这套工作流主要降低三类成本：
- 阶段执行成本：通过阶段角色 Prompt、模板和上下文约束，让 Agent 在对应阶段更快产出符合预期的交付物。
...
### 2.3 单阶段主要依靠 vibe 推进，但阶段 Prompt 与交付硬约束需要固定进工作流
现状： 在单个阶段内部，Agent 的主要工作方式仍然是 vibe：人给出目标，Agent 结合上下文进行生成、修改、总结、补充和修正。阶段角色 Prompt 不替代 vibe，而是沉淀该阶段的最佳实践，用来提高 vibe 的效率和稳定性。
不同阶段需要不同 Prompt：PM 阶段需要需求澄清、边界定义和验收标准；架构阶段需要模块拆分、接口契约和风险识别；开发阶段需要按任务落地、控制修改范围和补测试；Review 阶段需要只读审查、风险分级和需求一致性检查；测试阶段需要边界条件、回归范围和覆盖矩阵。
...
要解决的问题： 需要把阶段 Prompt 和交付硬约束都固定进工作流。Prompt 负责提高单阶段 vibe 的产出效率；硬约束负责保证阶段交付质量。硬约束不一定由 Harness 自己执行，但必须被声明为阶段完成和状态流转的判断依据。Agent 可以触发脚本、修复失败、记录结果，但不能绕过这些约束直接推进状态。
## 三、采用的方案
### 3.1 总体思路
...
Agent 在单阶段内部仍然以 vibe 方式执行；Harness 负责规定当前阶段、应读内容、应写产物、应使用 Prompt、完成前必须通过的 gate，以及需求变更时如何局部修正链路。

### 3.2 核心设计原则
- 阶段契约化：每个阶段都有输入、输出、Prompt、gate 和下一阶段入口。
- 产物仓库化：关键产物进入 `.docs/`、`.agent/` 或同一工作区，成为可寻址、可版本化事实源。
- 语义切片化：阶段文档按业务能力、技术主题、任务、风险或变更事件切片，避免长文档被固定 chunk 检索时丢失边界信息。
- Prompt 阶段化：每个 Prompt 只沉淀一个阶段或动作的 SOP，不写成巨型 prompt。
...
- 自举配置分层：开发 Harness 自身时可以有只服务于本仓库的 authoring overlay，用于记录工作流演进原则、包化约束和专用 Prompt；这些内容默认不进入通用 npm 包，也不默认分发给用户项目。

### 3.3 事实源与派生产物
真正的事实源是：
- `.agent/state/*.yaml`
- `.agent/pjsdlc_managed/policies/*.yaml`
- `<harnessRoot>/prompts/workflow/*/PROMPT.md`
- `.docs/**/*.md`
- `.docs/INDEX.md`
- `Makefile`
- `tools/*.py`
...
- `<harnessRoot>/prompts/`：阶段角色 Prompt 的 canonical source。默认 `<harnessRoot>` 是 `.agent`；当前仓库遵循默认值，因此使用 `.agent/prompts/workflow/**`。
...
除 `.agent/prompts/workflow/**` 需要保持 Agent hard index 之外，工作流相关默认配置统一放在 `.agent/pjsdlc_managed/**`。不要再维护 `.agent/policies/**` 或 `.agent/templates/**` 这类 legacy mirror，避免同一份配置出现多个事实源。
...
| 阶段 | Prompt | 主要输入 | 主要输出 | 出口 Gate | 下一阶段 |
|---|---|---|---|---|---|
...
## 八、阶段 Prompt
每个 Prompt 只负责一个阶段或动作。

通用阶段 Prompt 不只是文档切片规则，也必须包含可直接用于对话的专业角色提示词。角色提示词负责说明该角色如何澄清需求、提出取舍、识别 blocker、与用户共同生成阶段产物，并把产物落到 `.docs/`、`plan.yaml`、gate 或实现记录中。因为这些 Prompt 会作为 npm 包默认能力分发，角色提示词应保持通用、专业、强约束，不绑定具体业务项目，也不要求用户直接修改 managed Prompt。
...
- 不翻译 `current_phase`、`active_prompt`、`allowed_paths`、`required_gates`、`implementation_doc` 等字段名。
...
- 新增机制：阶段契约、统一事实源、阶段 Prompt、执行计划、交付 gate、实现文档、RFC、overview。
- 改变层级：不是提升 Agent 单次生成能力，而是提升 Agent 参与复杂软件工程时的阶段衔接和交付可验证性。
- 收益来源：降低阶段执行成本、阶段衔接成本和阶段交付成本。

## 十六、总结
AI SDLC Harness 是面向 AI Agent 的需求全链路工作流骨架。它把阶段目标、阶段产物、阶段 Prompt、执行计划、交付 gate、实现文档、语义切片、派生总览和 RFC 变更协议固定进仓库。
...
### 17.1 工作流配置的定义与范围
这里的“工作流配置”不只是一组 prompt 或 Prompt，而是定义 Harness 如何运行的一整套协议：

- Agent 入口和角色规则：`AGENTS.md`、`<harnessRoot>/prompts/workflow/**/PROMPT.md`。
...
普通业务项目通过 `sdlc-harness init` 得到工作流入口、Prompt、policy、template、state 初始结构和 `.docs/**` 产物目录；本仓库也拥有同样的工作流入口和运行状态。区别在于：普通业务项目在这套工作流之上开发业务系统，而本仓库在这套工作流之上开发 Harness 工作流本身。
...
1. 迭代 AI SDLC Harness 工作流配置本身：
   - 调整阶段规则、Prompt、policy、template、state protocol、plan protocol、memory protocol 和 validators。
...
prompts / policies / templates / sync / upgrade / migrations
```
...
- 当新增 package sync、migration、validator、Prompt、模板或策略时，Agent 应额外读取哪些约束？
- 某条自举规则什么时候应该晋升为通用 Harness 能力？

推荐预留目录：

```txt
.agent/prompts/authoring/
├── principles/
│   ├── package_workspace_decoupling.md
│   ├── migration_safety.md
│   └── source_sync_boundary.md
├── prompts/
│   ├── harness_package_design/PROMPT.md
│   └── harness_migration_review/PROMPT.md
└── policies/
...
|---|---|---|---|
| 通用 Harness 配置 | `.agent/prompts/workflow/**`, `.agent/pjsdlc_managed/**` | 是 | 面向所有接入项目的阶段 Prompt、模板、策略和默认规则 |
| 项目实例数据 | `.agent/state/**`, `.docs/**` | 否 | 当前项目的状态、需求、方案、实现、测试和发布事实 |
| Harness authoring overlay | `.agent/prompts/authoring/**` | 否 | 只约束本仓库迭代 Harness 自身的原则、专用 Prompt 和包化安全规则 |

Authoring overlay 的默认规则：
- `AGENTS.md` 可以声明本仓库作为 authoring workspace 时需要额外读取 `.agent/prompts/authoring/**`。
- `package sync-source` 默认不复制 `.agent/prompts/authoring/**` 到 `packages/sdlc-harness/assets/**`。
- `sdlc-harness sync` 和 `upgrade` 默认不把 authoring overlay materialize 到用户项目。
- 如果某条 authoring rule 对所有用户项目都有价值，必须通过 PRD / tech plan / RFC 明确晋升为通用 Prompt、policy、template、PROJECT_SPEC 或 README 规则，再进入包内 canonical assets。
- 如果某条 authoring Prompt 只服务于 Harness 包源码维护，例如 package source drift、migration safety、managed block compatibility，就应留在 authoring overlay，不污染通用阶段 Prompt。
...
| 影响面 | 需要遵守的原则 |
|---|---|
| README / PROJECT_SPEC / AGENTS | README 说明用户接入和日常使用；PROJECT_SPEC 说明分层模型和设计取舍；AGENTS 可声明本仓库额外读取 `.agent/prompts/authoring/**`。 |
...
| Prompt 设计 | 通用阶段 Prompt 放 `.agent/prompts/workflow/**`；维护 Harness 包自身的专用 Prompt 放 `.agent/prompts/authoring/**`。 |
...
- 默认 `<harnessRoot>/prompts/workflow/*/PROMPT.md`。
...
- `AGENTS.md`。
- `<harnessRoot>/prompts/workflow/**`，由 `sdlc-harness sync` 从包内 materialize 到工作区，作为 Prompt canonical source。
...
未配置时，默认 `<harnessRoot>` 是 `.agent`，因此 `init` 会生成 `.agent/config.yaml`、`.agent/state/**`、`.agent/prompts/workflow/**` 和 `.agent/pjsdlc_managed/**`。当前仓库遵循默认值，因此所有 Harness 配置都位于 `.agent/**`，Prompt 直接位于 `.agent/prompts/workflow/**`，不再经过额外中间目录。

`harnessFloderName` 作为历史兼容别名会被读取，但新配置应使用 `harnessFolderName`。

### 18.4 为什么仍要同步到工作区
多数 Agent 在启动或路由 Prompt 时，只读取工作区内固定目录，例如 `AGENTS.md`、`.agent/prompts/workflow/**/PROMPT.md` 或类似约定。它们通常不会直接扫描 `node_modules` 中的包内容。

因此 npm 包不能只把 Prompt 藏在包里。正确流程是：
...
`sync` 负责把包内默认 Prompt、模板、策略文件和默认 Makefile targets materialize 到工作区固定位置，并为 managed files 写入版本和 checksum metadata。
...
4. 更新 managed files。
5. 自动执行 sync，把最新 Prompt、模板、策略、默认 Makefile targets 和状态模板 materialize 到工作区。
6. 运行 `sdlc-harness doctor` 或对应 `make validate-harness`，输出升级报告。

### 18.7 包与项目解耦原则
Harness npm 包的设计必须像普通 npm 依赖一样，与用户仓库内容保持边界清晰。包可以提供默认规则和升级能力，但不能假设用户仓库仍是初始化时的模板状态；用户可能已经改过 `AGENTS.md`、`Makefile`、local policies、Prompt overrides、模板覆盖、状态数据或业务文档。
...
- 除 `<harnessRoot>/prompts/workflow/**` 作为 Agent hard index 保留在固定位置外，模板、策略、默认 Makefile targets 等工作流配置都必须收敛到 `<harnessRoot>/pjsdlc_managed/**`，不再生成 `<harnessRoot>/templates/**`、`<harnessRoot>/policies/**` 等 legacy mirror。
...
```txt
<harnessRoot>/overrides/prompts/<prompt-name>.md
