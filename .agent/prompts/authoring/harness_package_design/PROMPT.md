...
# Harness Package Design Authoring Prompt

## 目的

只服务于本仓库作为 AI SDLC Harness authoring workspace 时的工作流迭代。它补充通用
`pjsdlc_*` Prompt，帮助 Agent 在修改 Harness 自身、npm package、sync/upgrade/migration、
...
tech plan 或 RFC 明确晋升路径，再进入通用 Prompt、policy、template、`PROJECT_SPEC.md` 或
`README.md`。
...
- 通用 Harness 配置：应进入 `.agent/prompts/workflow/**`、`.agent/pjsdlc_managed/**` 或 package assets。
- 项目实例数据：只描述当前仓库状态和 `.docs/**` 产物，不应被 sync/upgrade 覆盖。
- Harness authoring overlay：只约束本仓库维护 Harness 自身的原则、专用 Prompt 和包化安全规则。
...
3. 不要把 `.agent/prompts/authoring/**` 同步到 `packages/sdlc-harness/assets/**`。
...
- [ ] 已确认 `.agent/prompts/authoring/**` 没有进入 package assets。
